	...
	async fetchSource(
		...
		if (cursor?.cursor) {
			url.searchParams.set(
				"blockNumber_gte",
				String(BigInt(cursor.cursor) + 1n),
			);
		}
		...
		if (!response.ok) {
			throw new Error(
				`Tally indexer request failed: ${response.status} ${response.statusText}`,
			);
		}
		...
		for (const rawEvent of rawEvents) {
			...
			items.push({
				...
				observedAt: new Date(
					Number(decoded.blockTimestamp) * 1000,
				).toISOString(),
...
export function createTallyIndexerBeatSourceAdapter(
	config: TallyIndexerBeatSourceAdapterConfig = {},
...
function normalizeBaseUrl(value: string | undefined): string {
	if (!value) {
		throw new Error(
			"Tally indexer source requires a locator URL or indexerBaseUrl",
		);
	}
