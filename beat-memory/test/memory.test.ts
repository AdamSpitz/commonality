...
describe("beat context memory", () => {
	it("extracts timestamped observations from ingested items and deduplicates them", async () => {
		await withTempDir(async (dir) => {
			...
			assert.equal(
				state.observations[0]?.observedAtStart,
				"2026-05-12T10:00:00.000Z",
			);
			assert.deepEqual(state.observations[0]?.supportingContentIds, [
				"twitter:tweet:1",
			]);
			assert.deepEqual(state.observations[0]?.sourceAuthors, [
				"twitter:handle:river",
			]);
			...
			assert.equal(
				(await loadBeatContextMemoryState(memoryFilePath)).observations.length,
				2,
			);
		});
	});
	...
	describe("retry/backoff for extraction failures", () => {
			...
			await withTempDir(async (dir) => {
				...
				const extractor: BeatObservationExtractor = {
					extractObservations: async (item) => {
						...
						return [
							{ observation: `Observed: ${item.text}`, confidence: "medium" },
						];
					},
				};
				...
				assert.equal(
					summary.retriedItemCount,
					1,
					"item that needed retries should be counted",
				);
		...
		it("marks an item failed after exhausting all retry attempts", async () => {
			await withTempDir(async (dir) => {
				...
				assert.equal(
					summary.failedItems[0]?.errorMessage,
					"Persistent LLM error",
				);
				assert.equal(
					summary.retriedItemCount,
					0,
					"item that failed all attempts is not counted as retried",
				);
		...
		it("retries only the failing item while processing others normally", async () => {
			await withTempDir(async (dir) => {
				...
				assert.equal(
					summary.observationCount,
					2,
					"both items should eventually succeed",
				);
				assert.equal(summary.failedItemCount, 0);
	...
	it("generates purpose-level summary snapshots above detailed observations", async () => {
		await withTempDir(async (dir) => {
			...
			const summary = await generatePurposeSummarySnapshots({
				...
				recentMetrics: {
					ingestion: { newItemCount: 2, skippedSourceCount: 0 },
				},
			});
			...
			assert.deepEqual(
				state.purposeSummarySnapshots
					?.map((snapshot) => snapshot.purpose)
					.sort(),
				["bridge_opportunity_context", "civility_context"],
			);
			assert.match(
				state.purposeSummarySnapshots?.[0]?.summary ?? "",
				/Recent .* context/u,
			);
			assert.ok(
				state.purposeSummarySnapshots?.some(
					(snapshot) => snapshot.sourceObservationIds.length > 0,
				),
			);
			assert.ok(
				state.purposeSummarySnapshots?.every(
					(snapshot) => snapshot.sourceCoverageNotes.length > 0,
				),
			);
		});
	});
		...
		await withTempDir(async (dir) => {
			...
			await saveBeatContextMemoryState(memoryFilePath, {
				...
				observations: [
					...
					},
					{
						...
						observation:
							"older evidence says the same phrase was previously ambiguous",
			...
			const generator: BeatPurposeSummarySnapshotGenerator = {
				createSnapshot: async (params) => {
					...
					assert.deepEqual(
						params.recentObservations.map((obs) => obs.id),
						["recent-1"],
					);
					assert.deepEqual(
						params.compactedObservations.map((obs) => obs.id),
						["old-summary"],
					);
					return {
			...
			assert.equal(
				state.purposeSummarySnapshots?.[0]?.summary,
				"LLM-authored semantic snapshot",
			);
			assert.deepEqual(state.purposeSummarySnapshots?.[0]?.liveTopics, [
				"border compromise",
			]);
			assert.deepEqual(
				state.purposeSummarySnapshots?.[0]?.sourceObservationIds,
				["recent-1"],
			);
		});
	});
	...
	it("generates source-management observations from purpose snapshots and coverage signals", async () => {
		await withTempDir(async (dir) => {
				...
				purposeSummarySnapshots: [
					{
						...
						uncertainties: [],
						recurringGaps: [
							"Repeated abstentions concern authors outside the current source set.",
						],
						usefulContext: [],
						sourceCoverageNotes: [
							"Coverage looks limited and factionally skewed toward one camp.",
						],
						sourceObservationIds: [],
					},
				],
			...
			const summary = await generateSourceManagementObservations({
				...
				coverageGapNotes: [
					"outside_beat requests mention @newparticipant through quotes",
				],
				outcomeNotes: [
					"query source is producing mostly off-beat campaign chatter",
				],
			});
			...
			const state = await loadBeatContextMemoryState(memoryFilePath);
			const sourceManagement = state.observations.filter((observation) =>
				observation.purposes?.includes("source_management"),
			);
			assert.equal(sourceManagement.length, summary.observationCount);
			assert.ok(
				sourceManagement.some((observation) =>
					/outside_beat requests/u.test(observation.observation),
				),
			);
			assert.ok(
				sourceManagement.every((observation) =>
					observation.sourceAuthors.includes("beat-agent-source-management"),
				),
			);
		});
	});
	...
	it("generates advisory source-management reports without applying updates", async () => {
		await withTempDir(async (dir) => {
				...
				observations: [
					{
						...
						kind: "item_observation",
						observation:
							"Source-management evaluation-demand signal: repeated outside_beat requests mention a missing labor organizer source.",
			...
			const summary = await generateSourceManagementReport({
				beatDefinition: {
					beatId: "us-political-twitter",
					purposes: ["civility_context", "source_management"],
					sources: [
						{
							id: "query:seed",
							type: "query",
							locator: "immigration reform",
							platform: "twitter",
						},
					],
				},
			...
			assert.equal(
				state.sourceManagementReports?.[0]?.health.underCovered,
				true,
			);
			assert.ok(
				state.sourceManagementReports?.[0]?.proposedUpdates.some(
					(update) => update.action === "ask_manager",
				),
			);
			assert.deepEqual(
				state.sourceManagementReports?.[0]?.effectiveSourceList.map(
					(source) => source.id,
				),
				["query:seed"],
			);
			...
			await saveBeatContextMemoryState(memoryFilePath, {
				...
				observations: [
					...
					},
					{
						...
						sourceAuthors: ["author-2"],
						keywords: ["bridge", "caucus"],
						purposes: ["bridge_opportunity_context"],
						createdAt: "2026-05-15T00:00:00.000Z",
					},
				],
			});
	...
	it("downweights thinly sourced observations by source diversity and time span", async () => {
		await withTempDir(async (dir) => {
			...
			const baseObservation = {
				...
				observation:
					"secure borders is being used as good faith compromise language",
				observedAtStart: "2026-05-15T00:00:00.000Z",
				observedAtEnd: "2026-05-15T00:00:00.000Z",
	...
	it("uses plain-text tags to retrieve observations when body keywords are sparse", async () => {
		await withTempDir(async (dir) => {
				...
				observations: [
					{
						...
						kind: "item_observation",
						observation:
							"Participants are using this as a callback to yesterday's argument.",
						observedAtStart: "2026-05-15T00:00:00.000Z",
						observedAtEnd: "2026-05-15T06:00:00.000Z",
						confidence: "medium",
						supportingContentIds: ["twitter:tweet:1"],
			...
			const relevant = await retrieveRelevantObservations({
				...
				memoryFilePath,
				queryText:
					"Does @river mean the kitchen table coalition argument here?",
				now: new Date("2026-05-15T12:00:00.000Z"),
			});
	...
	it("treats backfilled observations without sourceAuthors neutrally", async () => {
		await withTempDir(async (dir) => {
			...
			assert.deepEqual(state.observations[0]?.sourceAuthors, []);
			assert.equal(
				calculateObservationDiversityMultiplier(state.observations[0]!),
				1,
			);
		});
	});
	...
	it("compacts older item observations into a coarse summary", async () => {
		await withTempDir(async (dir) => {
			...
			assert.equal(
				state.observations[0]?.observedAtEnd,
	...
	describe("compactBeatMemory with LLM compactor", () => {
		it("uses compactor summary text when compactor is provided", async () => {
			await withTempDir(async (dir) => {
				...
				const compactor: BeatMemoryCompactor = {
					createSummary: async () =>
						"LLM semantic summary of border and coalition discourse.",
				};
				...
				assert.ok(
					state.observations[0]?.keywords.includes("border") ||
						state.observations[0]?.keywords.includes("coalition"),
		...
		it("falls back to keyword summary when compactor returns empty string", async () => {
			await withTempDir(async (dir) => {
				...
				const state = await loadBeatContextMemoryState(memoryFilePath);
				assert.ok(
					state.observations[0]?.observation.startsWith("Compacted"),
	...
	describe("stale-observation tracking", () => {
		it("sets lastActiveAt on an existing observation when a new item shares keywords", async () => {
			await withTempDir(async (dir) => {
				...
				const state = await loadBeatContextMemoryState(memoryFilePath);
				const reinforced = state.observations.find(
					(obs) => obs.id === "obs-border",
				);
				assert.equal(
					reinforced?.lastActiveAt,
		...
		it("does not set lastActiveAt on an observation whose keywords do not overlap with new items", async () => {
			await withTempDir(async (dir) => {
				...
				const state = await loadBeatContextMemoryState(memoryFilePath);
				const notReinforced = state.observations.find(
					(obs) => obs.id === "obs-kitchen",
				);
				assert.equal(
					notReinforced?.lastActiveAt,
					undefined,
		...
		it("downweights a stale compacted summary relative to a fresh item_observation on the same topic", async () => {
			await withTempDir(async (dir) => {
				...
				const staleCompactedSummary: BeatMemoryObservation = {
					...
					kind: "compacted_summary",
					observation:
						"Border debates dominated April. Compromise language was key.",
	...
	describe("detectContestedObservations", () => {
		...
		it("detects a contested group when observations share keywords but have disjoint author sets", () => {
			...
			assert.ok(
				groups[0]!.description.includes("border") ||
					groups[0]!.description.includes("policy"),
			);
		});
		...
		it("deduplicates groups with the same top-keyword signature", () => {
			...
			assert.equal(
				groups.length,
				1,
				"same keyword signature should not produce multiple groups",
			);
		});
		...
		it("detects multiple contested groups for different keyword clusters", () => {
			...
			assert.equal(
				groups.length,
				2,
				"should detect two distinct contested keyword clusters",
			);
