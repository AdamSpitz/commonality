...
describe("beat ingestion", () => {
	it("fetches configured sources, persists items, and deduplicates canonical IDs", async () => {
		await withTempDir(async (dir) => {
			...
			assert.equal(
				persisted.sourceCursors["accounts:moderates"]?.cursor,
				"page-1",
			);
	...
	it("continues ingesting other sources when one source fetch fails", async () => {
		await withTempDir(async (dir) => {
					...
					sources: [
						{
							id: "rss:ok-before",
							type: "rss",
							locator: "https://example.com/before.xml",
						},
						{
							id: "rss:failing",
							type: "rss",
							locator: "https://example.com/failing.xml",
						},
						{
							id: "rss:ok-after",
							type: "rss",
							locator: "https://example.com/after.xml",
						},
					],
				},
				stateFilePath,
			...
			assert.equal(
				persisted.sourceCursors["rss:ok-before"]?.cursor,
				"cursor:rss:ok-before",
			);
			assert.equal(persisted.sourceCursors["rss:failing"], undefined);
			assert.equal(
				persisted.sourceCursors["rss:ok-after"]?.cursor,
				"cursor:rss:ok-after",
			);
		});
	});
...
describe("detectIngestionAnomalies", () => {
	...
	it("respects custom thresholds", () => {
		...
		const lenient = detectIngestionAnomalies(items, {
			lowDiversityThreshold: 0.1,
		});
		assert.deepEqual(
			lenient,
			[],
			"should not flag when diversity > 0.1 threshold",
		);
		const strict = detectIngestionAnomalies(items, {
			lowDiversityThreshold: 0.5,
		});
		assert.equal(
			strict.length,
			1,
			"should flag when diversity < 0.5 threshold",
		);
