...
export function calculatePaymentRequired(currentGasPriceWei, config) {
    const modelPricing = LLM_PRICING[config.openRouterModel] || LLM_PRICING['anthropic/claude-3.5-haiku'];
    const llmCostUsd = (config.estimatedInputTokens / 1_000_000) * modelPricing.inputPer1M +
...
export function getPaymentFromHeader(xPaymentProof) {
...
export function formatPaymentProof(paymentId) {
    return `payment:${paymentId}`;
}
export function createPaymentRequiredResponse(details) {
    return {
