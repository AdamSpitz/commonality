...
// Generalized to support topicStatementId for indexer filtering (per specs/decoupling.md)
...
contract ProjectAlignment {
     ...
     * @param topicStatementId The IPFS CID of the topic for indexer filtering (can be bytes32(0) for no topic)
     */
     ...
     * @param topicStatementId The IPFS CID of the topic for indexer filtering (can be bytes32(0) for no topic)
     */
    function attestAlignment(address projectAddress, bytes32 statementId, bytes32 topicStatementId)
        external
    {
        require(
        ...
        emit ProjectAlignmentAttestation(msg.sender, projectAddress, statementId, topicStatementId);
    }
     ...
     * @param topicStatementIds Array of topic IPFS CIDs for indexer filtering (can contain bytes32(0) for no topic)
     */
