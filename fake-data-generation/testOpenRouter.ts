...
async function testSingleEvaluation(): Promise<void> {
  ...
  try {
    ...
    const usage = result.rawLlmResult.usage as Record<string, number> | null;
    if (usage) {
...
async function testBatchEvaluation(numTests = 3): Promise<void> {
...
async function testAttesterEvaluations(): Promise<void> {
  ...
  const attestersByType: Record<string, Attester[]> = {};
  attesters.forEach(a => {
...
async function printCostEstimate(): Promise<void> {
