...
async function generateAttestations(maxPairsPerDomain = 50) {
    ...
    for (const [domain, domainStatements] of Object.entries(statementsByDomain)) {
        ...
        for (const pair of selectedPairs) {
            try {
                const result = await evaluateImplicationWithLLM(pair.statement1, pair.statement2, apiKey);
...
async function getAttestation(fromStatementId, toStatementId) {
    ...
    return attestations.find(a => a.fromStatementId === fromStatementId && a.toStatementId === toStatementId) ?? null;
}
/**
...
export { generateAttestations, loadAttestations, getAttestation, getAttestationsForStatement, hasAttestations, clearAttestations };
