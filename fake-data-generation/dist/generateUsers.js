...
function generateWealth() {
    ...
    if (rand < 0.01)
        return 100; // 1% are whales
    if (rand < 0.1)
        return 10; // 9% are large holders
...
function pickRandomPosition(domainConfig) {
...
function addCorrelations(interests) {
    const politics = interests.politics;
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
    ...
    const hardhatAccountCount = parseInt(args.find(a => a.startsWith('--hardhat-count='))?.split('=')[1] ?? String(DEFAULT_ACCOUNT_COUNT));
