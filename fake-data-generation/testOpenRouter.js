 ...
 * 
 * Usage:
 *   node testOpenRouter.js [numTests]
 * 
 ...
 * 
 * Example:
 *   OPENROUTER_API_KEY=sk-or-xxx node testOpenRouter.js 3
 */
...
async function testSingleEvaluation() {
  ...
  console.log(`\nStatement 1: "${stmt1.content?.text || stmt1.text}"`);
  console.log(`Statement 2: "${stmt2.content?.text || stmt2.text}"`);
  console.log('\nEvaluating...\n');
  ...
  try {
    ...
    if (result.rawLlmResult.usage) {
      console.log(`  Tokens: ${result.rawLlmResult.usage.total_tokens} ` +
        `(${result.rawLlmResult.usage.prompt_tokens} prompt + ` +
        `${result.rawLlmResult.usage.completion_tokens} completion)`);
    }
  } catch (error) {
...
async function testAttesterEvaluations() {
  ...
  attesters.forEach(a => {
    if (!attestersByType[a.type]) attestersByType[a.type] = [];
    attestersByType[a.type].push(a);
  });
  
  // Test one of each type
  const testAttesters = [];
  for (const [type, list] of Object.entries(attestersByType)) {
    if (list.length > 0) {
...
async function main() {
  ...
  const validation = await validateOpenRouterSetup(API_KEY);
  ...
  try {
    ...
    console.error('  node generateAttesters.js 10');
    console.error('  node generateStatements.js');
    process.exit(1);
  }
