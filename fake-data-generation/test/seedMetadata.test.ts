...
test('crime-repeat-offenders seed collection copies the accepted exercise-4 statement texts', async () => {
  const exercise = JSON.parse(await readFile(
    new URL('../statement-generation-exercises/04-crime-repeat-offenders.json', import.meta.url),
    'utf8',
  )) as { groups: Array<{ statements: Array<{ id: string; role: string; text: string }> }> };
  const accepted = exercise.groups[0]?.statements ?? [];
  const live = flattenSeedStatements(await loadSeedCollections())
    .filter((record) => record.collection.id === 'crime-repeat-offenders')
    .map((record) => record.statement);
