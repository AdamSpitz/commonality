...
- [ ] **(Ask)** Decide the claim-link approach for tradfi donors without a wallet. Linkdrop SDK V3 has been evaluated and is the recommended path — skip a custom `TradFiBridgeEscrow` (see [bridges.md](docs/end-user/commonality/vision-and-strategy/ease-of-adoption/bridges.md): ERC-1155 on Base, gasless relay claim, built-in timeout-reclaim). The refund question is settled: because `refundERC1155` sends USDC to whoever holds the tokens, failed-contract refunds are best handled by the operator operationally, and a custom escrow wouldn't actually help (it does nothing once the donor has claimed). The one real open decision before committing: **does Commonality use Linkdrop's hosted relay, or run its own?** The relay sponsors gas on the *claim* side, which is our product surface — so this burden is ours, not the operator's (can't be pushed to the charity like refunds can). Needs a small spike to confirm the relay self-hosts cleanly and the per-claim fee/gas model. Note self-hosting the relay still leaves us on Linkdrop's deployed escrow *contracts* unless we fork those too (full independence ≈ having built `TradFiBridgeEscrow` anyway). Bridge-operator `buyERC1155` flow is already documented.
