...
- **(Tell)** Finished the remaining recurring-pledges ops on Base Sepolia. Incremental deploy plan showed `RecurringPledges` already live and unchanged at `0x91a8FcfB698BCd0642857C22196057ab2568D62b` (no contract redeploy). Funded `RECURRING_PLEDGE_SCHEDULER_ADDRESS` with 0.005 ETH, flipped `RECURRING_PLEDGE_SCHEDULER_ENABLED=true` in the Render template + generated `render.yaml`, copied scheduler RPC/key onto `commonality-service-host-workers`, and redeployed that worker. A 60s test pledge (`id=1`) produced a first `StandingPledgeExecuted` in the create tx (`0xacf3…`) and a scheduler-driven second execution (`0xc5f8…`, note 2) that the indexer event cache served. The test pledge was then cancelled so it would not keep firing.

- **(Tell)** Added and ran the causes-as-publications migration/regression gate: a frozen corpus now proves that v1/v2 local CauseStarter drafts retain exact wording and statement/CID associations while migrating together, and that an immutable v1 roster captured from the local IPFS corpus still decodes. The full CauseStarter suite (86 tests) and package typecheck pass. The rollout now has only the unsettled public delegate-offering semantics and non-expert user validation left.

- **(Tell)** Surfaced recurring-pledge demand on CauseStarter cause and statement pages. Multi-plank causes sum each distinct published statement's active monthly pledges and show the configured settlement currency, explicitly labeled as revocable auto-pull interest rather than guaranteed funding. My Notes now resolves each pledge's cause title and links to the statement instead of exposing a raw CID. Focused CauseStarter/UI tests and both production builds pass.
