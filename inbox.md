...
- **(Ask) Subscribing to a cause's mediator is a one-way door out of CauseStarter — worth a "profile" page?** The cause page's opt-in button deep-links to Tally `/settings?addNudger=…`, which enables the nudger; the resulting nudges then only ever render in Tally (`conceptspace/components/StatementSuggestions.tsx`, on statement detail pages). A supporter who lives in CauseStarter subscribes and sees *nothing happen* — the payoff is on another domain, later, on pages they may never visit. Idea: a profile page showing your causes **plus** your subscribed nudgers and the nudges you've received. Cheaper interim option is rendering a mediator's recent nudges against the cause's own planks on the cause page (`getStatementNudges` over `publishedCids`). Related: trusted nudgers are `localStorage` too, so the subscription doesn't follow you across devices — same underlying problem as the cause-drafts item above, and a profile page probably wants both solved together.
