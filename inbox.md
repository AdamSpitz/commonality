...
- In the verifier workspace, I want to make sure the various "manual" verifier checks (i.e. the ones that use LLMs) run with a sufficient understanding of the system. That is, the "strongest" (but most expensive) version of an LLM-using check is" get an LLM up to speed (i.e. read quite a lot of the docs and specs, including the "founder"-level ones), then tell it to look at one page or use case or aspect or whatever. If we did all the tests that way, then the "test suite" would consist of a big list of many different aspects of the project (broken down along many dimensions: break down each site feature by feature, or look at scalability, or documentation coherence, or robustness, or whatever), and then each item is to be read as "read all the docs to get yourself up to speed with a founder-level understanding of the project, then look at X". Now, that's probably overly expensive (we can probably )
