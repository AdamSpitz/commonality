...
- [ ] **(Ask → mostly resolved)** Statement hosting/display posture (Jul 2026): drop the general-purpose Tally browser in favor of Tally-as-module; per-vertical + per-author pinning instead of "we pin everything"; users self-publish statements in the calldata of their own signing transaction (pay-once, no host role for us — deliberately *not* we-upload-to-Arweave). A Jul 2026 design-resolution pass settled the three conceptual blockers: **statement identity = CID** (publication is an internal `(publisher, cid)` fact used only for availability + retraction; multiple publishers compose by OR); **`supportStatement` does not require `isPublished`** (support is a pure attestation about a CID, threaded-forum semantics); and **retraction suppresses only the retracting publisher's copy** (aggregation follows rendering). Also nailed down the speaker-vs-redistributor liability distinction (the user is the attributed *speaker* even though the vertical runs the UI; two independent retraction paths — real, foundational law in the general case, but the on-chain *mapping* is untested and wants counsel before mainnet). Remaining before build are two purely-technical scheduling-time items: CID representation format and the calldata/event cost benchmark. See [statement-hosting.md](specs/product/legal/statement-hosting.md), [self-published-statements.md](specs/tech/subsystems/conceptspace/self-published-statements.md), and the readiness note in [PublishedData](specs/tech/subsystems/published-data/README.md#readiness-note-design-resolved-except-two-technical-benchmarks-jul-2026).
