...
- [ ] **(Ask)** What is a cause actually made of? Working proposal in [shaping-your-cause-statements.md](docs/founder/shaping-your-cause-statements.md): **planks / views / anchors**. Single-issue statements are the atoms; conjunction and disjunction are both useful but neither has to be *published* to be displayed, because union/intersection over planks are client-side set operations — so a cause site can let visitors toggle issues and switch between "signed all of these" and "signed at least one" for free. Publishing an anchor is only needed for what a view can't do: sign, earmark, align. So "a cause has one main statement" becomes a default promoted view rather than a structural rule, with a promotion path (watch which combination people converge on, then reify it). Two rules that fell out and are worth keeping even if the rest changes: a conjunction view must be shown as **two bands** ("signed all five" plus "signed some, disagreed with none"), because `noOpinion` is the default and a strict intersection collapses to near-zero on silence rather than disagreement — this is what the three-valued belief state buys us; and **align low, aggregate high** (attach alignment attestations and earmarks to planks, never conjunctions, since a conjunction has no inbound arrows and propagates nowhere). **You said this isn't finished thinking, just progress**; open items are at the end of the doc, the live ones being scale of client-side signer-set folding and how direct-vs-indirect support stays legible inside a two-band intersection view. Also concluded: cause-assist is doing the mediator's job (same "attester will bless it *and* someone will sign it" needle; *coalition unbundling* in [hidden-majority-patterns.md](docs/end-user/common-sense-majority/hidden-majority-patterns.md) is this exact problem from the other side), so it should be a strategy config on the bridge-creator engine per [bridge-building-for-founders.md](specs/product/bridge-building-for-founders.md) — sharing the engine and pattern catalog but never a strategy prompt, since a founder mobilizes a side where the mediator de-polarizes one.

- [ ] **(Tell → decide if worth queuing)** Bug found while checking the above: **NoteIntent earmarks don't reliably roll up.** `getNoteIntentAttestationsByStatement` is exact-match on statement CID; only `getTotalFundingForCause` expands, and it sources its CID set from the *alignment* traversal, so a plank with earmarked money but no aligned projects yet contributes nothing — while the cause board's own "Earmarked funds" panel doesn't roll up at all and can disagree with the total beside it. Independent of the statement-shape question, though note the views model routes around it (a view queries each plank exactly and unions client-side). Details in the open-questions section of the doc above. Also note CauseStarter today gates supporting statements on main→supporting, which is sound only for the conjunctive shape and unsound for a hedged/broad main statement — worth deciding before more is built on it.
