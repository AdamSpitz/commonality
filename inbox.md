...
- **(Tell)** Implemented the core causes-as-publications authoring shift: CauseStarter now uses a reusable retrieval-first statement picker, loads exact immutable text before CID reuse, offers rejection/“none fit”/manual correction paths, sends only post-retrieval gaps to cause-assist drafting, and records content-free local funnel events. Existing drafts, roster versions, MutableRefs, optional-anchor behavior, and derived views remain intact. Implication and cause-assist guidance now accept determinate breadth/rhetoric removal while rejecting added concessions, and assistance is explicitly separated from mediation. Focused CauseStarter, implication-attester, and cause-assist tests/typechecks/lints pass. The remaining cross-workflow reuse and full journey/user validation stay unchecked in the implementation plan and the root TODO remains open.
