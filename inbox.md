...
- [ ] **(Ask)** Decide the claim-link approach for tradfi donors without a wallet. Linkdrop SDK V3 has now been evaluated (see [bridges.md](docs/end-user/commonality/vision-and-strategy/ease-of-adoption/bridges.md) — ERC-1155 on Base, gasless relay claim, built-in timeout-reclaim; strong candidate for skipping a custom `TradFiBridgeEscrow`). Two decisions remain before committing: (1) can we accept the assurance-contract *failure* refund (token→USDC via `refundERC1155`) being handled off-contract by the operator rather than inside the escrow — if not, that's the one reason to build custom; (2) are we OK depending on Linkdrop's hosted relay, or do we want to self-host it (needs a spike to confirm a clean self-host path + the per-claim fee/gas model). The bridge-operator `buyERC1155` flow itself is already documented in bridges.md.
