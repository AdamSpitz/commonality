...
Seam 3 makes the *success* edge composable (where a payout goes). Its dual is the *failure* edge: where a refund goes when a contract fails. `refundERC1155(holder, …)` pushes the settlement token back to the receipt holder — fine when `holder` is an EOA, but a dead-end when the pledge was funded from a **delegatable note**. In that case the non-transferable receipts live *inside* `DelegatableNotes` (as ERC1155-typed notes, chain preserved), so an EOA-style refund cannot replenish the revocable note.
...
This is the load-bearing primitive for the "if P fails, money returns to my pool" / ordered-fallback-cascade capability discussed in [the product doc](/specs/product/composability.md#thoughts-on-the-above). Because receipts are non-transferable, the refund destination remains the original EOA or delegatable-note holder. The cascade *policy* itself ("then try B") stays off-chain (an executor + intent record, reusing the [recurring-pledges](/specs/product/recurring-pledges.md) intent/execution split); a trustless on-chain `FallbackRouter` is a later, optional escalation with a real custody cost.
