...
- **`bookmarked-causes`**: Published Commonality causes the user chose to keep. Value is last-write-wins JSON `{ version, causes, removed }`. `causes` are `{ owner, slug, updatedAt? }` identities, not statement CIDs. `removed` is a tombstone list so a stale device cannot union a deletion back onto the wallet. Version 1 documents (causes only) still parse. Unpublished drafts stay off this ref.
- **`personal-funding-board`**: The wallet user's public Commonality dashboard definition: included statement CIDs, geographic scope, and an optional preferred money source. Cached locally and explicitly synchronized; conflicting device and wallet copies require a user choice rather than silently overwriting either one.
