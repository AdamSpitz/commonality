...
3. ✅ **Beat finder.** Lives in `beat-agent` (`finder.ts`) as a thin finder/metrics-only adapter that reads beat-memory ingestion state and POSTs candidates to an attester. The reusable finder mechanics are in `finder-core`: JSON state persistence, JSON-state candidate-pass orchestration, submitted/not-promising/failed bookkeeping, JSON candidate submission, text-quality/keyword scoring, scored text-candidate selection, and scored text-to-evaluation-request construction. The remaining beat-agent code is intentionally beat-specific glue: adapting `BeatIngestedItem`s, preserving beat-specific config aliases, and wiring the attester endpoint. Further consolidation into `content-finder` would be optional product packaging, not outstanding substrate/consumer refactoring debt.
4. ✅ **Bridge/context consumers.** Consume `beat-memory`'s context API rather than duplicating social ingestion or synthesizing their own hidden beat memory. `bridge-creator` fetches generic `/context` endpoints from configured trusted context sources, passes a topic plus optional purpose filter, and normalizes native beat-memory observation responses into bridge context snapshots. Remaining work here is deployment wiring: point a real bridge creator at a real beat-memory instance.
...
- Finder mode is now finder/metrics-only: it reads beat-memory-produced ingestion state, scans ingested items, submits selected candidates to an attester endpoint, and records submitted/not-promising/failed outcomes with retry counts. It no longer ingests sources or mutates memory. Generic state persistence, candidate-pass bookkeeping, JSON submission, text-quality/keyword scoring, scored text-candidate selection, and scored text-to-evaluation-request candidate construction live in `finder-core`; beat-agent adapts beat-memory items and beat-specific config aliases into that generic path.
...
6. **Add LLM-authored source assessment and retrieval guidance.**
   - 🟡 Started. `beat-memory` now has a persisted `sourceAssessments` state section plus `generateSourceAssessments`, and worker ticks with the `source_management` purpose refresh advisory, plain-language per-author assessments from observed source authors and source-management manager notes. This is deliberately descriptive guidance, not a numeric reputation score, and it is covered by focused beat-memory tests.
   ...
   - Remaining work: replace/augment the heuristic assessment text with an LLM source-assessment generator over the beat definition, declared purposes, recent ingested items grouped by source/author, extracted observations, anomaly/contested-observation reports, coverage gaps, evaluation outcomes, and any manager notes.
   - Remaining work: retrieval should use conventional code only to attach relevant source assessments to candidate observations and to enforce rare hard operator overrides such as “quarantine this source” or “outside this beat.” The evaluator/reranker LLM should decide how much the assessment matters in context: e.g. If the source assessment says “this account is useful for finding out the right-wing perspective on a topic, but can't be counted on to address the opposing arguments,” take that into account when evaluating the actual content you're looking at.
