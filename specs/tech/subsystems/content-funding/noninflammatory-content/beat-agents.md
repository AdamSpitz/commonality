...
| **5. Fold the beat finder into `finder-core`/`content-finder`.** | 🟡 **Started** — beat finder state persistence, generic JSON-state candidate-pass orchestration, generic candidate-pass bookkeeping, generic JSON candidate submission, generic text-quality/keyword scoring, generic scored text-candidate selection, and generic scored text-to-evaluation-request candidate construction now use `finder-core`; beat-agent keeps only the beat-memory item adapter, beat-specific config aliases, and attester endpoint wiring. |
...
3. 🟡 **Beat finder.** Still lives mostly in `beat-agent` (`finder.ts`) as finder/metrics-only logic that reads beat-memory ingestion state and POSTs candidates to an attester. The first consumer-side consolidation steps are done: its JSON state persistence, generic JSON-state candidate-pass orchestration, generic submitted/not-promising/failed candidate pass, generic JSON candidate submission helper, reusable text-quality/keyword scoring primitive, generic scored text-candidate selector, and generic scored text-to-evaluation-request candidate construction now use `finder-core`. It is **not yet** folded into `finder-core` / `content-finder` as "a finder whose candidate feed is a beat-memory ingestion stream." Do the remaining moves only when they fall out naturally, or alongside the finder-quality work in the to-do list.
