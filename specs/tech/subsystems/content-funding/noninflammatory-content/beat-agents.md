...
5. **Improve memory quality beyond keyword retrieval.**
   ...
   - Alternative to consider before reaching for embeddings: lean on the hierarchical human-readable summaries the system already produces (`compacted_summary` observations, purpose summary snapshots, and potentially summaries-of-summaries on top of those). Two variants:
     - *Summary-only context:* drop per-query retrieval and just include the relevant purpose summary snapshot in the evaluator prompt. Simpler than today; viable if snapshots carry enough specific detail (named accounts, phrases, events) to ground a verdict.
     - *Summary + on-demand lookup tool:* include the summary up front and give the evaluator LLM a bounded `lookupObservations(topic)` tool to pull specifics when the summary isn't enough. `/evaluate-content` is already a multi-second operation dominated by IPFS upload and on-chain attestation, so a small number of extra LLM round-trips is within budget; cap iterations to keep per-request cost predictable.
   - **LLM-portability consideration.** Vector embeddings commit the system to a specific *embedding model* (separate from the generation LLM, but still a second model dependency). Stored vectors are not comparable across models, so switching embedders requires re-embedding the entire memory store. Approaches that keep stored memory as plain text remain portable across any LLM and avoid that lock-in. Portable alternatives worth considering:
     - *Hierarchical natural-language summaries* (the option above) — a "fairly detailed table of contents" that any LLM can read.
     - *Better lexical retrieval* — upgrade the current keyword scorer with BM25/stemming/phrase matching. Still pure code, still text-only.
     - *LLM-as-retriever* — at query time, have whichever generation LLM is in use pick relevant observation IDs from a candidate list or summary index. Stateless; works with any LLM that can read text and emit JSON.
     - *LLM-assigned tags at write time* — when an observation is extracted, also assign named tags (entities, sub-topics, faction labels) beyond the existing `purposes`. Retrieval becomes tag lookup. Tags are plain strings; the LLM is used as a stateless transformer, not a vector source.
   ...
   - These portable approaches compose. A plausible strong baseline before reaching for embeddings: hierarchical summaries + LLM-assigned tags + BM25-style lexical scoring over both summaries and raw observations. The unifying property is that stored memory stays plain text and any LLM is used only as a stateless transformer at write or query time — never as a model whose specific weights the stored data depends on.
