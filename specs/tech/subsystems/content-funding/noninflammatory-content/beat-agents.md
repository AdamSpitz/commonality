...
5. **Improve memory quality beyond keyword retrieval with text-native hybrid retrieval.**
   ...
   - Do not jump straight to vector embeddings. First build a strong plain-text baseline that preserves auditability and LLM portability:
     - Always include the latest relevant purpose summary snapshot in the evaluator prompt as the beat-level orientation layer.
     - Add write-time tags/entities/phrases to observations: accounts, people, organizations, factions, topics, slogans, recurring phrase meanings, relevant events, and purpose tags. These should be plain strings, not model-specific vectors.
     - Replace naive keyword overlap with BM25-style lexical scoring over observations, compacted summaries, purpose summaries, and tags. Blend the BM25 score with existing recency, purpose-match, source-diversity, and thin-context signals; add boosts for exact phrase, hashtag, account-handle, and tag/entity matches.
     - Optionally add a bounded LLM reranking step: retrieve a larger candidate set cheaply, then ask the current generation LLM to choose the genuinely relevant observation IDs and explain why. This keeps stored memory model-agnostic while using the LLM as a stateless query-time judge.
