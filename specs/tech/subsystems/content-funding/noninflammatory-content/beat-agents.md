...
| **5. Fold the beat finder into `finder-core`/`content-finder`.** | 🟡 **Started** — beat finder state persistence and generic candidate-pass bookkeeping now use `finder-core`; beat-specific scoring and attester request construction still live in `beat-agent/finder.ts`. |
