...
- Memory quality is improving: LLM-backed observation extraction is wired when `BEAT_AGENT_LLM_EXTRACTION_ENABLED=true`, and memory compaction now produces a semantic discourse narrative via LLM (same flag) instead of a keyword-frequency list. Retrieval is still keyword-based. Stale-observation tracking (preventing old summaries from silently dominating when discourse meaning changes) is not yet implemented. There is no production-grade retry/backoff for failed extraction work.
