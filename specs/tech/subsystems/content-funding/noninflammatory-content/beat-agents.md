...
| **5. Fold the beat finder into `finder-core`/`content-finder`.** | 🟡 **Started** — beat finder state persistence, generic candidate-pass bookkeeping, generic JSON candidate submission, and the generic text-quality/keyword scoring primitive now use `finder-core`; beat-agent keeps only the beat-memory item adapter, beat-specific config aliases, and attester request construction. |
