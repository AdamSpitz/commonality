...
| **5. Fold the beat finder into `finder-core`/`content-finder`.** | 🟡 **Started** — beat finder state persistence, generic candidate-pass bookkeeping, generic JSON candidate submission, generic text-quality/keyword scoring, and generic scored text-candidate selection now use `finder-core`; beat-agent keeps only the beat-memory item adapter, beat-specific config aliases, and attester request construction. |
