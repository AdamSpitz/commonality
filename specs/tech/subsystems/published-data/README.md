...
5. **Calldata/event encoding and cost benchmark.** Keep the calldata+event implementation. `DataPublished` emits the content bytes as well as carrying them in calldata, because it makes indexer extraction straightforward and keeps the contract storage-free. Benchmark tooling lives at `npm run benchmark:published-data --workspace=hardhat` and compares the production contract with a benchmark-only calldata-only variant for 1KB, 4KB, and 10KB payloads. Detailed local Hardhat and Base Sepolia reruns in Jul 2026 showed the expected ~8 gas/log-byte premium at 1KB, and 0 `receipt.gasUsed` delta at 4KB/10KB despite thousands of extra log-data bytes. The best explanation is post-Pectra calldata-floor transaction gas accounting: once calldata dominates charged transaction gas, the extra LOG execution gas does not raise `receipt.gasUsed`. Base Sepolia OP-stack fee fields did not reveal a hidden large marginal event fee. See [workflow/published-data-benchmark-2026-07-19.md](/workflow/published-data-benchmark-2026-07-19.md). Revisit only if Base/mainnet gas accounting changes, or if typical payloads grow far beyond the benchmarked 10KB range.
