...
The first durable backend is **IPFS**, and the seam makes it unusually cheap: because `dataId` is `sha256(content)`, the CIDv1 for the content is `publishedDataIdToCid(dataId)` — raw codec, derivable from the on-chain pointer alone, with no manifest, mapping table or second pointer to keep in sync. The **read** half of this is built but **not yet wired in**: `createIpfsContentResolver` (`ipfs-resolver.ts`) exists and is tested, while `createDefaultContentResolver` still returns calldata alone. That is deliberate — until a copier exists the mirror is always empty, so enabling it would buy nothing but a real network fetch and its timeout on every lookup. Note also that `ipfsConfig.gatewayUrl` is configured for unrelated IPFS uses, so its mere presence must not be read as "resolve PublishedData content through IPFS": that conflation silently adds network I/O to environments that previously had none.

When the copier lands, compose the two with `createFallbackContentResolver([calldata, ipfs])`. Calldata goes *first*, which is the opposite of the usual migration shape: calldata is the canonical byte layer here rather than a legacy one being migrated off, so it holds the content for any publication whose transaction the RPC still serves, whereas a mirror is populated asynchronously and may legitimately not have the bytes yet. Since `resolvePublishedContent` hash-verifies every answer, ordering is a performance choice and never a trust one.

The **write** half — a copier that watches `DataPublished` and pins the bytes — does not exist yet, so archive availability remains an open risk in practice: today nothing populates the mirror the resolver reads.
