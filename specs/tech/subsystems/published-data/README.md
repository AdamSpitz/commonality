...
This is deliberately behind an interface because **calldata is not expected to be the permanent answer.** It rests on RPC providers retaining transaction history indefinitely, which is an availability assumption we do not control. Moving to a durable content store means writing one more `ContentResolver` and changing which one `createDefaultContentResolver` builds — most likely wrapped in `createFallbackContentResolver` so pre-migration content keeps resolving. Applications can override it wholesale via `machinery.publishedContentResolver`. No caller above the seam changes, and nothing about the contract, the data model, or the legal posture changes with it.
