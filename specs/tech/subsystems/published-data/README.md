...
5. **Calldata-only encoding (revised Aug 2026 — this reverses the earlier decision).** `DataPublished(publisher, dataId)` carries **no content**. Both parameters are indexed, so the log body is empty and the bytes exist only in the publishing transaction's calldata. The earlier decision kept the content in the event body because it made indexer extraction trivial; that convenience was exactly the problem, since it meant every indexer following the logs stored a full copy of user content. See §"Pointers only: what the indexer sees" below.
...
**Storage vs. calldata vs. events.** Content bytes are never written to contract storage (~20k gas per 32 bytes — prohibitive, and unnecessary) and never emitted in a log. Only the small facts are stored: the publication bit and retraction bit per (publisher, cid). The bytes ride in calldata and nowhere else, recoverable from transaction history. No contract can ever read the content onchain — consumers get existence/retraction bits only, which is all they need.

## Pointers only: what the indexer sees

The contract announces *that* a publication happened, never *what was published*. This is the property the whole design is arranged around, and it is worth stating plainly:

**A general-purpose indexer following `PublishedData` logs stores `(publisher, dataId, txHash, block, logIndex)` and nothing else. It does not see, store, or serve user content.**

That closes a category of problem rather than mitigating it. The [shared-feed-topology](/specs/tech/indexer/shared-feed-topology.md) worry about bytes escaping through generic routes (`/api/events`, `/sql/*`, GraphQL) does not need route-by-route analysis, because there are no bytes in the event to escape. `refuse-serve` becomes unnecessary rather than unavailable. And it means one general-purpose indexer can be operated for everyone without the operator taking on a publisher's role — see [statement-hosting.md](/specs/product/legal/statement-hosting.md) on duties attaching to roles.

### Retrieval is a swappable seam

Readers get bytes through a `ContentResolver` (`@commonality/sdk/subsystems/published-data`): given a publication pointer, return the content. Today the only implementation recovers bytes from the publishing transaction's calldata, walking smart-account executions, batches and bundled UserOperations (validated across every publication route in [spikes/the-graph-nested-calldata](/spikes/the-graph-nested-calldata/README.md)).

This is deliberately behind an interface because **calldata is not expected to be the permanent answer.** It rests on RPC providers retaining transaction history indefinitely, which is an availability assumption we do not control. Note that going pointers-only also *removed* a de-facto backup: the old event body meant our Postgres held a full copy of every statement. Losing that is the point, but it makes archive availability the live risk on this path. Moving to a durable content store means writing one more `ContentResolver` and changing which one `createDefaultContentResolver` builds — most likely wrapped in `createFallbackContentResolver` so pre-migration content keeps resolving. Applications can override it wholesale via `machinery.publishedContentResolver`. No caller above the seam changes, and nothing about the contract, the data model, or the legal posture changes with it.

What makes that swap safe rather than merely convenient is content addressing: `dataId` **is** `sha256(content)`, so `resolvePublishedContent` verifies every backend's answer against the on-chain pointer before returning it. A resolver cannot substitute content, which is why an untrusted or third-party retrieval backend is as trustworthy as a first-party one.

The first durable backend is **IPFS**, and the seam makes it unusually cheap: because `dataId` is `sha256(content)`, the CIDv1 for the content is `publishedDataIdToCid(dataId)` — raw codec, derivable from the on-chain pointer alone, with no manifest, mapping table or second pointer to keep in sync. The read half is `createIpfsContentResolver` (`ipfs-resolver.ts`). `createDefaultContentResolver` composes it after calldata whenever a non-mock `ipfsConfig.gatewayUrl` is configured.

Calldata goes *first*, which is the opposite of the usual migration shape: calldata is the canonical byte layer here rather than a legacy one being migrated off, so it holds the content for any publication whose transaction the RPC still serves, whereas a mirror is populated asynchronously and may legitimately not have the bytes yet. This ordering also avoids new IPFS network traffic on the healthy calldata path. Since `resolvePublishedContent` hash-verifies every answer, ordering is a performance choice and never a trust one.

The write half is the standalone [`published-data-ipfs-mirror`](/published-data-ipfs-mirror/README.md) process. It watches finalized `DataPublished` logs directly from any RPC, recovers and verifies transaction calldata, and pins through any Kubo-compatible IPFS API. It mirrors everything, because selective redistribution must wait for and consume the shared policy-list evaluator rather than inventing another filter. The artifact is available to independent archival volunteers and operators that consciously choose that role; running it is **not** required for a vertical founder. Pointers-only keeps the indexer content-blind, but a mirror operator deliberately retrieves and retains the bytes and must not inherit the indexer's low-risk description by mistake.

The direct raw-CID identity has a hard 256 KiB single-block boundary. The current Base Sepolia recovery fixture measured 15 real publications totaling 423 bytes (28 bytes average), and the production document shapes are small JSON statements/metadata, so current evidence is orders of magnitude below it. The copier nevertheless rejects anything above 256 KiB rather than silently producing a chunked root CID. Large documents require a separately specified content-addressed manifest; EthStorage's 131,072-byte cap makes that shared follow-up relevant before either backend accepts large content.

#### Two rules for whoever writes the second backend

The read side of the seam is built. The *write* side — getting bytes into a durable store in the first place — does not exist yet, and these two constraints are what keep a second backend from dragging the publish flow and the legal posture along with it. They come out of [Smoothing A1](/spikes/ethstorage/README.md#smoothing-a1-2026-08-02) in the EthStorage spike, where a storage network on a different chain from our contracts made both failure modes concrete.

1. **The upload is asynchronous and never blocks publication.** Publish on-chain with calldata canonical; mirror to the durable store afterwards, retryable, where a slow or failed upload is a no-op rather than a stuck publish. The tempting alternative — confirm the upload is readable *before* emitting the publication — makes someone else's storage network a hard dependency of our publish latency and our success rate.

2. **The uploader need not be the author.** Because bytes are hash-verified against `dataId`, anyone can upload identical bytes and the result is indistinguishable; a missing mirror is healable by a third party. Do not design a write path that requires the publishing user to hold funds, credentials or a session on the storage network. That assumption is what turns "add a mirror" into "smooth over a flow the user cannot complete alone", which is the role we are trying not to take on — and it is why sponsoring or relaying the upload on the user's behalf is the wrong fix (option 5 in the spike).

### `unavailable` is not `not-published`

Because content retrieval is now a separate network operation from the publication lookup, it can fail on its own. It must fail *loudly*: a content fetch that cannot be satisfied raises `ContentUnavailableError`, which display layers map to `unavailable`. It must never degrade to `not-published`, because `not-published` and `retracted` remove a statement from aggregate counts. An RPC outage silently deleting people's signatures from headline numbers would be a far worse failure than showing "temporarily unavailable".
...
- **Indexer ingestion:** done, and deliberately minimal. The indexer caches `DataPublished`/`DataRetracted` logs and exposes active/retracted publication status plus publication *pointers*. It caches no content, because there is none in the logs to cache; clients resolve bytes themselves through the ContentResolver seam above.
