...
The one proposed exception is [combinator statements](combinator-statements.md): a closed `all` / `any` over other statement CIDs, so a promoted conjunction or disjunction is a deterministic graph node rather than a slogan the LLM attester has to interpret. That file is tentative.
