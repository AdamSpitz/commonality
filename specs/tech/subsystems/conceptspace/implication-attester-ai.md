...
- **Original statement → modified statement:** These should generally *not* be attested as implications. The modified statement may add concessions or framings that the original signer didn't intend. Instead, the connection between original and modified statements should go through the [nudge system](nudges.md) — "you might also believe this" — rather than "you logically already believe this." The bridge-creator publishes these as signed off-chain nudge messages (see the nudger architecture in [nudges.md](nudges.md)), not as on-chain attestation requests.
