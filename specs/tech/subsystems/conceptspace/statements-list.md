...
We've got a smart contract called MutableRefUpdater (see [Mutable Refs spec](../../mutable-refs/README.md)), which we're going to use to let each user store a mutable ref whose value is the IPFS CID of some JSON containing a list of statement CIDs that he wants to hold on to.
