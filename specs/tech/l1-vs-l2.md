...
| **Must be atomic together (unexamined)** | `CreatorAssuranceContract` ↔ its factory ↔ `ContentRegistry` ↔ `BeneficiaryRegistry` ↔ `BeneficiaryEscrow` | Same chain, same transaction. cross-chain-notes.md does **not** cover this cluster: its argument rests on an assurance pledge being a latency-tolerant threshold commitment, which does not obviously transfer to channel claiming and escrow. Treat as a genuine constraint until someone examines it. |
