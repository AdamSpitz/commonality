...
npm run build:domains      # builds all domains in one pass
npm run build:ipfs         # builds active domain in hash-routing mode for IPFS deployment
npm run build:ipfs:domains # builds all domains in IPFS mode
```
...
| No operated cause directory. `ExplorerPage` is not mounted; a third party may run `explorer-curator` | [ADR 0014](../decisions/0014-no-operated-generic-explorer.md); [Conceptspace Explorer](subsystems/conceptspace/explorer.md) |
