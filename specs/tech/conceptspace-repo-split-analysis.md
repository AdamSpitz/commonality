...
The page-level split below is implemented. Tally's manifest no longer mounts funding-portal routes. Conceptspace statement, settings, and home pages no longer import `fundingportals`, `content-funding`, or LazyGiving. `StatementSupportingContent` and `LinkedSocialAccountsSection` live in content-funding. Commonality's statement page renders supporting content and the submission form next to its own cause board. Commonality's settings page renders linked social accounts next to the shared trust sections. An ESLint rule rejects those funding imports from `ui/src/conceptspace/**` and `ui/src/domains/tally/**`. The production entry imports only the active domain (`@ui-active-domain`), so a Tally build does not load the other manifests. `ui/scripts/tallyFundingGraph.test.mjs` walks that graph for Tally and for the Conceptspace domain and rejects funding modules. `useMachinery` does not import content-funding; funding manifests install `lookupVerifiedTwitterAssociation` themselves. The retro-funding diagram is mounted by LazyGiving's docs route, not by the shared docs page.
...
Commonality already has its own [statement page](../../ui/src/commonality/pages/StatementPage.tsx) and [settings page](../../ui/src/commonality/pages/SettingsPage.tsx). The settings page imports `DirectTrustSettingsSection` and `NudgerSettingsSection` from Conceptspace and renders funding discovery controls itself. The statement page renders `StatementRenderer` for the shared body (operand links keep the page's own `mode` query) and adds its own sign control, funding board, `CauseBoard`, `CauseLeaderboard`, and cause funding. It does not mount Tally's believe/disbelieve controls; those stay on the Tally statement page.
...
Signer profiles no longer import content-funding. `SDKMachinery.verifiedSocialAssociation` is an optional lookup. Without it, address profiles still resolve ENS text records and beliefs and account assertions are unchanged. `lookupVerifiedTwitterAssociation` reads `BeneficiaryIdentity` (who proved a channel or DNS name), not the funding registry. Tally installs that lookup when `VITE_BENEFICIARY_IDENTITY_ADDRESS` is set. A match is not a payout address and not authorization to claim funds. The registry copies the proved owner in once as the initial payout, then owns rotation, project control, disavowal, and the claim waiting period. Do not move escrow.
...
The chain steps are [ConceptspaceAcceptanceJourneys.test.js](../../hardhat/test/ConceptspaceAcceptanceJourneys.test.js): one test deploys only Conceptspace contracts (publish, belief, withdraw, implication, trust, parent→modified nudge batch, mutable-ref resolve, retract) and a second creates a project, attests that same statement onto it, and buys a receipt. Subscribing and muting a mediator stay browser lists (`commonality:trustedNudgers`, `commonality:mutedNudgers`); the chain test records the publication and the trust score those lists follow, and the settings page tests cover the mute control. Bridge documents, the old `causestarter.bridge-cluster` kind, and parent→modified direction are [acceptance-journeys.test.ts](../../sdk/src/subsystems/displayable-documents/acceptance-journeys.test.ts), which does not import a payment module. Twitter verification and settlement tokens stay optional machinery: a Conceptspace deployment omits them instead of filling zeros.
