...
Roster document schema (kind, extras, slug checks, mediator and bridge-link parsing, inclusion-rule shape, and CID preview) lives in `@commonality/sdk/displayable-documents`. The Commonality page still publishes the roster and loads coherence badges from funding attestations. `causeRoster.ts` still mixes that funding read with the document. Extract the remaining publication actions, and the bridge cluster/triple formats, into the same non-UI package. Separate financial enrichment from document reading. Both the service's [cluster publisher](../../services/bridge-creator/src/clusterPublisher.ts) and browser authoring should consume the same format contract. Keep existing schema identifiers, CIDs and mutable-ref keys readable; renaming `commonality.*` document kinds is not needed to change source ownership.
