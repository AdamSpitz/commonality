...
Service kinds are split into `conceptspaceServiceKinds` and `fundingServiceKinds`. [conceptspaceServiceRegistry.ts](../../service-host/src/conceptspaceServiceRegistry.ts) registers implication, bridge, explorer, and beat-memory services and does not import content-funding services or [recurringPledgeScheduler.ts](../../service-host/src/recurringPledgeScheduler.ts). [fundingServiceRegistry.ts](../../service-host/src/fundingServiceRegistry.ts) registers content finder/attester, beat-agent, and the pledge scheduler. The default [serviceRegistry.ts](../../service-host/src/serviceRegistry.ts) still composes both for the combined host. A Conceptspace-only process should use the conceptspace factories, not the combined map. Ponder's default config is still the shared feed. `INDEXER_CONTRACTS=conceptspace` on that file drops assurance, delegation, and content-funding contracts but still loads their ABI modules. [ponder.conceptspace.config.ts](../../indexer/ponder.conceptspace.config.ts) is the entry that does not import them (`npm run dev:conceptspace` in the indexer). Alignment attestations stay. `GET /api/project-read-demand` and the unindexed-project log counter on `/api/events` run only when `INDEXER_CONTRACTS` is the shared feed. `ponder.conceptspace.config.ts` sets that variable to `conceptspace`, so the Conceptspace entry answers 404 for the funding route. Published-data and raw-event routes stay. Deployment manifests are not split yet.
