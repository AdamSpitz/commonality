...
Roster, bridge-cluster, and bridge-triple document formats live in `@commonality/sdk/displayable-documents`. The bridge creator and the seed helper build cluster documents through `buildClusterDocument` instead of a second extras shape. The Commonality page still publishes the roster and the cluster (wallet, mutable ref, runtime addresses) and loads coherence badges from funding attestations. `causeRoster.ts` still mixes that funding read with the document. Extract the remaining publication actions into the same non-UI package, and keep financial enrichment out of document reading. Keep existing schema identifiers, CIDs and mutable-ref keys readable; renaming `commonality.*` document kinds is not needed to change source ownership. New service-published clusters use the fuller markdown body from `renderClusterContent`, so their CIDs differ from the old one-line service body; already-published documents still parse.
...
Service kinds are split into `conceptspaceServiceKinds` and `fundingServiceKinds`. [conceptspaceServiceRegistry.ts](../../service-host/src/conceptspaceServiceRegistry.ts) registers implication, bridge, explorer, and beat-memory services and does not import content-funding services or [recurringPledgeScheduler.ts](../../service-host/src/recurringPledgeScheduler.ts). [fundingServiceRegistry.ts](../../service-host/src/fundingServiceRegistry.ts) registers content finder/attester, beat-agent, and the pledge scheduler. The default [serviceRegistry.ts](../../service-host/src/serviceRegistry.ts) still composes both for the combined host. A Conceptspace-only process should use the conceptspace factories, not the combined map. Ponder's default config is still the shared feed. `INDEXER_CONTRACTS=conceptspace` on that file drops assurance, delegation, and content-funding contracts but still loads their ABI modules. [ponder.conceptspace.config.ts](../../indexer/ponder.conceptspace.config.ts) is the entry that does not import them (`npm run dev:conceptspace` in the indexer). Alignment attestations stay. Deployment manifests and financial API routes are not split yet; `src/api` still ships with both configs.
