...
### 1. **IPFS Content Fetching & Hydration** (High Priority)

**Current Issue:** The UI manually fetches IPFS content in [ui/src/conceptspace/pages/StatementPage.tsx:79-95](../ui/src/conceptspace/pages/StatementPage.tsx):
...
- Each component reimplements IPFS fetching

**SDK Solution Needed:**

```typescript
// New SDK function in sdk/src/graphql-queries/conceptspace.ts
export async function getStatementWithContent(
  ...
  metrics?: { directBelievers: number, indirectSupporters: number }
}>
```
...
- Potential for caching (IPFS content is immutable)
- Could batch multiple content fetches
- Single source of truth for content resolution

### 2. **User's Indirect Support Filtering** (High Priority)

**Current Issue:** [ui/src/conceptspace/pages/UserProfilePage.tsx:92-104, 169-227](../ui/src/conceptspace/pages/UserProfilePage.tsx) has complex business logic to find which statements a user indirectly supports:

```typescript
// N+1 query pattern - fetches indirect supporters for EVERY belief separately
...
await Promise.all(
  userBeliefs.map(async (statement) => {
    ...
    const userSupport = supporters.filter((s) =>
      s.user.toLowerCase() === displayAddress.toLowerCase()
    )
...
**Problems:**
- **N+1 query pattern** - Major performance issue with many beliefs
- Complex filtering logic duplicated in UI
- Business logic for "what does indirect support mean" lives in UI
- Inefficient: fetches all supporters just to filter for one user
...
interface IndirectSupportInfo {
  statement: Statement,
  supportedVia: Array<{
    directlyBelievedStatement: Statement,
...
- Business logic centralized in SDK
- Could be optimized at the GraphQL/indexer level
- Reusable across different UI views
- Clear semantics for indirect support

**Implementation Notes:**
- The indexer could add a specialized query for this
- Or the SDK could optimize by fetching all implications once, then computing client-side
