...
Frozen as [ADR 0008](/specs/decisions/0008-operated-surfaces-are-lenses.md) (Aug 2026). The design goal above admits a third option that the original framing missed, and Commonality is built on it.
...
- **Editorial judgments become attestations, not site properties.** Anything the operator wants to assert (for Commonality: that a cause's roster matches its own summary and hides no riders) is published as a positive-only, narrowly-scoped, viewer-trust-configurable attestation bound to an exact artifact hash. That makes the judgment *portable* — a founder-run site can display it and cannot forge it — instead of making the operated site the only trustworthy place to look.
