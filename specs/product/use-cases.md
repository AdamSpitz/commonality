...
| # | Use case | Who wants what | Status | Gap / next step |
| --- | --- | --- | --- | --- |
| D1 | **Delegate to someone who knows the field** | "I care about this cause but can't evaluate projects — here's my budget, you decide." | Partial | The money half is solid: `/delegation/notes` shows funds, totals, and per-fund `DELEGATE` / `RECLAIM` actions; the deposit form takes an amount, a delegate, and an intended statement. **"Delegate to" is no longer a bare hex box** (2026-07-27): it uses the shared `AddressPicker`, so you can pick a saved delegate by name or enter an ENS name, and confirmed addresses are remembered. A delegate can now share a funding-specific page containing a versioned offering over immutable statement scopes and a derived track record. What remains missing is Commonality-authored *discovery* — no delegate directory or search — which is [deliberately post-MVP](#what-verification-turned-up). You still learn a delegate's link or address out-of-band the first time. |
| D2 | **Be a delegate** | Build a public track record of directing other people's money well. | Smooth | A delegate can publish, revise, share, and withdraw a versioned offering over immutable statement scopes from the public LazyGiving delegate page. The page displays that offering beside the address's recomputed controlled-fund track record; it does not treat the offer as an identity profile, endorsement, or authorization over donor funds. Cause pages deep-link an exact statement into the offering picker. |
...
| E2 | **Existing nonprofit gets on the rails** | An org adopts Commonality for ordinary donation tracking, before any crisis. | Rough | **Upgraded from Missing 2026-07-27 — there is now a door.** `/participate` is split into "As an individual" (the original six entry points) and "As an organization", and the latter leads to a new `/for-organizations` page that adapts [for-established-orgs.md](/docs/end-user/commonality/vision-and-strategy/ease-of-adoption/for-established-orgs.md): a three-step first session (take donations as notes → direct them to projects you vet → the ledger is the annual report), the four adoption levels as a dial the org controls, and a note for orgs with no track record yet. What remains rough is that this is still *narrative*, not tooling — an org that reads it and says yes has no org-shaped object to create, no way to declare itself the sole trusted attester for its own board without hand-editing trust config, and no org profile other people can find. |
