...
6. **Live anchor management.** Persistent anchor storage using the record shape from Decisions. Curate the seed anchor set from `hidden-majority` topics. Reflection job writes `status: proposed`; operator approves via CLI. `GET /anchors` endpoint. Partial bridge-creator-side scaffolding now exists for JSON anchor storage, curated seed anchors, `GET /anchors`, an anchor-reflection LLM seam that appends `status: proposed` anchor records from trusted CSM context plus previous publication summaries, an operator CLI (`npm run anchors --workspace=@commonality/bridge-creator -- ...`) that lists proposed anchors and approves/retires/deletes anchor records, and long-running service scheduling via `BRIDGE_CREATOR_ANCHOR_REFLECTION_INTERVAL_MS`. Remaining step-6 work: feed reflection real signing/ignore outcome summaries rather than only previous publication text.

7. **`.well-known/nudger.json` endpoint.** Per the generic nudger discovery spec — name, description, signer address, strategy prompt URL, anchors URL, trusted sources, status. Implemented in bridge-creator and reflected in `specs/tech/subsystems/nudger/README.md`.
