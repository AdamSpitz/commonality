...
6. **Live anchor management.** Persistent anchor storage using the record shape from Decisions. Curate the seed anchor set from `hidden-majority` topics. Reflection job writes `status: proposed`; operator approves via CLI. `GET /anchors` endpoint. Partial bridge-creator-side scaffolding now exists for JSON anchor storage, curated seed anchors, `GET /anchors`, and an operator CLI (`npm run anchors --workspace=@commonality/bridge-creator -- ...`) that lists proposed anchors and approves/retires/deletes anchor records. Remaining step-6 work: reflection job that writes proposed anchors from CSM context plus past publication/signing outcomes.
