...
## Publishing the UI build to IPFS — helps vs. hurts, by layer (Jul 2026)

Adam asked: doesn't publishing the UI code to IPFS open a can of worms — once many others pin it, the frontend becomes practically impossible to take down, which sounds like exactly the "you engineered yourself unable to comply" trap? Answer: no, because *publishing an artifact* and *operating a service* are different acts on opposite sides of the law, and the un-takedownability attaches only to the first (harmless) one. (Not legal advice; confirm with counsel before mainnet. This is consistent with the general doctrine below, not just this repo's framing.)

**Split "put the UI on IPFS" into two things:**

- **(A) Publishing the build artifact** (content-addressed, mirrorable, others re-pin it) = **code publication.**
- **(B) Making the *serving path* uncensorable even by us** (we literally can't take our own default front door down or filter it) = **engineered incapacity at the service layer.**

The law treats these oppositely — the *Van Loon*/Tornado Cash split (see "Does the open/decentralized architecture help?" above): publishing code is protected; operating services is where liability lives.

**(A) helps and is protected.** Code is protected expression — *Bernstein v. DOJ* (9th Cir. 1999) and *Junger v. Daley* (6th Cir. 2000) in the US; s. 2(b) of the Charter in Canada (weaker, balanceable under s. 1, but the baseline holds; and Canada's no-Section-230 gap is about carrying *third-party* content at the display layer, not about publishing *our own* code). The real-world proof: *Van Loon* (5th Cir. 2024) shielded the immutable Tornado Cash contracts, while Roman Storm was convicted (Aug 2025) for **operating an unlicensed money-transmitting business** — not for publishing code. Prosecutors invoked "built to be uncontrollable," but it attached to the *service he ran*, not the artifact's existence.

Un-takedownability of (A) creates **no compellable duty**, because **duty follows control**. We can be ordered to act only on what we control (our live front door, pins, denylist, DNS/ENS pointers); we cannot be ordered to do the impossible about others' pins. No control → no lever we're failing to pull. The practical un-takedownability that *looks* scary is what *removes* the duty rather than creating a breachable one.

**(B) is the trap — and publishing (A) does not put us in it.** "We can't take it down" at the service layer just means "we never wrote the filter," which a regulator reads as culpable, not decentralized (and for sanctions, near the worst fact pattern). But putting the build on IPFS does **not** strip our compliance ability: the mutable DNS/ENS pointer in front of the immutable artifact **is** the retained lever — push a new build, update the denylist, or point away. Mutable pointer + immutable artifact is therefore the *ideal* structure, not a grudging compromise: the frozen layer earns the protected-publication and factual-decentralization benefits; the mutable layers keep us a controllable operator of the service we actually run.

**The one real caveat — same lesson as images:** un-takedownability is harmless *only because the build is content-neutral and lawful*. Code is not absolutely protected (*Universal v. Corley*, 2001, enjoined DeCSS distribution under the DMCA; export-controlled crypto; CSAM/obscenity). Permanence is a one-way ratchet, so **never inscribe a compliance-sensitive value into the frozen artifact.** Concretely:

- **The denylist must be runtime-fetched, not baked into the immutable build.** If each build embeds its denylist, an old re-pinned build carries an *old, shorter* denylist and will display content we're now required to suppress — resurrecting the un-takedownability problem as an actual compliance failure. Have the live UI fetch the *current* denylist at load time from an endpoint we control, so even old pinned builds consult today's suppression list. The killable denylist endpoint is a feature (our compliance capability survives); someone forking to strip the fetch is *their* conduct. This sharpens item 1 above ("a config file the UI fetches") into a hard constraint: fetched at runtime, never inscribed.

**Rule of thumb:** publish the UI build to IPFS (and mirror via ENS/eth.limo) — protected code publication, and it makes decentralization factual. But never let "it's on IPFS" mean "we can't moderate our own default front door." Keep the client-side denylist runtime-fetched and current, keep the pins ours, keep the DNS/ENS pointer ours. The immutability we want is at the *contract* layer, where inability is a shield; at the *UI* layer, retained control is the shield.
