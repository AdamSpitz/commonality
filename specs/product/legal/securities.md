...
This is a material risk reduction, not a legal conclusion. Before mainnet, Canadian and US securities counsel must still review whether the contingent zero-interest claim is a note/security, the waterfall characterization, and delegation or future scout fees. The **separate content-funding design** that used to turn prospective receipts into *transferable* per-content-item tokens has since been brought in line (2026-08-05): `MaterializedContentTokens` now rejects holder-to-holder transfers, so every content-funding receipt is non-transferable recognition and money returns only through the capped reimbursement waterfall. Counsel should still confirm that conclusion rather than assume it. Stale non-legal specs describing LazyGiving resale must be corrected and must not shape shipped UI or marketing.
