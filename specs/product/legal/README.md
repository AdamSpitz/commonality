...
3. Turn display-denylist capability into an operation: production configuration, reporting address, owner/on-call responsibility, documented decisions, unpinning where applicable, and display **and aggregation** suppression tests. The intended end state, so that independent vertical operators can comply without each building a moderation department, is [composable policy lists](/specs/tech/subsystems/policy-lists/README.md) — subscribable blocklists the operator can override, evaluated the same way on every surface that displays or serves content. That spec's v1 is deliberately scoped to content enforcement from operator-local files and needs no chain; on-chain checkpointing is deferred until a real second keeper exists. **Neither gates this item** — the spec is explicit that it must not delay operating the existing denylist properly.
