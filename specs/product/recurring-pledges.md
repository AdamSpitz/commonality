...
Status: **MVP implemented on dev/testnet-bound code paths**: contracts, SDK folds/actions, indexer event registration, service-host scheduler, setup/cancel UI, funding-portal/cause-summary/leaderboard totals, and Render scheduler wiring exist. Remaining readiness is deployment activation/verification after the next contract redeploy fills in the `RecurringPledges` address and the scheduler key is funded/enabled. The note primitive is one-shot — `deposit` (create a note) + `delegate` (hand it down a chain) — so a recurring pledge is "do that on a monthly cadence." It's deliberately orthogonal to the rest of the system.
...
- **MVP:** the public standing-pledge intent record + the hands-off auto-pull execution. This needs one small *additive* contract entry point (an allowance-based create-and-delegate, so the auto-created note is rooted at the user for revocability) plus the offchain scheduler. It does **not** touch existing contract functions or their semantics. The contract helper is ERC-20-general, but the MVP UI uses only the already-selected settlement token (USDC in production) so displayed totals are unambiguous `$ / month`; native ETH is rejected at setup and deferred to the later escrow option. The first pledge executes immediately at setup, then recurs after each period. The scheduler is a small standalone *logical* service bundled into `service-host`. See locked decisions in the [tech note](../tech/subsystems/delegation/recurring-pledges.md#mvp-decisions-locked).
