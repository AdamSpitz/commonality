...
We'll be providing default AI services to bootstrap the system, but none of this requires people to trust our AIs; users can configure which AIs they trust in Settings. (See [trust model](docs/end-user/csm/vision-and-strategy/trust-model.md) for more detail.)
...
- **Movement-specific explorers** (e.g. CSM) — elicits what a user believes in order to find bridging opportunities. For the vision behind the CSM bridge-creator-plus-explorer combination, see [the CSM mediator doc](/docs/end-user/csm/vision-and-strategy/mediator.md).
