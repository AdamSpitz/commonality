...
The credible-threat pattern has a natural preparation phase that maps onto Commonality's [dial-not-switch](./dial-not-switch.md) adoption path:

**Step 1: Get on the rails.** Before any crisis, the institution (hospital, school, community program) starts using Commonality infrastructure for its existing charitable arm. Donations come in through [bridge operators](./bridges.md), get recorded onchain, donors get leaderboard credit. Functionally identical to what they're doing now — just on infrastructure that can't be taken away.
