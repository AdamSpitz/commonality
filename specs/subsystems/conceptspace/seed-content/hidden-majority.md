...
- **Commonality:** I'd be okay with it if abortion was available up to some reasonable point in pregnancy (such as the first trimester), with restrictions after that.
...
- **Moderate-from-B:** I want immigration controlled and legal and kept to reasonably small levels so that the melting pot can function, but I'm not anti-immigrant — people who come here legally and contribute should be welcomed. The current system is broken and needs fixing.
- **Commonality:** The immigration system is badly broken and needs to be fixed so that legal immigration works (at levels that are not zero, but are low enough to keep the melting pot functioning) and illegal immigration is reduced. People who are here legally should be treated well; people who are here illegally should be deported.
...
- Each cause-area statement → its top-level "I care about X" interest area (see [top-level.md](./top-level.md))
