...
Write the docs for humans (narrative, plain language). Then add a block to the docs/end-user/tldr-for-llms.md file: what this concept is, when a user would encounter it, what actions they might want help with. This lets an LLM orient itself quickly without degrading the human reading experience.
...
- Commonality’s in-app docs (`ui/src/commonality` `/docs/*`) bundle `docs/end-user/commonality/`, `shared/`, and `commonality/`. The spoken briefing is [commonality/index.md](/docs/end-user/commonality/index.md) (bulletin board + two twists; the rest is objections). The jobs catalog is [the-jobs.md](/docs/end-user/commonality/the-jobs.md). The agent/protocol index is [for-llms.md](/docs/end-user/commonality/for-llms.md) (SDK + IPFS + attester HTTP + MCP); generated TypeDoc/contract docs are `/api-docs/sdk/` and `/api-docs/contracts/`.
