...
- CauseStarter’s in-app docs (`ui/src/causestarter` `/docs/*`) bundle `docs/end-user/causestarter/`, `shared/`, and `commonality/`. The spoken briefing is [causestarter/index.md](/docs/end-user/causestarter/index.md) (bulletin board + two twists; the rest is objections). The jobs catalog is [the-jobs.md](/docs/end-user/causestarter/the-jobs.md). The agent/protocol index is [for-llms.md](/docs/end-user/causestarter/for-llms.md) (SDK + IPFS + attester HTTP + MCP); generated TypeDoc/contract docs are `/api-docs/sdk/` and `/api-docs/contracts/`.
