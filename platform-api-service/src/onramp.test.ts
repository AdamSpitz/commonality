...
type JwtOptions = Parameters<typeof generateJwt>[0];
type FakeJwt = typeof generateJwt & { calls: JwtOptions[] };

/** A generateJwt stand-in that records its args and returns a fixed token. */
function fakeJwt(token = 'stub-jwt'): FakeJwt {
  const fn = (async (options: JwtOptions) => {
...
type FakeFetch = typeof fetch & { calls: { url: unknown; init: RequestInit | undefined }[] };

/** A fetch stand-in that records the request and returns the given body/status. */
function fakeFetch(body: string, status = 200): FakeFetch {
...
describe('createCoinbaseOnrampSession', () => {
  ...
  it('rejects a non-positive preset fiat amount', async () => {
    await expectHttpError(
      createCoinbaseOnrampSession(
        CREDENTIALS,
        { address: VALID_ADDRESS, presetFiatAmount: '0' },
        fakeFetch('{}'),
        fakeJwt(),
      ),
      400,
  ...
  it('rejects a malformed fiat currency', async () => {
    await expectHttpError(
      createCoinbaseOnrampSession(
        CREDENTIALS,
        { address: VALID_ADDRESS, fiatCurrency: 'dollars' },
        fakeFetch('{}'),
        fakeJwt(),
      ),
      400,
  ...
  it('surfaces a 502 when no session token is present in the response', async () => {
    await expectHttpError(
      createCoinbaseOnrampSession(
        CREDENTIALS,
        { address: VALID_ADDRESS },
        fakeFetch(JSON.stringify({ other: 'x' })),
        fakeJwt(),
      ),
      502,
      'coinbase_onramp_error',
