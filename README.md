...
- Content-funding MVP is fully implemented across contracts, SDK, platform API service, attester infrastructure, and UI. All three platforms (Twitter, YouTube, Substack) now have complete verification flows, and conceptspace now reuses the Twitter channel-verification linkage so linked accounts can show up outside the content-funding UI too. Remaining work: run the live Playwright flow against a real local stack, and non-MVP work (embedded wallets/off-ramp).
