...
- Subjectiv trust-network computation now runs in a Web Worker, rehydrates cached trusted sets plus visited per-user direct-trust mappings from IndexedDB on startup, refreshes on direct-trust edits/manual refresh/window focus/a periodic timer, streams partial trusted-set updates into the UI while recomputation is still underway, and now has unit coverage across traversal progress, worker fallback/recovery, hook refresh triggers, and IndexedDB cache persistence. Wording cleanup and a higher-level integration/e2e coverage pass are still pending.
