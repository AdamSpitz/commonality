...
- Indexer redesign Phase 4 **in progress**: ABI corrections done in eventDecoder.ts (all contract events now match actual Solidity signatures). Next: migrate pubstarter, delegation, and mutable-refs queries to event cache + folds (following the pattern from conceptspace/fundingportals). 239 SDK tests passing.
