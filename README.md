...
- Indexer redesign Phase 4 **in progress**: SDK eventDecoder now imports ABIs from generated files (indexer/abis/), decoders fixed to use correct Solidity field names. Need to fix pubstarter folds tests, add event cache helpers, then migrate pubstarter/delegation/mutable-refs queries to event cache + folds. Build passes, tests need fixing.
