...
- Subjectiv trust-network computation now runs in a Web Worker, rehydrates cached trusted sets plus visited per-user direct-trust mappings from IndexedDB on startup, refreshes on direct-trust edits/manual refresh/window focus/a periodic timer, streams partial trusted-set updates into the UI while recomputation is still underway, and now has both focused unit coverage for traversal/cache infrastructure and higher-level UI integration coverage across the direct-trust settings flow plus funding-portal and leaderboard trust-aware filtering. A wording cleanup pass is still pending.
