...
- Content-funding MVP complete: contracts, SDK, deployment, fake-data, platform API service, and UI all implemented. Real `ChannelVerifier` contract deployed (replaces mock). Claim Flow UI implemented with tweet-based verification modal. Create Contract page now validates author via Platform API Service (wired).
