...
- Subjectiv trust-network computation now runs in a Web Worker, rehydrates cached trusted sets plus visited per-user direct-trust mappings from IndexedDB on startup, refreshes on direct-trust edits/manual refresh/window focus/a periodic timer, and now streams partial trusted-set updates into the UI while recomputation is still underway. Wording cleanup and a broader coverage review are still pending.
