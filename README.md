...
- Content-funding MVP now complete: contracts, SDK, deployment, fake-data, platform API service, UI, and all gaps fixed (share link added to Create Contract success state, API consistency between hooks). Remaining gaps are future work (embedded wallet, fiat off-ramp, embed previews, claim flow inline withdraw/take-control).
