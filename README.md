...
That's it. This starts a local Hardhat blockchain, deploys the smart contracts, starts IPFS, the Ponder indexer, and the content-funding platform API service, then publishes the SPA build to the local IPFS gateway and prints the resulting `http://localhost:8080/ipfs/<cid>/#/` URL. The latest CID and gateway URL are also written to `./data/ui-ipfs/`. After that, run `./data.sh --seed` to populate the chain with fake data (10 users, 3 rounds).
