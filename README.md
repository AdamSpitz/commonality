...
- Content-funding MVP is implemented across contracts, SDK, platform API service, attester infrastructure, and UI. Recent follow-up work fixed the claim/create-contract UI paths, added focused `CreateContractPage` coverage for the key validation and creator-vs-third-party flows, and now surfaces already-registered content immediately during create-contract validation.
- Remaining content-funding follow-up is now narrower: tighten multi-attester display in the UI and run the live Playwright content-funding flow against a real local stack. Non-MVP work remains embedded wallets/off-ramp.
