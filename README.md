...
- Content-funding: smart contracts implemented and reviewed, SDK fully wired (ABIs, event decoders, fold functions, cross-cutting query helpers, `fetchAndFoldContentFundingState`, `getAllChannelOverviews`), deployment flow and fake-data pipeline complete. UI in progress: Browse Creators, Channel Page, Create Contract, Creator Dashboard (`/content/dashboard`), Pubstarter project detail page integration, and Funding Portal integration are all live.
