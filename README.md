...
- Content-funding: smart contracts implemented and reviewed (access control, escrow routing, veto window, creator-controlled channels, third-party veto-bypass fix), platform API service for channel resolution and content validation with canonicalization helpers in SDK, indexer integration now captures all content-funding events (ContentRegistry, ChannelRegistry, ChannelEscrow, CreatorAssuranceContractFactory), and the SDK now provides both fold functions and cross-cutting query helpers for channel overviews, content-item status, per-channel contracts, and vetoable contracts. Remaining work is deployment wiring, fake-data seeding, and the UI (browse creators, channel pages, create contract, creator dashboard, integration with pubstarter project detail).
