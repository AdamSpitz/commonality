...
Commonality is a system for decentralized crowdfunding of public goods: people can fund projects aligned with shared values without needing a central organization to coordinate them. See [What is Commonality?](./specs/README.md#what-is-commonality) for the product overview.
  ...
  - [Verifier workspace](./verifier/README.md): current report on what's working. LLM “human tester” leaves are **manual**; if Adam asks you (the chat LLM) to run one, follow [Run an LLM-judgment check from this chat](./verifier/README.md#run-an-llm-judgment-check-from-this-chat) — do not spawn `pi` / OpenRouter.
  - [Testing inventory](./workflow/testing-inventory.md): map of conventional tests, verifier coverage, and remaining testing gaps
  ...
  - **Commonality** (SPA in [`ui/src/commonality/`](./ui/src/commonality/), `VITE_DOMAIN=commonality`; glue/backlog [`commonality-ui/`](./commonality-ui/), [`commonality-ui/TODO.md`](./commonality-ui/TODO.md)): founder-first core domain (eventual primary entry; Tally / LazyGiving / etc. de-emphasized as tools). Included in `./scripts/services.sh --start` and `./scripts/deploy-commonality.sh`. Local start currently publishes **only** the Commonality IPFS bundle (`LOCAL_UI_DOMAINS`; restore all UIs with `LOCAL_UI_DOMAINS=all` — see [local development](./workflow/local-development.md)). LLM helpers: [`cause-assist/`](./cause-assist/). Protocol MCP: [`mcp/`](./mcp/). **Drive the UI in a browser:** [Agent / browser automation](./commonality-ui/README.md#agent--browser-automation) (`dev-browser` CLI is always available; Playwright MCP is the Grok tool to install so a fresh chat sees browser tools). Agent job map: [`docs/end-user/commonality/for-llms.md`](./docs/end-user/commonality/for-llms.md).
  ...
  - [Marketing](./specs/product/marketing/README.md)
