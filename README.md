...
- Subjectiv trust-network computation now runs in a Web Worker, rehydrates cached trusted sets plus visited per-user direct-trust mappings from IndexedDB on startup, and refreshes on direct-trust edits, manual refresh, window focus, and a periodic timer. Partial-progress updates and wording cleanup are still pending.
