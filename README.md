...
- Content-funding MVP is implemented across contracts, SDK, platform API service, attester infrastructure, and UI. Recent follow-up work added YouTube video-description verification plus Substack RSS-feed verification support to the platform API service, fixed the claim/create-contract UI paths, added focused `CreateContractPage` coverage for the key validation and creator-vs-third-party flows, now surfaces already-registered content immediately during create-contract validation, renders per-content known attesters coherently, uploads real metadata to IPFS instead of placeholder CIDs, integrates content-funding-aware display in funding-portal aligned-projects list, and caches oEmbed preview results. Remaining follow-up is wiring the Substack claim flow in the UI, testing RSS verification against a real publication, and running the live Playwright flow against a real local stack. Non-MVP work remains embedded wallets/off-ramp.
