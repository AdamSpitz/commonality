...
- Indexer redesign: **Funding portals fully off GraphQL**. Last two GraphQL calls (`GetProjectDetailsDocument`, `GetParticipantSummariesDocument`) replaced with chain reads + event cache folds. Only remaining GraphQL usage is `indexer-sync.ts` (`_meta` query). 616 tests passing.
