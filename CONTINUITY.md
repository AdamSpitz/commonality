...
## 2026-05-29 — Testnet operational wallet/env wiring automated

- Reworked `scripts/generate-wallets.mjs` for first non-local deploy: it now writes generated private keys and finder trust secrets into `.env.secrets`, writes public operational addresses/payment recipients/UI trust defaults into gitignored `deployments/wallets.env`, refuses to overwrite existing generated values unless `--force` is passed, and prints the full secret block for password-manager storage.
- `scripts/setup-env.sh` now loads `deployments/<network>.env`, `deployments/wallets.env`, and `.env.secrets` together, emits role-specific service keys/payment addresses/finder trust keys/UI default trust env vars into generated env files, sets chain IDs by network, and requires `deployments/wallets.env` for non-local setup.
- Hardhat now auto-loads `.env`, `deployments/wallets.env`, and `.env.secrets`; non-local `deploy.js` deploys `ChannelVerifier` with `CHANNEL_VERIFIER_TRUSTED_SIGNER_ADDRESS` instead of silently using the deployer.
- Fixed service-host legacy env mode to mount default HTTP routes for HTTP-capable services, so Render/Docker `/health` works without manually setting route prefixes. Added coverage.
- Render/Docker/platform/indexer wiring updates: added missing indexer `NUDGE_PUBLICATIONS_CONTRACT_ADDRESS`; added platform `CHANNEL_VERIFIER_ADDRESS` and `CHAIN_ID`; switched Docker finder trust keys to the generated role-specific names.
