...
## 2026-06-22 — Successful-projects discovery slider

- Did the discovery-slider half of the TODO.md "Finish/polish successful projects" Tell item (its last remaining piece; item now marked complete).
- Added an explicit three-stop discovery slider ("My network" → "+1 hop" → "Anyone") to the Successful tab on the cause board, surfacing the existing `maxHops` trust-traversal knob per `specs/product/alignment-anti-abuse.md`. "My network" = direct trust (maxHops 1); "+1 hop" = network + one transitive hop (maxHops 2); "Anyone" drops the trust filter entirely (passes `undefined` for the trusted-attester set and weights → flat count-based success confidence fallback).
- New UI: `discoveryLevels.ts` (level→maxHops mapping + labels), `DiscoverySlider` (MUI Slider with discrete marks; disabled with a visible sign-in hint when the viewer isn't signed in — a hover tooltip would never fire on a disabled slider), and `SuccessfulProjectsTab` (wraps `SuccessfulProjectsList`, holds slider state, runs a dedicated `useTrustedSet(address, { maxHops })` call scoped to the tab so the Successful filter is independent of the Aligned tab's and only computes when the tab is mounted). `StatementFundingPortalPage` now renders `SuccessfulProjectsTab` for the successful branch; the page-level `useTrustedSet` no longer threads `trustWeights` to the successful list.
- `maxHops` plumbed end-to-end: `computeSubjectivTrustedSetResult` → `SubjectivTrustWorkerRequest`/worker → `computeSubjectivTrustedSet` client → `useTrustedSet` (new option + effect dep). The IndexedDB cache key now includes `maxHops` (cache version `v2`→`v3`, DB version 1→2 with a stale-record clear on `upgradeneeded`) so different hop depths don't thrash each other's cached trusted set.
- The Aligned tab is intentionally untouched; mirroring the control there is a separate future follow-up.
- Checks passed: new/updated unit tests across all layers (DiscoverySlider 5, SuccessfulProjectsTab 6, useTrustedSet maxHops forwarding 1, worker client 1, computation 1, cache maxHops isolation 1, StatementFundingPortalPage successful-tab switch 1); `npx vitest run src/fundingportals src/shared` (423, 37 files); `npm run typecheck --workspace=ui`; touched-file ESLint clean; `npm run build --workspace=ui`. Reported in `inbox.md` (Tell tier).

## 2026-06-22 — Per-module public API: conceptspace barrel

- Did the `conceptspace` slice of the TODO.md per-module public-APIs Tell item (next module after `fundingportals`; `shared` remains).
- Added root `ui/src/conceptspace/index.ts` barrel re-exporting only `StatementRenderer` — the single component that crosses the module boundary today (rendered by `fundingportals/pages/ExplorerPage.tsx`). Every other component (`CreateStatementForm`, `BeliefControls`, `SupportMetrics`, `StatementSuggestions`, settings sections, …) and the utils stay module-internal (promote-on-demand).
- Rerouted the one external eager consumer (`fundingportals/pages/ExplorerPage.tsx`) from `'../../conceptspace/components/StatementRenderer'` to `'../../conceptspace'`. `pages/*` stays as the deep subpath half: `domains/tally/manifest.tsx` still dynamically `import('../../conceptspace/pages/…')` directly, so code-split chunks are preserved. `domains/index.ts` → `./conceptspace/manifest.tsx` is the domains wrapper (no `../`), not the feature module.
