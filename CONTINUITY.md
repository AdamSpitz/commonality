...
- IPFS adds force CIDv1, raw leaves, and pinning, verify the returned CID, persist an atomic retry cursor, and reject content above the 256 KiB raw-block boundary. Existing Base Sepolia recovery measurements (15 publications, 423 bytes total) are far below the boundary; large content still needs a shared manifest design.
