...
- **One-way Conceptspace dependencies, with Tally free of funding.** Conceptspace is the horizontal layer; Civility and CSM stay verticals. Stay in this repo — do not split git history. Shared non-financial components may be used by both apps. Tally adds nothing financial. Commonality stays a complete app: do not move its flows out into Tally or into a package it stops calling. It adds funding on its own pages (no optional slots on Tally). Enforce the import direction. Plan: [conceptspace repo-split analysis](./specs/tech/conceptspace-repo-split-analysis.md).
