...
## Other things to do soon
  ...
  - It kinda feels like the right way to answer "what have you been building?" is to show a couple of websites: noninflammatory.eth.link and common-sense-majority.eth.link, both of which say "built on top of commonality.eth.link". Can we (a) get those ENS domains, and (b) make sure that our UI code is structured in such a way that it's easy for us to build several entry points like that?
  - Have we implemented some kind of mechanism for doing *recurring* funding? "I want to put $20/month towards cause C, delegated to delegate D."
  - Have we implemented some way for content writers, or fans of content writers, to submit their channel to the content finder services? (Have we even implemented any content finder services? Maybe not.) The idea is that there should be services out there that watch for particular kinds of content and then submit the good ones to the content attesters. Maybe those finder services might have an API where (maybe for a fee) you can submit a particular post, or a whole channel, so that the finder watches it, and decides for itself whether the post is good and/or whether the channel is worth watching in the future.
