...
- [Service bundling](specs/tech/service-bundling.md) — collapse the AI services into a couple of host processes. Sub-tasks:
    ...
    - Follow-up cleanup (the unification work left these rough edges — see [service-bundling.md](specs/tech/service-bundling.md#remaining-cleanup)):
      ...
      - [x] Finish pushing content-attester env parsing into its own package. `service-host/src/envConfig.ts` no longer inlines `readPromptTemplateFromEnv`; the `serviceConfigLoaders` entry for `content-attester` is `(e) => loadContentAttesterConfig(e)` with no extra fields, matching the other six kinds.
      - [x] Finish the worker→service rename in the JSON config schema. The field is now `services: [...]` in `ServiceHostConfig`, `parseServiceHostConfig`, error messages, `envConfig.ts`, `index.ts`, `supervisor.ts`, test files, and `README.md`. Old JSON configs with `workers:` will be rejected with a clear error — no silent dual-name compatibility shim.
      - [x] Don't default `routePrefix` for non-HTTP service kinds in the multi-instance env path. `buildInstanceWorker` in `service-host/src/envConfig.ts` only assigns a default `routePrefix` when the kind is in `httpServiceKinds`; finder instances declared via `SERVICE_HOST_INSTANCES` start cleanly with no HTTP listener.
      - [x] Close the asymmetry between the single-kind and multi-instance env paths. The multi-instance path (`buildInstanceWorker`) now sets `enabled: true` and a `restartDelayMs` derived from `*_RESTART_DELAY_MS` (per-instance prefix first, falling back to per-kind prefix), matching the single-kind path (`buildSingleKindWorker`).
