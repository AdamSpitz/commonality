...
## Other big things to do soon
  ...
  - Question: when we implemented the client-side folds, did we put a version number or something like that into the folding logic? I want to make sure we can update the code when we need to (and have all the clients out there do the right thing: maybe if the new folding logic is backward compatible then keep the existing state and just update the logic; maybe blow away the existing state and recompute it from scratch; etc.). Does [our indexer spec](specs/indexer/README.md) say anything about this? If not, write something up there, and implement it.
  - Question: to what extent can the indexer's thin event cache be lazy/idempotent? Like, currently IIUC we just start following the chain starting at some block number and then we record all the events we care about, and so when clients ask for events of a particular type we just give them what we've got because we assume we've got it all. But it'd be nice if the indexer was capable of being lazier than that - e.g. start with an empty DB, and then when asked for events of a particular type for a particular entity the indexer goes and asks the blockchain node and keeps track of which block number ranges it's got... I dunno, this doesn't really sound feasible. Just brainstorming.
