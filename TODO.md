...
- [ ] **(Tell)** Continue the [automation backlog extracted from the manual plan](./verifier/manual-validation-plan.md#11-automation-backlog-extracted-from-this-manual-plan). Triage/restructure is done: the backlog is now split into already adequately covered, small standalone, coherent chunk/harness project, and defer/manual-only work. Next, pick one coherent chunk to implement rather than nibbling random tiny slices. Candidate chunks: CSM movement-count propagation once the count feature exists. The operations/degradation canary expansion, AI-service fixture harness v1, LazyGiving remaining UI-state matrix, and per-domain explanatory-affordance gaps chunks are done at representative/compact coverage scope. Keep the old goal: LLM validation time should be spent on judgment rather than mechanical checks.
