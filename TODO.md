...
- [ ] **Securities redesign — contract follow-through after non-transferable receipts + reimbursement waterfall.** Core Hardhat contract work is now in place and the full Hardhat suite passes: `PremintingERC1155` receipts are non-transferable except mint/burn and owner-authorized primary-market/refund bridges; LazyGiving factories authorize assurance contracts as bridges; `MultiERC1155AssuranceContract` tracks `totalEarlyContributions`/`totalRetroReceived` and has capped `donateRetroactive` + O(1) pro-rata `withdrawReimbursement`; SDK and indexer ABIs have been regenerated. Remaining: decide whether to remove secondary-market deployment/indexing/SDK surfaces from LazyGiving factories/manifests or leave `ERC1155SecondaryMarket` only as unrelated legacy code, then run broader repo checks. Ask-tier: securities-sensitive, needs lawyer review before mainnet. See [retroactive-funding-redesign.md](specs/product/legal/retroactive-funding-redesign.md#resolved-decisions-jul-2026) and latest notes in [CONTINUITY.md](CONTINUITY.md).
