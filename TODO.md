...
- Thin CSM to a Commonality-shaped movement site. Right now its manifest pulls in `/content` (Civility), `/projects` (Pubstarter), `/portal` (Alignment), and `/organize` (nudgers), so it's functionally a super-app that duplicates four product surfaces. The spec frames it as a *movement* site that "uses" those products — to match that, drop the integrated product routes and replace them with prominent signpost links (the way Commonality links to Pubstarter/Alignment). Keep the thesis pages, popular CSM-related statements list, and nudger discovery.
