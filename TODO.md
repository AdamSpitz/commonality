...
- Think through the general notion of implication attestations and what's needed to make them reliable. In particular, the current question in my head is something like: "Do we need some notion of 'context'?" I don't mean some formalized JSON field; that's gonna be brittle (see [lean-on-ai.md](specs/product/lean-on-ai.md)). I mean that the implication attester prompt should include an instruction to make sure that the statement's context is reasonably clear. e.g. "I am pro-choice" is *probably* meant to be in the context of the abortion debate, but that's maybe not quite clear enough, so the attester should probably refuse to make implications using it; instead it should return false and give a reason like "context isn't explicit". The statement writer should instead submit a statement like "In the abortion debate, I am pro-choice." Does that make sense?
- Read over the general docs and specs for this project, then take a look at the prompt for the implication attester and make sure it's written as well as we can manage; this is a core piece of the system and people need to trust it; it needs to be right.
