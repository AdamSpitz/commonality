...
- [ ] **(Trust)** Add a `SuccessAttestation` handler to the indexer (`indexer/`) so those events are written to the event cache. The contract emits `SuccessAttestation` (in `AlignmentAttestations.sol`) and the SDK/UI are fully built, but the indexer has no handler for it, so the Successful tab on cause boards always shows empty. Mirror the existing `AlignmentAttestation` handler.
