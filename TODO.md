...
- [Service bundling](specs/tech/service-bundling.md) — collapse the AI services into a couple of host processes. Sub-tasks:
    ...
    - Follow-up cleanup (the unification work left these rough edges — see [service-bundling.md](specs/tech/service-bundling.md#remaining-cleanup)):
      - [x] Normalize AI-service package `npm test` scripts so they actually execute the repo’s TypeScript test files without ad hoc `mocha --import=tsx ...` commands during verification.
      ...
      - [x] Let the env-var path run multiple instances of the same kind in one host. Today names like `'implication-attester'` are hardcoded strings in `envConfig.ts`, so running e.g. `content-attester-neutral` + `content-attester-left-eval-right` in one host requires the JSON-config path. Drive instance discovery off env (e.g. `SERVICE_HOST_INSTANCES=content-attester-neutral,content-attester-left-eval-right` with per-instance prefixes `CONTENT_ATTESTER_NEUTRAL_*`), so env mode is a full substitute for the JSON config mode. Acceptance: the two previously-deleted `content-attester-*` compose entries could be resurrected as one `service-host` container with two instances, using env vars only.
      - [ ] Finish pushing content-attester env parsing into its own package. `service-host/src/envConfig.ts:109` still inlines `readPromptTemplateFromEnv` and splices `promptTemplate` onto the content-attester config, instead of letting `@commonality/content-attester`'s `loadConfigFromEnv` read `CONTENT_ATTESTER_PROMPT_TEMPLATE` / `CONTENT_ATTESTER_PROMPT_TEMPLATE_FILE` itself. This is the last remaining "service-host knows the shape of one specific service's config" case and it's exactly the accidental complexity the spec warned against. Acceptance: the `serviceConfigLoaders` entry for `content-attester` is `(e) => loadContentAttesterConfig(e)` with no extra fields, matching the other six kinds; adding/removing a content-attester field requires zero edits in `service-host/`.
      - [ ] Finish the worker→service rename in the JSON config schema. The types `WorkerHostConfig`/`WorkerKind`/etc. got renamed, but the JSON config field is still `workers: [...]` (see `ServiceHostConfig.workers` in `service-host/src/config.ts:27` and the `workers[${index}]` error messages / `parseHostedServiceConfig` / `parseServiceHostConfig`). Rename the field to `services` in the type, parser, error messages, `envConfig.ts`, `index.ts`, and any JSON config files (check `docker-compose.yml`, `render.yaml`, and anything in `service-host/test/`). Old JSON configs with `workers:` can either be migrated or rejected with a clear error — don't leave a silent dual-name compatibility shim.
      - [ ] Don't default `routePrefix` for non-HTTP service kinds in the multi-instance env path. `service-host/src/envConfig.ts:132` unconditionally sets `routePrefix: routePrefix ?? '/' + instanceName` for every instance, but finders have no `createApp` factory, so a finder instance declared via `SERVICE_HOST_INSTANCES` makes `createServiceHostApp` throw at boot ("does not expose an HTTP app"). Only assign a default `routePrefix` when `serviceAppFactories[kind]` exists; leave it `undefined` otherwise. Acceptance: `SERVICE_HOST_INSTANCES=implication-finder-a,implication-finder-b` starts cleanly with no HTTP listener.
      - [ ] Close the asymmetry between the single-kind and multi-instance env paths. The single-kind path (`buildSingleKindWorker` in `service-host/src/envConfig.ts`) sets `enabled: true` and a `restartDelayMs` derived from `*_RESTART_DELAY_MS`; the multi-instance path (`buildInstanceWorker`) sets neither. Make multi-instance honor the same `*_RESTART_DELAY_MS` convention (per-instance prefix first, falling back to per-kind prefix) so behavior is consistent no matter which env-var path the operator uses.
