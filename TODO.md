...
- Improve UI test coverage across all UI surfaces. Current state: the UI already has substantial Vitest coverage for many Commonality pages/components and Playwright coverage for the main Commonality flows (wallet connection, statement creation, belief expression, profile, pubstarter, delegation, Subjectiv/funding-portal, and a content-funding creation/browse flow), plus light domain route tests for the four branded landing pages. It is not yet reasonably complete across every user-facing surface. To get there:
  ...
  - Second, here's a to-do list to clean up weak spots in the recent UI-test additions:
    - Fix `HighProfileSigners`: a signer with `ensName` but no `twitterHandle` currently renders `@undefined`. The test in `ui/src/conceptspace/components/HighProfileSigners.test.tsx` should not expect that bug; fix the component fallback behavior and update the test to expect the intended label.
    - Tighten `DocsPage.test.tsx`: remove conditional assertions like "if external links exist" / "if blockquotes exist". Either choose fixture docs that definitely contain the element being tested, or add small test-only fixtures/mocks so the assertion must actually run.
    - Revisit `ui/test-plan.md`: it currently overstates coverage ("All major pages and components..."). Make it a coverage inventory with known gaps, and keep it aligned with this TODO list.
    - Audit recently added tests for overly shallow assertions such as "some heading exists", "some alert exists", or "some disabled button exists". Prefer asserting the specific accessible name, visible copy, route, submitted payload, state transition, or error message that matters.
    - Check for tests that bless implementation quirks instead of product behavior. If a test reveals weird UI output, surprising copy, or a broken fallback, fix the component and assert the desired behavior.
  - Third, here's a list of remaining work to do:
