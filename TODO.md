...
- Fix the `platform-api-service` Coinbase onramp test suite (8 failing tests in `onramp.test.js`). They fail with `UserInputValidationError: Invalid key format - must be either PEM EC key or base64 Ed25519 key` from the CDP SDK's `generateJwt` — the tests feed a dummy CDP key that the SDK now rejects. Give the tests a valid throwaway EC/Ed25519 key or mock `generateJwt`/`createCoinbaseOnrampSession`. This is unrelated to the retroactive-funding redesign, but it's currently the only *functional* red: it stops `verifier:fast` at `ai-fixtures.deterministic` and reddens `facet.functionality`.

- Refresh the security facet after the retroactive-funding contract rewrite (the RF redesign changed assurance-contract behavior: `forgoReimbursement`, reimbursement-fold refund clamp, atomic normal donation, removed secondary-market/token-burn). `facet.security` leaves are stale. Run `verifier-run automated.hardhat-contracts` and `verifier-run review.security.slither` (plus `review.security.contracts` if refreshing the judgment), then `npm run verifier:security`. While there, clear the unused-var lint hits the redesign left in `AssuranceContracts.test.js` and `ContentFunding.test.js`.
