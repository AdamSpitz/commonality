...
- [ ] **(Tell)** Triage and restructure the [automation backlog extracted from the manual plan](./verifier/manual-validation-plan.md#11-automation-backlog-extracted-from-this-manual-plan) before doing more drive-by automation. The current backlog has lots of partially covered items, so first convert it into a clearer checklist of concrete remaining test tasks, tagged as: already adequately covered, small standalone, coherent chunk/harness project, or defer/manual-only. Then pick one coherent chunk to implement next rather than nibbling random tiny slices. Candidate chunks: operations/degradation canary expansion; AI-service fixture harness v1; CSM publication/count propagation; LazyGiving remaining UI-state matrix; per-domain explanatory-affordance gaps. Keep the old goal: LLM validation time should be spent on judgment rather than mechanical checks.
