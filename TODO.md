...
- [ ] **(Tell)** Continue the [automation backlog extracted from the manual plan](./verifier/manual-validation-plan.md#11-automation-backlog-extracted-from-this-manual-plan). Triage/restructure is done: the backlog is now split into already adequately covered, small standalone, coherent chunk/harness project, and defer/manual-only work. Next, pick one coherent chunk to implement rather than nibbling random tiny slices. Candidate chunks: AI-service fixture harness v1; CSM movement-count propagation once the count feature exists; LazyGiving remaining UI-state matrix; per-domain explanatory-affordance gaps. The operations/degradation canary expansion chunk is done at representative-canary scope. Keep the old goal: LLM validation time should be spent on judgment rather than mechanical checks.
