...
- Address the remaining fresh workflow-review findings from 2026-07-24: Content Funding's `/content/new` URL flow can still route human handles to an unparseable/unindexed channel dead end; LazyGiving's disconnected contribution controls and scout-vs-donation copy remain ambiguous. The shared Aligning/CSM cause-board no-project state now offers actionable create-project and browse-projects recovery CTAs. `product.workflows` should be refreshed after the remaining fixes land. (The standing de-crypto/landing-copy issues remain separately tracked in [inbox.md](/inbox.md).)
