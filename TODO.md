...
- [ ] **(Tell)** Remove the global-`verifier-run-command`-on-PATH fragility. The `automated.*` and `security.*` verifier checks set `"command": ["verifier-run-command"]`, which resolves only via the globally npm-linked binary in `~/.npm-global/bin`. When `root`/`facet.*` eagerly spawn a missing/stale child in a reduced-PATH environment (scheduler, rollup refresh), that binary isn't found and the check errors with `spawn verifier-run-command ENOENT` — which surfaced as a security-facet blind spot on `security.contract-invariants` (the underlying tests actually pass; it was purely the spawn env). There's already a workspace-local equivalent, `verifier/checks/automated/run-command-check.mjs`, that does the same thing. Repoint the ~25 affected `*.def.json` files at `["node", "checks/automated/run-command-check.mjs"]` so they don't depend on a global binary being on PATH.
