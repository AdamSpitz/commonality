...
- [ ] **Securities redesign — non-transferable receipts + reimbursement waterfall.** Implement the resolved Jul-2026 securities posture: make LazyGiving receipt tokens non-transferable (burn-to-zero only); add a pull-based, O(1), pro-rata-simultaneous reimbursement waterfall (`totalEarlyContributions`/`totalRetroReceived`, no seniority) with retroactive donations **capped** at `totalEarlyContributions` (excess rejected, no overflow-to-project); no sweeteners. Retire `ERC1155SecondaryMarket` and `DelegatableNotes.purchaseFromSecondaryMarket` from the retroactive-funding flow (no UI, indexing, or linking). Ask-tier: securities-sensitive, needs the lawyer review before mainnet. See [retroactive-funding-redesign.md](specs/product/legal/retroactive-funding-redesign.md#resolved-decisions-jul-2026).
