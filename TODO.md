...
- Finish the operational cutover of the Alignment site to `aligning.works`. The visible site name is Aligning, Base Sepolia deployment config and cross-domain routing now use `https://aligning.works`, and platform API CORS includes that origin. Remaining: point the domain at the Alignment/Aligning IPNS name, provision a valid public TLS certificate (the current endpoint only responds when certificate verification is disabled), deploy the refreshed config/UI, and smoke-test cross-domain links. Internal `alignment` identifiers can remain.
...
- Address the fresh workflow-review findings from 2026-07-24: Aligning and CSM cause boards need actionable no-project recovery CTAs; Content Funding's `/content/new` URL flow can still route human handles to an unparseable/unindexed channel dead end; LazyGiving's disconnected contribution controls and scout-vs-donation copy remain ambiguous. `product.workflows` is fresh and fails on three of four workflow reviews. (The standing de-crypto/landing-copy issues remain separately tracked in [inbox.md](/inbox.md).)

- Resolve the fresh docs-coherence findings from 2026-07-24: use the visible `Aligning` name consistently in end-user docs, reconcile whether Published Data is a core eighth subsystem or a cross-cutting technical subsystem, fix the broken security-review link in `inbox.md`, and consider adding a short product lede to the top-level README. Also refresh the stale `review.docs-broken-refs` leaf afterward.
