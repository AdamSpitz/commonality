...
- [ ] **(Tell)** Continue PublishedData integration. The contract and SDK reader/id helpers are now implemented, CID representation is pinned (`bytes32 dataId = sha256(content)` → CIDv1/raw/sha2-256), benchmark tooling exists (`npm run benchmark:published-data --workspace=hardhat` with a calldata-only comparison contract), and the spec documents remaining indexer/UI work. Remaining: run/record the calldata+event vs calldata-only benchmark under the intended live Base Sepolia/Base fee conditions if needed, finish remaining SDK/UI reader integration beyond the new raw-event ingestion, `/api/published-data/:publisher/:dataId` default-reader endpoint, and SDK API cache, then wire the conceptspace composer/display/aggregation policy. Keep `supportStatement` ungated and library defaults honoring only publisher self-retraction. See [specs/tech/subsystems/published-data/README.md](specs/tech/subsystems/published-data/README.md).
