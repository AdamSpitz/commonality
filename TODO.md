...
- [ ] Triage the `automated.dependency-audit` red (4+ unallowlisted high/critical production vulns as of 2026-07-05). Notable advisories: `@hono/node-server` (auth bypass in static middleware), `drizzle-orm` & `kysely` (SQL injection), `lodash` (prototype pollution / `_.template` code injection), `vite`, `ws`, and the `viem`/`ponder`/`@privy-io/react-auth` chains. Upgrade what's safely bumpable, and allowlist (with justification) anything that's dev-only or not reachable in production.

- [ ] Reconcile the `security.package-lock-dependencies` red: 153 lockfile dependencies added since the last security baseline review (as of 2026-07-05, likely from recent lockfile/Docker-install churn). Review the new packages and refresh the security baseline so the check goes green.
