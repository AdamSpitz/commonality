...
- [ ] **(Tell)** Investigate and fix local deep-stack verifier failures: `artifact.ipfs-domain-smoke`, `stack.user-journeys`, and `stack.restart-consistency` exit 7. Since `stack.fresh-seeded` and `operations.local-stack-health` pass, start from route/journey/restart-specific evidence rather than assuming the whole stack is broken. (Progress 2026-07-08: `stack.restart-consistency` now passes locally and reports a clear precondition failure instead of raw curl exit 7 when the indexer API is down. `artifact.ipfs-domain-smoke` now forces Playwright to start a fresh artifact server under verifier runs; the stale-server failure reproduced and then passed after killing the old local process. The same fresh-server setting was applied to `stack.user-journeys`, but that expensive check still needs rerun/verification.)
