...
- Improve UI test coverage across all UI surfaces. Current state: the UI already has substantial Vitest coverage for many Commonality pages/components and Playwright coverage for the main Commonality flows (wallet connection, statement creation, belief expression, profile, pubstarter, delegation, Subjectiv/funding-portal, and a content-funding creation/browse flow), plus light domain route tests for the four branded landing pages. It is not yet reasonably complete across every user-facing surface. To get there:
  ...
  - Add mobile/responsive AppShell tests: drawer open/close, primary and secondary navigation in the drawer, selected-state behavior, "More" menu behavior on desktop, and no layout-breaking navigation labels across the four domain shells.
  - Add direct tests for shared shell/infrastructure surfaces that are currently only incidentally covered: `AppShell`, `AddressDisplay`, `PrivyWalletButtonImpl` / embedded-wallet mode, `DocsPage`, and top-level route composition around `App`.
  ...
  - Expand content-funding UI coverage: `BrowseCreatorsPage`, `ChannelPage`, `CreatorDashboardPage`, `ClaimFlowModal`, `ContentFundingProjectSection`, claim-link sharing, verified vs unclaimed channel states, withdraw/vetoable-contract states, and platform-specific Twitter/YouTube/Substack branches.
  ...
  - Expand pubstarter UI coverage for the project-detail sub-surfaces that are not directly tested: burn, refund, withdraw, secondary market, leaderboard, trade history, project header, and disconnected-wallet prompts. Keep one or two Playwright flows for actual buy/sell/refund/withdraw behavior.
  ...
  - Expand delegation/mutable-ref coverage for `AvailableDelegatableFunding`, delegated-funds empty/error/loading states, note-detail edge cases, saved refs creation/update/delete behavior, and route-level smoke tests.
  - Expand conceptspace coverage for `HighProfileSigners`, Twitter handle hint rendering, Explorer personalization/error/loading states, Settings nudger metadata discovery failures, muted topic/nudger edge cases, and statement content submission states.
