...
- [ ] **Securities redesign — contract follow-through after non-transferable receipts + reimbursement waterfall.** A first pass is in progress in the Hardhat contracts: `PremintingERC1155` receipts are non-transferable except mint/burn and owner-authorized primary-market/refund bridges; LazyGiving factories authorize assurance contracts as bridges; `MultiERC1155AssuranceContract` now tracks `totalEarlyContributions`/`totalRetroReceived` and has capped `donateRetroactive` + O(1) pro-rata `withdrawReimbursement`. Focused tests for non-transferability and reimbursement pass. Remaining: retire/update old `ERC1155SecondaryMarket` and `DelegatableNotes.purchaseFromSecondaryMarket` tests/code paths (full Hardhat suite currently fails where old tests expect transferability/secondary-market fills), decide whether to remove secondary-market deployment from LazyGiving factories/manifests/ABIs or leave only as unrelated legacy code, regenerate/sync ABIs after contract shape settles, and then run full Hardhat + repo checks. Ask-tier: securities-sensitive, needs lawyer review before mainnet. See [retroactive-funding-redesign.md](specs/product/legal/retroactive-funding-redesign.md#resolved-decisions-jul-2026) and latest notes in [CONTINUITY.md](CONTINUITY.md).

- [ ] **Securities redesign — UI + docs.** Replace buy-and-burn with the "close the loop" reimbursement donation flow; surface outstanding-unreimbursed amounts; display raw per-scout records ("scouted N, reimbursed M, outstanding K") with **no** aggregate expected-return stats; add a UI-only "suggested delegates" panel ranked by track record/work product (never a protocol mechanic, never framed as donor returns). Rewrite `docs/end-user/.../retroactive-funding.md` around reimbursement + reputation and scrub the profit narrative (grep "return/reward/invest/profit/price difference"). See [retroactive-funding-redesign.md](specs/product/legal/retroactive-funding-redesign.md#resolved-decisions-jul-2026).

- [ ] **Channel-claiming — cheap legal wins (no trustless verifier).** Per the Jul-2026 near-term posture: (1) timelock + multisig the `ChannelVerifier` owner / `setTrustedVerifier` levers (owner-key triage); (2) anchor a hash of the public proof (tweet/RSS URL) on-chain so a dishonest verifier is independently detectable; (3) sanctions screening at platform-identity resolution (claim/display time), before any wallet exists; (4) "created by a fan; @creator is not affiliated" framing on claim/display pages. Do **not** build a trustless verifier now (demand-gated). See [channel-claiming.md](specs/tech/subsystems/content-funding/channel-claiming.md#near-term-posture-cheap-wins-now-trustless-verifier-demand-gated-jul-2026).
