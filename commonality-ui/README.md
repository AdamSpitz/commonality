...
lives in [`ui/src/commonality/`](../ui/src/commonality/) and is built as
...
- Local deploy loads `PUBLISHED_DATA_CONTRACT_ADDRESS` /
  ...
  (`./scripts/services.sh --start` or `./scripts/deploy-commonality.sh`).
...
2. Seed `ui/.env` (and optionally overlay `commonality-ui/.env`) from the running Docker SPA config (contract addresses + tool domain URLs). Vite HMR on :5174 reads `ui/.env`. IPFS/local publish merges `ui/.env` then `commonality-ui/.env`, then root contract-address mappings:
   ...
   (Needs Docker Commonality on `:8090` once so `config.json` is available. Re-seed after a chain re-deploy.
   Alternatively copy `VITE_*` keys from `deployments/localhost.env` / `ui/.env`, or re-run
   `./scripts/deploy-contracts.sh localhost` which mirrors addresses into `commonality-ui/.env`.)
...
Commonality uses [ConnectKit](https://docs.family.co/connectkit) + wagmi.
...
That publishes the Commonality SPA to local IPFS (gateway
...
Commonality tool cards open the other domain SPAs at
...
- **Landing pitch is jobs, not a movement lifecycle.** `/docs`
  (`docs/end-user/commonality/index.md`) is the spoken briefing: bulletin board
  ...
  (`docs/end-user/commonality/the-jobs.md`) are the “do the part you’d do anyway”
  catalog. Do not restore Start → Grow → Deliver or “build a Movement.”
- **In-app docs** (`/docs/*`) bundle `docs/end-user/commonality/`, `shared/`,
...
- **Commonality is a lens, not a directory** ([ADR 0008](../specs/decisions/0008-operated-surfaces-are-lenses.md)).
...
- **Cause store** (`ui/src/commonality/lib/causeStore.ts`) keeps planks in `localStorage` so
