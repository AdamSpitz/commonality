# Commonality API map (for LLMs)

Task-oriented index for agents. Signatures live in generated TypeDoc; this page only maps **jobs → imports / HTTP**. Do not treat Commonality screens as an API — they will change.

Human briefing: [Commonality](./index.md). Concept orientation: [tldr-for-llms.md](../tldr-for-llms.md).
...
Regenerate with `npm run build:docs` at the repo root (`typedoc` on `sdk/`, solc docs on `hardhat/`). Local Vite (`commonality:dev` on :5174) serves the same trees at those paths when the generated folders exist.

There is **no Commonality REST API**. The UI is a lens over the SDK plus a few helper HTTP services.
...
| **Coherence badge worker** | worker package | operator-only chain write | Commonality site operator mints badges; cause-assist HTTP must not hold that key. |
...
Writes (`upload_ipfs`, `believe_statement`) stay off unless `COMMONALITY_MCP_WRITES=1`. `believe_statement` also needs `MCP_PRIVATE_KEY` (or `ETHEREUM_PRIVATE_KEY`) — a **user** signer, not a remapped service role key. On local Hardhat, use a funded Anvil account (prefer `#19`); repository operators can find setup details in `mcp/README.md`. Implication evaluate may still return HTTP 402 (x402).

## Rules of thumb

- Prefer SDK queries over scraping Commonality.
