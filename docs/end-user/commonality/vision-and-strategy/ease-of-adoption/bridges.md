...
> **Decision: adopt Linkdrop SDK V3 for claim links; skip a custom `TradFiBridgeEscrow`.** Linkdrop supports ERC-1155 claim links on Base, holds tokens in a non-custodial escrow between deposit and claim, sponsors claim gas via a relay so the recipient can claim into a brand-new wallet, and has built-in timeout-reclaim for links the donor never claims. Assurance-contract *failure* refunds are handled by the operator off-contract (it reclaims expired tokens and refunds the donor's card through its own already-regulated process); a custom escrow wouldn't improve on this. **One decision is still open:** whether Commonality uses Linkdrop's hosted relay or self-hosts it — the claim flow is Commonality's own product surface, so that operational burden is ours either way. The full evaluation and the open relay decision live in [specs/tech/bridges.md](/specs/tech/bridges.md).
...
2. **Claim-link or escrow mechanism** — Needed when an operator has a donor who does not yet have a wallet. **Linkdrop SDK V3 is the chosen approach** (ERC-1155 on Base, gasless relay claim, built-in timeout-reclaim), letting us skip custom escrow code. Assurance-failure refunds are handled by the operator off-contract. The one open decision — hosted vs. self-hosted relay — is tracked in [specs/tech/bridges.md](/specs/tech/bridges.md).
3. **Optional `TradFiBridgeEscrow`** — Only if Linkdrop (or similar) proves insufficient. Note that the on-contract refund path is *not* a strong reason to build this: because `refundERC1155` sends the USDC refund to whoever holds the tokens, a custom escrow would only save the operator a reclaim step when the donor hasn't claimed, and does nothing once the donor has claimed (see [specs/tech/bridges.md](/specs/tech/bridges.md)). Refunds are better left to the operator operationally.
