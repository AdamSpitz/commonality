...
Product: [fund-now-claim-later.md](/specs/product/fund-now-claim-later.md). Tech: [claimable-beneficiaries.md](/specs/tech/subsystems/claimable-beneficiaries.md). This page is the adoption-story half.
...
A domain is the name donors already type. DNS + HTTPS is the world's existing claim system. The enforceable sentence is "funds reserved for the controller of `example.org`," not "funds legally reserved for Example Charity, Inc." Legal-entity IDs (EIN, charity-commission number) are layered later, and only when a verifier still has a writable proof (e.g. the official registry lists that domain). See assurance levels in the [product spec](/specs/product/fund-now-claim-later.md#assurance-levels).
