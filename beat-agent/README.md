...
**Before broad/public deploy, the service still needs:** a realistic testnet rehearsal on a narrow curated beat and pilot tuning of retrieval/source weighting. The existing defensive layers include prompt-boundary hygiene, source-diversity/time-span retrieval weighting, operator-configured source-author retrieval weights (`BEAT_AGENT_SOURCE_AUTHOR_WEIGHTS_JSON`), richer citation metadata, thin-context UI warnings, configurable UI trust-policy warnings for low-diversity ambient context, URL/CID canonical-ID validation, ingestion anomaly detection (`detectIngestionAnomalies`), and contested-observation detection (`detectContestedObservations`). The package ships a concrete Twitter/X ingestion adapter for account, query, and list sources; other platform adapters remain future work.
...
- `retrieveRelevantObservations` ranks stored observations with a local BM25-style lexical scorer over observation text, keywords, purposes, and plain-text tags, then blends in exact phrase/handle/hashtag/tag boosts, coarse recency, source-diversity/time-span, and optional operator-configured source-author weights so thinly sourced or low-trust bursty observations are still usable but down-weighted.
...
- `generateSourceManagementReport` turns those observations into a persisted advisory manager report with explicit health flags and proposed source-list update actions. With `BEAT_AGENT_LLM_EXTRACTION_ENABLED=true`, worker ticks use `createLlmSourceManagementReportGenerator`; otherwise a deterministic heuristic reporter is used. Reports are validated and persisted but not auto-applied.
...
5. when `source_management` is an active purpose, refreshes source-management observations and a persisted advisory manager report;
6. optionally runs finder mode.
...
- `BEAT_AGENT_SOURCE_AUTHOR_WEIGHTS_JSON` — optional JSON object mapping source author/account IDs to retrieval multipliers; use values below `1` to downweight noisy/spammy sources, `0` to quarantine a source from retrieval ranking, and values above `1` to upweight operator-trusted sources
