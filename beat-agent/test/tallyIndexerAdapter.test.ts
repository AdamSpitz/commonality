...
const STATEMENT_CID = 'bafkreihdwdcefgh4dqkjv67uzcmw7ojee6xedzdetojuzjevtenxquvyku';
...
describe('Tally indexer beat source adapter', () => {
  it('fetches DirectSupport events from the indexer and maps them to ingested items', async () => {
    ...
    const adapter = new TallyIndexerBeatSourceAdapter({
      ...
      fetchStatementText: async () => 'A statement about practical common ground.',
      limit: 25,
    });

    const result = await adapter.fetchSource(
      { id: 'tally:local', type: 'tally_indexer', locator: 'http://localhost:42069', platform: 'tally' },
      { lastFetchedAt: '2026-01-01T00:00:00Z', cursor: '100' },
    );

    assert.equal(requestedUrls[0], 'http://localhost:42069/api/events?eventName=DirectSupport&limit=25&blockNumber_gte=101');
    assert.equal(result.cursor, '123');
    assert.equal(result.items.length, 1);
    assert.match(result.items[0]?.contentCanonicalId ?? '', /^tally:direct-support:0x0+123:0$/);
  ...
  it('keeps source items usable when statement text is unavailable', async () => {
    const adapter = new TallyIndexerBeatSourceAdapter({
      fetch: (async () => jsonResponse({ items: [directSupportRawEvent('124', 2)] })) as typeof fetch,
      fetchStatementText: async () => null,
    });

    const result = await adapter.fetchSource(
      { id: 'tally:local', type: 'tally_indexer', locator: 'http://localhost:42069' },
      undefined,
    );
