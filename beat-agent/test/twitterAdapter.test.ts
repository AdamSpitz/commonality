...
import { createTwitterBeatSourceAdapters, TwitterBeatSourceClient } from '../src/twitterAdapter.js';
...
describe('Twitter/X beat source adapter', () => {
  it('fetches account timeline sources and maps tweets to beat-ingested items', async () => {
    ...
    const result = await client.fetchAccountSource(
      { id: 'account:alice', type: 'account', locator: '@alice', platform: 'twitter' },
    ...
    assert.deepEqual(result.items.map((item) => item.contentCanonicalId), [
      'twitter:uid:123:200',
      'twitter:uid:123:201',
    ]);
  ...
  it('creates query and list adapters using recent-search and list endpoints', async () => {
    ...
    const queryResult = await adapters.query?.fetchSource(
      { id: 'query:common-ground', type: 'query', locator: '"common ground" lang:en', platform: 'twitter' },
      undefined,
    );
    ...
    assert.equal(queryResult?.items[0]?.contentCanonicalId, 'twitter:uid:555:300');
