...
export function detectIngestionAnomalies(
  ...
  const uniqueAuthors = new Set(
    newItems.flatMap((item) => {
      ...
      if (item.authorHandle) return [item.authorHandle.toLowerCase().replace(/^@/u, '')];
      return [];
    }),
  );
  const newItemCount = newItems.length;
  const uniqueAuthorCount = uniqueAuthors.size;
  const diversityRatio = newItemCount > 0 && uniqueAuthorCount > 0
    ? uniqueAuthorCount / newItemCount
    : 1;

  if (newItemCount >= minItemsForDiversityCheck && uniqueAuthorCount > 0 && diversityRatio < lowDiversityThreshold) {
    anomalies.push({
