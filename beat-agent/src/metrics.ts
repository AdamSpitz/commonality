...
import type { ExtractObservationsSummary, CompactBeatMemorySummary } from './memory.js';
...
function buildIngestionMetrics(s: BeatIngestionRunSummary): BeatAgentIngestionMetrics {
  const skippedByReason: Partial<Record<BeatIngestionSkippedSource['reason'], number>> = {};
...
function buildMemoryMetrics(
  ...
  for (const obs of observations) {
    ...
    if (!oldestObservationAt || ts < oldestObservationAt) oldestObservationAt = ts;
    if (!newestObservationAt || ts > newestObservationAt) newestObservationAt = ts;
  }
...
function buildExtractionMetrics(s: ExtractObservationsSummary): BeatAgentExtractionMetrics {
  return {
...
function buildCompactionMetrics(s: CompactBeatMemorySummary): BeatAgentCompactionMetrics {
  return {
...
function buildEvaluationMetrics(s: CoverageGapSummary): BeatAgentEvaluationMetrics {
  const reasons: Array<{ reason: string; count: number }> = [
    { reason: 'outside_beat', count: s.byReason.outside_beat.count },
    { reason: 'insufficient_ambient_context', count: s.byReason.insufficient_ambient_context.count },
    { reason: 'insufficient_local_context', count: s.byReason.insufficient_local_context.count },
    { reason: 'unsupported_platform', count: s.byReason.unsupported_platform.count },
...
export function generateBeatAgentWorkerMetrics(
  ...
  return {
    ...
    ingestion: params.ingestionSummary ? buildIngestionMetrics(params.ingestionSummary) : null,
    memory: params.memoryObservations
      ? buildMemoryMetrics(params.memoryObservations, now, staleObservationDays)
      : null,
    extraction: params.extractionSummary ? buildExtractionMetrics(params.extractionSummary) : null,
    compaction: params.compactionSummary ? buildCompactionMetrics(params.compactionSummary) : null,
    evaluation: params.coverageSummary ? buildEvaluationMetrics(params.coverageSummary) : null,
    finder: params.finderSummary ? buildFinderMetrics(params.finderSummary) : null,
  };
}
...
export function formatBeatAgentWorkerMetricsReport(metrics: BeatAgentWorkerMetrics): string {
  const lines: string[] = [
  ...
  if (metrics.memory) {
    ...
    if (m.oldestObservationAt) {
      lines.push(`  Range: ${m.oldestObservationAt} → ${m.newestObservationAt}`);
    }
  }
