...
export function createTwitterBeatSourceAdapters(
  ...
  return {
    account: { fetchSource: (source, cursor) => client.fetchAccountSource(source, cursor) },
    query: { fetchSource: (source, cursor) => client.fetchQuerySource(source, cursor) },
    list: { fetchSource: (source, cursor) => client.fetchListSource(source, cursor) },
  };
}
...
export class TwitterBeatSourceClient {
  ...
  constructor(private readonly config: TwitterBeatSourceAdapterConfig) {
    ...
    this.maxResults = clampInteger(config.maxResults ?? DEFAULT_MAX_RESULTS, 10, 100);
  }
  ...
  async fetchAccountSource(
    ...
    return this.fetchTweetCollection(source, `/2/users/${encodeURIComponent(userId)}/tweets?${params}`);
  }
  ...
  async fetchQuerySource(
    ...
    return this.fetchTweetCollection(source, `/2/tweets/search/recent?${params}`);
  }
  ...
  async fetchListSource(
    ...
    return this.fetchTweetCollection(source, `/2/lists/${encodeURIComponent(listId)}/tweets?${params}`);
  }
  ...
  private async fetchTweetCollection(
    ...
    return {
      items,
      cursor: maxNumericId(items.map((item) => getTweetIdFromCanonicalId(item.contentCanonicalId)))
        ?? undefined,
    };
  }
  ...
  private async fetchJson<T>(path: string): Promise<T> {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Twitter/X API request failed with status ${response.status}: ${body}`);
    }
...
function addSinceId(params: URLSearchParams, cursor: BeatSourceCursor | undefined): void {
...
function getTweetIdFromCanonicalId(canonicalId: string): string | null {
  ...
  return parts.length === 4 && /^\d+$/.test(parts[3] ?? '') ? parts[3] ?? null : null;
}
