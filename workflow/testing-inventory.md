...
4. **Rendered-product judgment:** some product LLM checks still judge source/docs rather than rendered pages or screenshots. Founder-authored E2E leaves (`review.founder-e2e.*`) exist so Adam can describe a surface in his own words; they currently explore source, not a live browser session. `review.testnet-two-person-lab` is the live two-client overlay (chat-session dump-prompt). Deterministic `testnet.two-person-browser` drives two Chromium contexts on live CauseStarter and cause-assist atomize; signed UI connect is still Privy/MetaMask (headless uses lab keys via SDK when mutation is on).
