...
## Slice A — Dead surface (easy deletes)

### SDK

- Delete unused barrel `sdk/src/subsystems/index.ts` (no package export, no
  importers). Same leftover may exist under `sdk/dist/`.
- Drop unused `getContractAddresses()` and `isEventCacheAvailable()` in
  `sdk/src/utils/eventCacheClient.ts`.
- Remove unused `ts-node` from `sdk/package.json` (mocha uses `tsx` via
  `sdk/.mocharc.json`).
- Stale `dist/` ABI leftovers (`ChannelVerifierAbi`, `ERC1155SecondaryMarketAbi`,
  `MarketplaceFactoryAbi`) are regenerated output, not source of truth; `npm run
  clean && npm run build` in `sdk/` if they confuse you.

### Indexer / Hardhat

- Delete unused `indexer/src/utils/cid-types.ts` (copy of SDK; nothing in indexer
  imports it).
- Empty leftover dirs: `hardhat/contracts/conceptspace/`,
  `hardhat/contracts/fundingportals/`, `hardhat/contracts/lazy-giving/` (real
  contracts live in `statements/`, `alignment-attestations/`,
  `individual-projects/`).
- Root `package.json` script `deploy-local` points at missing
  `hardhat/scripts/deploy-local.js`. Hardhat’s own package already maps
  `deploy-local` to `scripts/deploy-incremental.js`. Fix the root script or
  remove it.

### UI

- Delete unused: `ui/src/App.css` (`main.tsx` uses `index.css`),
  `ui/src/causestarter/index.css` (never imported; `CauseShell`’s
  `className="cs-safe-bottom"` therefore does nothing — move padding into sx if
  still wanted), `ui/src/causestarter/pages/NotFoundPage.tsx` (App uses shared
  NotFoundPage), `ui/src/causestarter/wagmi.ts` (re-export of `../wagmi`),
  `ui/src/causestarter/test/setup.ts` (Vitest uses `ui/src/test/setup.ts`),
  `ui/src/causestarter/virtual-end-user-docs.d.ts` (duplicate of
  `ui/src/virtual-end-user-docs.d.ts`), unused `export const causestarterRoutes`
  in `ui/src/domains/causestarter/manifest.tsx`.
- Duplicate `loadDisplayDenylist()` in `ui/src/App.tsx` after `main.tsx` already
  awaits it — drop the extra call if still present.

### Scripts / services

- Delete `scripts/generate-beat-agent-wallet.mjs` (roles already in
  `scripts/generate-wallets.mjs`). Nothing calls it.
- Delete or document `scripts/start-tmux.sh` (zero references; pre-Compose
  windows; ignores Docker/`services.sh`).
- Fold `scripts/fund-contract-admin.mjs` into
  `scripts/fund-base-sepolia-wallets.mjs --only CONTRACT_ADMIN_ADDRESS` if
  still a duplicate of the same parseEnv/RPC/send path.
- Delete `checkExistingAttestation` in
  `services/implication-attester/src/blockchain.ts` (always `return false`; no
  callers).
- Delete duplicate `services/beat-agent/config/us-political-csm.example.json`
  (byte-identical to `services/beat-memory/config/us-political-csm.example.json`;
  README already points at beat-memory). Update
  `workflow/bridge-creator-csm-next-steps.md` if it still cites the beat-agent
  copy.
- Delete leftover beat-agent `dist/` modules that no longer exist in `src/`
  (`extractor`, `ingestion`, `memory`, `promptSafety`, `tallyIndexerAdapter`,
  `twitterAdapter` — moved to beat-memory).
- Gitignore `**/dist` consistently: `.gitignore` only lists some packages;
  `service-host/dist`, `platform-api-service/dist`, `coherence-badge-worker/dist`,
  `fake-data-generation/dist` still appear.
- `fake-data-generation/generateAttestations.ts` loads/generates attesters then
  `void attesters` — delete the stub.
- Drop unused ABI `void` imports / “suppress unused” comments in
  `fake-data-generation` (`fundingAndDelegationActions.ts`, `attackScenarios.ts`,
  `runSimulation.ts`, `generateStatements.ts`, `invariantChecker.ts`,
  `testOpenRouter.ts`).

---

## Slice B — ABI sync and indexer event registration

Silent drift: generated ABI files look official but are missing from sync maps.

- Add `RecurringPledges` and `NudgePublications` to `sdk/scripts/sync-abis.ts`.
  Both exist under `sdk/abis/` and are imported from `sdk/src/abis.ts`. Indexer
  sync already includes `NudgePublications`; neither SDK nor indexer sync
  includes `RecurringPledges`. Prefer a `--check` mode like the indexer script.
- Add `PublishedData` to `indexer/scripts/sync-abis.ts` (`PublishedDataAbi.ts` is
  marked auto-generated but is not in `CONTRACTS_TO_SYNC`). SDK already syncs it.
- Add `RecurringPledges` to indexer sync (`indexer/abis/RecurringPledgesAbi.ts`
  has no auto-generated header).
- In `indexer/ponder.config.ts`, look up factory events **by name** instead of
  `AssuranceContractFactoryAbi[0]` / `PremintingERC1155FactoryAbi[0]`.
- Delete duplicated inline event fragments `prospectiveRoundCreatedEvent`,
  `prospectiveRoundMaterializedEvent`, `creatorContractCreatedEvent` in
  `ponder.config.ts` — they already exist on the factory ABIs.
- Rename export in `indexer/abis/AssuranceContractAbi.ts`: it currently exports
  `MultiERC1155AssuranceContractAbi` and is aliased in `ponder.config.ts`.
- Register events the SDK already folds but the indexer never indexes, in
  `indexer/src/events-cache/index.ts`:
  - `Implications:ImplicationRevoked`
  - `AlignmentAttestations:AlignmentRevoked`
  - `AlignmentAttestations:SuccessRevoked`
- Collapse the copied `register("…")` lists for `AssuranceContract`,
  `CreatorAssuranceContract`, and `ProspectiveContentAssuranceContract` (same
  eight events × 3). One array of event names, three prefixes.
- `ValueThresholdConditionFactoryAbi` in `ProjectFactoriesAbi.ts` is unused —
  generate factory ABIs the way the SDK does, or stop pretending the trimmed
  event-only file is hand-maintained (`ProjectFactories: null` in the sync map).
- Child `startBlock`s in `ponder.config.ts` for factory-spawned contracts ignore
  manifest start blocks and hardcode `LAZYGIVING_START_BLOCK` /
  `CONTENT_FUNDING_START_BLOCK`. Use `deploymentStartBlock(...)` like the
  parents.
- Remove no-op `chainForContract()` in `ponder.config.ts` (every call is
  `"default"`). Dedup chain IDs vs `indexer/src/utils/chain.ts`.
- Default `ethGetLogsBlockRange: … ?? 10` for `base-sepolia` contradicts
  `indexer/README.md` (range 10 is a catch-up footgun; Render default 1000).
- Stale comments: `indexer/src/index.ts` “lightweight registry tables”;
  `ponder.schema.ts` “Phase 4”; README warning about accidental `.js` next to
  ABIs (`noEmit: true` now).

Hardhat-side related:

- `hardhat/README.md` still documents `marketplace/` and
  `ERC1155SecondaryMarket` tests that do not exist.
- `hardhat/scripts/deploy.js` duplicates `deploy-incremental.js`. README and
  `scripts/deploy-contracts.sh` tell operators to use incremental;
  `docker-compose.yml` still runs `deploy.js`.
- Env-name drift: `deploy-incremental.js` writes aliases
  (`ALIGNMENT_ATTESTATIONS_CONTRACT_ADDRESS` / `_ADDRESS` /
  `PROJECT_ALIGNMENT_CONTRACT_ADDRESS`, same for DelegatableNotes);
  `ponder.config.ts` only reads the unprefixed names.
- Same interfaces copied instead of imported: `IChannelVerifier` in
  `ChannelRegistry.sol` and `MockChannelVerifier.sol`; sliced copies of
  `IChannelRegistry` / `IChannelEscrow` across content-funding contracts.
- Extract one DelegatableNotes test fixture (copied `beforeEach` across
  `DelegatableNotes.*.test.js`, `RecurringPledges.test.js`, and a
  `SecurityRegression.test.js` block). `purchase.test.js` and `refund.test.js`
  share ~70 lines of market setup.
- Move `describe("MockChannelVerifier")` out of the ~1.8k-line
  `ContentFunding.test.js`.
- `NudgePublications.sol` has no Hardhat test; coverage baseline already notes
  this in `workflow/contract-coverage-baseline.md`.

---

## Slice C — Dedup constants and small helpers

- Unify belief-state constants. Three parallel sets:
  - `BeliefStates` in `sdk/src/subsystems/conceptspace/types.ts`
  - `NO_OPINION` / `BELIEVES` / `DISBELIEVES` in `conceptspace/actions.ts`
  - `BELIEF_NO_OPINION` etc. as `bigint` in `sdk/src/utils/chain-reads.ts`
  Keep one source (plus a bigint view if viem needs it). `getUserBeliefs` should
  stop comparing `beliefState === 1` / `=== 2` as magic numbers in
  `conceptspace/queries.ts`. Collapse `getUserBeliefs` / `getUserDisbeliefs`
  (same fetch/fold/title/excerpt mapping).
- One `erc20ApproveAbi` + `approveERC20Spend` instead of identical blobs in
  `lazy-giving/actions.ts`, `delegation/actions.ts`,
  `delegation/recurring-pledges.ts`, `content-funding/actions.ts`.
- Dedup policy-list JSON validators: `requireRecord`, `requireExactKeys`,
  `UNPAIRED_SURROGATE` copied in `sdk/src/policy-lists/subjects.ts` and
  `documents.ts`.
- Stop re-exporting ABIs from `content-funding/index.ts`
  (`ChannelRegistryAbi` / `ChannelEscrowAbi`). Consumers already import
  `@commonality/sdk/abis`.
- Delete the open question `// should this be a separate subsystem?` in
  `sdk/src/subsystems/delegation/index.ts` — either split `note-intent-actions.ts`
  or drop the comment.
- Extract `scripts/lib/parseEnvFile` used in `generate-wallets.mjs`,
  `fund-base-sepolia-wallets.mjs`, `generate-render-yaml.mjs`,
  `generate-render-secrets.mjs`, `setup-testnet-ai-policy.mjs`,
  `check-local-config-sync.mjs`, `publish-ui-to-ipfs.mjs`,
  `deployment-manifest.mjs`, etc.
- Share attester env helpers: `requireEnvFrom` / `readStringFrom` /
  `readNumberFrom` duplicated in content-attester, implication-attester,
  beat-agent, beat-memory, `service-host/src/envConfig.ts`. `attester-core`
  already has process.env variants.
- Move `wrapUntrusted` from `services/content-attester/src/evaluator.ts` to
  `services/beat-memory/src/promptSafety.ts` (beat-agent already imports the
  latter).
- Share log-poller loop + atomic JSON cursor (`state.ts`) between
  `coherence-badge-worker` and `published-data-ipfs-mirror`; same for
  `docker-entrypoint.sh`.
- Share `checkAttesterBalance` (0.01 ETH) across content-attester,
  implication-attester, beat-agent, cause-assist — belongs in `attester-core`.
- Implication-attester: `loadConfig()` still requires `ATTESTER_PRIVATE_KEY` /
  `X402_PAYMENT_ADDRESS` while `loadConfigFromEnv()` (service-host) requires
  `IMPLICATION_ATTESTER_PRIVATE_KEY`. After service-host, generated
  `services/implication-attester/.env` is leftover; make `loadConfig()` a thin
  alias and stop writing the per-package env from `setup-env.sh`.
- Port `platform-api-service/src/rateLimit.ts` (safer: instance store, `req.ip`)
  into `services/attester-core/src/rateLimit.ts` (spoofable `X-Forwarded-For`).
- Finder HTTP batch wrappers: `content-finder` and `implication-finder`
  `attesterClient.ts` both wrap `postJsonInBatches`.
- Unify eslint configs under `services/*`; `cause-assist`,
  `coherence-badge-worker`, and `published-data-ipfs-mirror` skip
  `eslint.metrics.mjs`. Comment in `eslint.metrics.mjs` about import path
  `../eslint.metrics.mjs` is already wrong for `services/*`.
