...
- [ ] **Cross-link crawler for UI/docs:** add a deterministic crawler that extracts internal UI links, docs links, and configured external links from domain manifests/pages. (Docs inventory/link existence is partly covered by `npm run check:docs-inventory` and UI docs tests; domain manifest/rendered landing-link route resolution is partly covered in `npm run test:vitest --workspace=ui -- CrossDomainSmoke`; exhaustive crawling and external-link allowlist/status checks remain pending.)
  - [ ] Internal app links render without console errors. (Absolute in-app links from public end-user docs are route-checked in `npm run test:vitest --workspace=ui -- CrossDomainSmoke` and render-checked in `npm run test:vitest --workspace=ui -- DomainDeepLinksSmoke`, both included in `npm run test:fast`; broader rendered-page crawling remains pending.)
  ...
  - [ ] External links match an allowlist or return successful HTTP status in a scheduled/non-precommit job. (Public end-user docs plus domain navigation/landing-page external hosts are allowlisted in `npm run test:vitest --workspace=ui -- ExternalLinksAllowlist`, included in `npm run test:fast`; exhaustive rendered-page crawling and live status checks remain pending.)
