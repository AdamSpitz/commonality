...
describe('cause-assist request guards', () => {
  it('allows browser requests from the testnet Commonality origin', async () => {
    const baseUrl = await start()
    const response = await fetch(`${baseUrl}/health`, {
      headers: { Origin: 'https://testnet.commonality.works' },
    })
    assert.equal(response.headers.get('access-control-allow-origin'), 'https://testnet.commonality.works')
