...
Without an API key, the suggester uses local templates and the safety filter uses heuristics only.

## Configuration

| Env | Default | Notes |
| --- | --- | --- |
| `XAI_API_KEY` / `GROK_API_KEY` / `GROK_API_Key` | — | xAI key (repo local file: `.env.grok`) |
...
```bash
# from repo root — loads .env.grok via deploy script when using Docker
set -a; source .env.grok; set +a
