...
- [ ] **Finish retrieval sources for the shared statement picker.** CauseStarter now searches the general published-statement feed before offering AI drafts, supports rejecting every result, and resolves exact text before reusing a CID. It must also search the relevant trusted Explorer `curated-collection` map before concluding that no existing statement fits. This is retrieval input, not a CauseStarter cause directory; do not add cause browse/ranking/promotion. Cross-workflow picker adoption remains tracked in the [causes-as-publications implementation plan](/specs/product/causes-as-publications-implementation-plan.md#6-reuse-the-picker-in-distinct-workflows).
