...
- [x] **Roster is a publication.** Stable `/cause/:owner/:slug` + pinned `@version`, PublishedData roster document, MutableRef tip, history UI, preview-before-publish with peer "Publish anyway", separate coherence check, on-chain positive-only coherence attestation by the **CauseStarter operator** (cause-assist key is `msg.sender`; founder never self-attests), atomic publish+updateRef when the wallet supports EIP-5792 (sequential fallback), per-plank "added later" markers from ref history.
- [ ] **Operator coherence badges via chain worker, not SPA.** After publish the browser still calls cause-assist `POST /attest-coherence` (rate-limited hot wallet). Replace with a worker that watches `MutableRefUpdater.RefUpdated`, filters to `causestarter.roster` docs, and mints badges internally. Full implementable write-up: root [`TODO.md`](../TODO.md) item *“Move operator coherence badges off the public HTTP path”*.
