...
Nudger-specific services such as `services/implication-graph-nudger/` and `services/bridge-creator/` keep their strategy logic local, and import the shared pieces from this package.
...
This keeps purely graph-based strategies such as `services/implication-graph-nudger/` from requiring unrelated OpenRouter environment variables, while still letting AI-based nudgers like `services/bridge-creator/` declare their extra requirements explicitly.
