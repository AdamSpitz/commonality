...
import type { BeatMemoryConfidence, BeatMemoryPurpose } from "./types.js";
import type {
	BeatDefinition,
	BeatIngestedItem,
	BeatSource,
...
export type BeatMemoryObservationKind =
	| "item_observation"
	| "compacted_summary";
...
export type BeatSourceManagementActionType =
	| "add"
	| "remove"
	| "downweight"
	| "upweight"
...
export interface BeatObservationExtractor {
	...
	) => Promise<ExtractedBeatObservation[]>;
}
...
export interface BeatPurposeSummarySnapshotGenerator {
	createSnapshot: (
		params: BeatPurposeSummarySnapshotGeneratorParams,
	) => Promise<BeatPurposeSummarySnapshotDraft>;
}
...
export interface BeatSourceManagementReportGenerator {
	createReport: (
		params: BeatSourceManagementReportGeneratorParams,
	) => Promise<BeatSourceManagementReportDraft>;
}
...
const defaultExtractor: BeatObservationExtractor = {
	extractObservations: async (item) => {
		...
		return [
			{
				observation: sanitizeUntrustedText(
					`${item.authorHandle ? `${item.authorHandle}: ` : ""}${text}`,
				),
				confidence: "medium",
...
export async function loadBeatContextMemoryState(
	filePath: string,
): Promise<BeatContextMemoryState> {
	try {
		...
		return {
			...
			observations: Array.isArray(parsed.observations)
				? parsed.observations.map(normalizeLoadedObservation)
				: [],
			purposeSummarySnapshots: Array.isArray(parsed.purposeSummarySnapshots)
				? parsed.purposeSummarySnapshots.map(
						normalizeLoadedPurposeSummarySnapshot,
					)
				: [],
	...
	for (const item of params.items) {
		...
		for (const [index, observation] of extracted.entries()) {
			...
			state.observations.push({
				...
				observation: normalizeWhitespace(
					sanitizeUntrustedText(observation.observation),
				),
				...
				sourceAuthors: unique(
					observation.sourceAuthors?.length
						? observation.sourceAuthors
						: getItemSourceAuthors(item),
				),
				keywords: uniqueKeywords(
					observation.keywords ?? tokenize(observation.observation),
				),
				tags: uniqueTags(observation.tags),
				purposes: normalizeObservationPurposes(
...
export async function retrieveRelevantObservations(
	...
	const candidates = state.observations
		...
		.filter((observation) =>
			observationMatchesPurposes(observation, params.purposes),
		)
		.filter(
			(observation) =>
				!params.contentCanonicalId ||
				!observation.supportingContentIds.includes(params.contentCanonicalId),
		);
	...
	const scored = candidates
		...
		.sort(
			(a, b) =>
				b.score - a.score ||
				Date.parse(b.observation.observedAtEnd) -
					Date.parse(a.observation.observedAtEnd),
		);

	return scored
		.slice(0, params.maxObservations ?? 8)
...
export async function generatePurposeSummarySnapshots(
	...
	for (const purpose of params.purposes) {
		const purposeObservations = state.observations
			.filter((observation) => observation.beatId === params.beatId)
			.filter((observation) =>
				observationMatchesPurposes(observation, [purpose]),
			);
		...
		const previousSnapshot = state.purposeSummarySnapshots
			.filter(
				(existing) =>
					existing.beatId === params.beatId && existing.purpose === purpose,
			)
			.sort((a, b) => Date.parse(b.generatedAt) - Date.parse(a.generatedAt))[0];

		const observedAtStart =
			observations.length > 0
				? minIso(observations.map((observation) => observation.observedAtStart))
				: nowIso;
		const observedAtEnd =
			observations.length > 0
				? maxIso(observations.map((observation) => observation.observedAtEnd))
				: nowIso;
		const draft = params.snapshotGenerator
			? await params.snapshotGenerator.createSnapshot({
					beatId: params.beatId,
					purpose,
			...
			: createHeuristicPurposeSummarySnapshotDraft(
					params.beatId,
					purpose,
					observations,
					params.recentMetrics,
				);
		...
		state.purposeSummarySnapshots = [
			snapshot,
			...state.purposeSummarySnapshots.filter(
				(existing) =>
					!(existing.beatId === params.beatId && existing.purpose === purpose),
			),
		];
		const keptCounts = new Map<BeatMemoryPurpose, number>();
		state.purposeSummarySnapshots = state.purposeSummarySnapshots.filter(
			(existing) => {
...
function buildSourceManagementNotes(
	...
	for (const snapshot of snapshots) {
		const weakCoverage = snapshot.sourceCoverageNotes.filter((note) =>
			/single source|low|gap|skew|limited|blocked|under-covered|over-broad|noise/iu.test(
				note,
			),
		);
		...
		for (const gap of snapshot.recurringGaps.slice(0, 2)) {
			notes.push(
				`Source-management coverage gap from ${snapshot.purpose}: ${gap}`,
			);
		}
		if (
			snapshot.factions.length > 0 &&
			snapshot.sourceCoverageNotes.some((note) =>
				/source author\(s\)|faction|skew|diversity/iu.test(note),
			)
		) {
			notes.push(
				`Source-management faction-balance signal from ${snapshot.purpose}: visible factions include ${snapshot.factions.slice(0, 4).join("; ")}.`,
			);
		}
	}
	...
	for (const note of coverageGapNotes.slice(0, 6)) {
		notes.push(
			`Source-management evaluation-demand signal: ${sanitizeUntrustedText(note)}`,
		);
	}
	for (const note of outcomeNotes.slice(0, 6)) {
		notes.push(
			`Source-management outcome signal: ${sanitizeUntrustedText(note)}`,
		);
	}
	if (currentSources.length === 0) {
		notes.push(
			"Source-management assignment signal: no current sources were supplied for inspection; manager should verify the effective beat source list is configured and inspectable.",
		);
	} else if (currentSources.length < 3) {
		notes.push(
			`Source-management assignment signal: current source list is narrow (${currentSources.length} source(s): ${currentSources.slice(0, 5).join(", ")}); watch for factional skew and under-coverage.`,
		);
	}

	return unique(
		notes.map((note) => normalizeWhitespace(note)).filter(Boolean),
	).slice(0, 12);
}
...
function createHeuristicPurposeSummarySnapshotDraft(
	...
	const keywords = topKeywords(
		observations.flatMap((observation) => observation.keywords),
		12,
	);
	const sourceAuthors = unique(
		observations.flatMap((observation) => observation.sourceAuthors),
	);
	const recentObservationTexts = observations
		.slice(0, 6)
	...
	return {
		...
		factions: extractLinesMatching(
			recentObservationTexts,
			/faction|camp|coalition|side|moderate|progressive|conservative|left|right/iu,
			4,
		),
		phraseMeanings: extractLinesMatching(
			recentObservationTexts,
			/phrase|means|meaning|used|called|reference|dog whistle|ironic/iu,
			4,
		),
		uncertainties: extractLinesMatching(
			recentObservationTexts,
			/unclear|uncertain|contested|ambiguous|unknown|may|might/iu,
			4,
		),
		recurringGaps: buildRecurringGaps(observations, recentMetrics),
		usefulContext: recentObservationTexts.slice(0, 5),
		sourceCoverageNotes: buildSourceCoverageNotes(
...
export async function generateSourceManagementObservations(
	...
	const snapshots = (state.purposeSummarySnapshots ?? [])
		.filter(
			(snapshot) =>
				snapshot.beatId === params.beatId &&
				snapshot.purpose !== "source_management",
		)
		.sort((a, b) => Date.parse(b.generatedAt) - Date.parse(a.generatedAt))
		.slice(0, 8);
	const notes = buildSourceManagementNotes(
		snapshots,
		params.currentSources ?? [],
		params.coverageGapNotes ?? [],
		params.outcomeNotes ?? [],
	);
	...
	for (const [index, note] of notes.entries()) {
		const id = buildObservationId(
			params.beatId,
			[`source-management:${nowIso.slice(0, 10)}`],
			note,
			index,
		);
		if (knownIds.has(id)) {
...
export async function generateSourceManagementReport(
	...
	const sourceManagementObservations = state.observations
		.filter(
			(observation) => observation.beatId === params.beatDefinition.beatId,
		)
		.filter((observation) =>
			observationMatchesPurposes(observation, ["source_management"]),
		)
		.sort(
			(a, b) =>
				Date.parse(b.lastActiveAt ?? b.observedAtEnd) -
	...
	const draft = params.reportGenerator
		...
		: createHeuristicSourceManagementReportDraft(
				params.beatDefinition,
				sourceManagementObservations,
				purposeSummarySnapshots,
				params.recentMetrics,
			);
	const report: BeatSourceManagementReport = {
		...
		generatedAt: nowIso,
		summary:
			normalizeWhitespace(sanitizeUntrustedText(draft.summary)) ||
			`No source-management report was produced for ${params.beatDefinition.beatId}.`,
		health: normalizeSourceManagementHealth(draft.health),
		proposedUpdates: normalizeProposedSourceUpdates(
			draft.proposedUpdates ?? [],
			params.beatDefinition.sources,
		),
		managerNotes: unique(
			(draft.managerNotes ?? [])
				.map((note: string) => normalizeWhitespace(sanitizeUntrustedText(note)))
				.filter(Boolean),
		).slice(0, 12),
		effectiveSourceList: params.beatDefinition.sources,
		sourceManagementObservationIds: sourceManagementObservations.map(
	...
	state.sourceManagementReports = [
		report,
		...(state.sourceManagementReports ?? []).filter(
			(existing) => existing.beatId !== params.beatDefinition.beatId,
		),
	].slice(0, params.maxReports ?? 10);
	await saveBeatContextMemoryState(params.memoryFilePath, state);
	return {
		generatedReportCount: 1,
		proposedUpdateCount: report.proposedUpdates.length,
	};
}
...
export async function compactBeatMemory(
	...
	const observedAtStart = minIso(
		candidates.map((observation) => observation.observedAtStart),
	);
	const observedAtEnd = maxIso(
		candidates.map((observation) => observation.observedAtEnd),
	);
	const supportingContentIds = unique(
		candidates.flatMap((observation) => observation.supportingContentIds),
	);
	const sourceAuthors = unique(
		candidates.flatMap((observation) => observation.sourceAuthors),
	);
	const keywords = topKeywords(
		candidates.flatMap((observation) => observation.keywords),
		24,
	);
	...
	if (params.compactor) {
		try {
			observationText = await params.compactor.createSummary(
				params.beatId,
				candidates,
			);
		} catch {
	...
	state.observations = [
		...state.observations.filter(
			(observation) => !candidateIds.has(observation.id),
		),
		summaryObservation,
	];
	await saveBeatContextMemoryState(params.memoryFilePath, state);
	...
	return {
		compactedObservationCount: candidates.length,
		createdSummaryCount: 1,
	};
}
...
function getExtractionErrorMetadata(
	error: unknown,
): Omit<ExtractObservationsFailedItem, "contentCanonicalId"> {
...
function normalizeLoadedPurposeSummarySnapshot(
	snapshot: Partial<BeatPurposeSummarySnapshot>,
): BeatPurposeSummarySnapshot {
	return {
		id: snapshot.id ?? "",
		beatId: snapshot.beatId ?? "",
		purpose: snapshot.purpose ?? "civility_context",
		generatedAt: snapshot.generatedAt ?? "",
		observedAtStart: snapshot.observedAtStart ?? "",
		observedAtEnd: snapshot.observedAtEnd ?? "",
		summary:
			typeof snapshot.summary === "string"
				? sanitizeUntrustedText(snapshot.summary)
				: "",
		liveTopics: Array.isArray(snapshot.liveTopics)
			? snapshot.liveTopics.filter(
					(value): value is string => typeof value === "string",
				)
			: [],
		factions: Array.isArray(snapshot.factions)
			? snapshot.factions.filter(
					(value): value is string => typeof value === "string",
				)
			: [],
		phraseMeanings: Array.isArray(snapshot.phraseMeanings)
			? snapshot.phraseMeanings.filter(
					(value): value is string => typeof value === "string",
				)
			: [],
		uncertainties: Array.isArray(snapshot.uncertainties)
			? snapshot.uncertainties.filter(
					(value): value is string => typeof value === "string",
				)
			: [],
		recurringGaps: Array.isArray(snapshot.recurringGaps)
			? snapshot.recurringGaps.filter(
					(value): value is string => typeof value === "string",
				)
			: [],
		usefulContext: Array.isArray(snapshot.usefulContext)
			? snapshot.usefulContext.filter(
					(value): value is string => typeof value === "string",
				)
			: [],
		sourceCoverageNotes: Array.isArray(snapshot.sourceCoverageNotes)
			? snapshot.sourceCoverageNotes.filter(
					(value): value is string => typeof value === "string",
				)
			: [],
		sourceObservationIds: Array.isArray(snapshot.sourceObservationIds)
			? snapshot.sourceObservationIds.filter(
					(value): value is string => typeof value === "string",
				)
			: [],
...
function normalizeLoadedSourceManagementReport(
	report: Partial<BeatSourceManagementReport>,
	...
	return {
		...
		summary:
			typeof report.summary === "string"
				? sanitizeUntrustedText(report.summary)
				: "",
		health: normalizeSourceManagementHealth(report.health),
		proposedUpdates: normalizeProposedSourceUpdates(
			report.proposedUpdates ?? [],
			report.effectiveSourceList ?? [],
		),
		managerNotes: Array.isArray(report.managerNotes)
			? report.managerNotes
					.filter((value): value is string => typeof value === "string")
					.map(sanitizeUntrustedText)
			: [],
		effectiveSourceList: Array.isArray(report.effectiveSourceList)
			? report.effectiveSourceList
					.map(normalizeBeatSource)
					.filter((source): source is BeatSource => source !== undefined)
			: [],
		sourceManagementObservationIds: Array.isArray(
			report.sourceManagementObservationIds,
		)
			? report.sourceManagementObservationIds.filter(
					(value): value is string => typeof value === "string",
				)
			: [],
	};
...
function normalizeSourceManagementHealth(
	raw: unknown,
): BeatSourceManagementHealthFlags {
	const value =
		raw && typeof raw === "object"
			? (raw as Partial<BeatSourceManagementHealthFlags>)
			: {};
	return {
...
function normalizeProposedSourceUpdates(
	...
	return updates
		...
		.map((update) => ({
			...
			sourceId:
				typeof update.sourceId === "string" ? update.sourceId : undefined,
			source: normalizeBeatSource(update.source),
			rationale: normalizeWhitespace(
				sanitizeUntrustedText(update.rationale ?? ""),
			),
			evidence: Array.isArray(update.evidence)
				? update.evidence
						.filter((value): value is string => typeof value === "string")
						.map(sanitizeUntrustedText)
						.slice(0, 8)
				: [],
			confidence: (update.confidence === "high" || update.confidence === "low"
				? update.confidence
				: "medium") as BeatMemoryConfidence,
			expectedEffect: normalizeWhitespace(
				sanitizeUntrustedText(update.expectedEffect ?? ""),
			),
		}))
		.filter(
			(update) =>
				...
				(update.action === "add"
					...
					: update.sourceId === undefined ||
						currentSourceIds.has(update.sourceId) ||
						update.action === "ask_manager" ||
						update.action === "split_beat"),
		)
		.slice(0, 8);
}

function isSourceManagementAction(
	action: unknown,
): action is BeatSourceManagementActionType {
	return (
		action === "add" ||
		action === "remove" ||
		action === "downweight" ||
...
function normalizeBeatSource(raw: unknown): BeatSource | undefined {
	...
	return {
		...
		minPollIntervalMs:
			typeof source.minPollIntervalMs === "number"
...
function createHeuristicSourceManagementReportDraft(
	...
	const joined = [
		...evidence,
		...snapshots.flatMap((snapshot) => snapshot.sourceCoverageNotes),
	].join(" ");
	const skippedSourceCount =
		typeof recentMetrics?.ingestion === "object" &&
		recentMetrics.ingestion &&
		"skippedSourceCount" in recentMetrics.ingestion
			? Number(
					(recentMetrics.ingestion as { skippedSourceCount?: unknown })
						.skippedSourceCount,
				)
			: 0;
	const health = normalizeSourceManagementHealth({
		underCovered:
			/under-covered|coverage gap|outside_beat|narrow|limited|no current sources/iu.test(
				joined,
			),
		overBroad: /over-broad|off-beat|noise|mostly off-beat/iu.test(joined),
		factionallySkewed: /skew|faction/iu.test(joined),
		blockedByApiLimits:
			skippedSourceCount > 0 || /api limit|rate limit|blocked/iu.test(joined),
		unsureAboutBeatBoundaries: /unsure|unclear|ambiguous|outside_beat/iu.test(
			joined,
		),
	});
	...
	if (health.underCovered) {
		proposedUpdates.push({
			action: "ask_manager",
			rationale:
				"Recent source-management evidence suggests coverage gaps or repeated outside-beat demand that require operator judgment before changing sources.",
			evidence: evidence.slice(0, 4),
			confidence: "medium",
			expectedEffect:
				"Clarifies whether to broaden this beat, add sources, or split demand into another beat.",
		});
	}
	if (health.overBroad) {
		proposedUpdates.push({
			action: "narrow_query",
			sourceId: beatDefinition.sources.find((source) => source.type === "query")
				?.id,
			rationale:
				"Recent evidence suggests at least one source may be producing noisy or off-beat items.",
			evidence: evidence.slice(0, 4),
			confidence: "medium",
			expectedEffect:
				"Reduces wasted extraction/evaluation work and improves beat focus.",
		});
	}
	return {
		...
		health,
		proposedUpdates,
		managerNotes: evidence.length
			? evidence.slice(0, 6)
			: [
					"No source-management observations are available yet; keep the current source list under manual review.",
				],
	};
}

function normalizeLoadedObservation(
	observation: Partial<BeatMemoryObservation>,
	...
	return {
		...
		confidence: observation.confidence ?? "medium",
		supportingContentIds: Array.isArray(observation.supportingContentIds)
			? observation.supportingContentIds
			: [],
		sourceAuthors: Array.isArray(observation.sourceAuthors)
			? unique(observation.sourceAuthors)
			: [],
		...
		lastActiveAt:
			typeof observation.lastActiveAt === "string"
				? observation.lastActiveAt
				: undefined,
	};
}
...
function getItemSourceAuthors(item: BeatIngestedItem): string[] {
	...
	if (item.platform && item.authorHandle) {
		return [
			`${item.platform}:handle:${item.authorHandle.replace(/^@/u, "").toLowerCase()}`,
		];
	}
...
function buildBm25CorpusStats(
	...
	return {
		...
		averageDocumentLength:
			observations.length === 0 ? 0 : totalLength / observations.length,
		documentFrequencies,
	};
}
...
function scoreObservation(
	...
	const lexicalScore = scoreObservationBm25(
		observation,
		queryTokens,
		corpusStats,
	);
	const boostScore = scoreObservationExactAndTagBoosts(
		observation,
		queryTokens,
	);
	...
	const recencyScore = recencyWeight(
		Date.parse(observation.lastActiveAt ?? observation.observedAtEnd),
		nowMs,
	);
...
function scoreObservationBm25(
	...
	if (
		...
		corpusStats.averageDocumentLength === 0
	)
		return 0;
	const tokens = tokenizeObservationForRetrieval(observation);
	...
	for (const queryToken of new Set(queryTokens)) {
		...
		const documentFrequency =
			corpusStats.documentFrequencies.get(queryToken) ?? 0;
		const idf = Math.log(
			1 +
		...
		const lengthNormalization =
			k1 * (1 - b + b * (tokens.length / corpusStats.averageDocumentLength));
		score += idf * ((frequency * (k1 + 1)) / (frequency + lengthNormalization));
	}
	return score;
}

function scoreObservationExactAndTagBoosts(
	observation: BeatMemoryObservation,
	queryTokens: string[],
): number {
	...
	const keywordMatches = observation.keywords.filter((keyword) =>
		queryTokenSet.has(keyword),
	).length;
	const tagMatches = tags.filter((tag) =>
		tokenize(tag).some((token) => queryTokenSet.has(token)),
	).length;
	const exactTagMatches = tags.filter((tag) =>
		normalizedQuery.includes(` ${normalizeWhitespace(tag).toLowerCase()} `),
	).length;
	...
	return (
		...
		tagMatches * 1.5 +
		exactTagMatches * 2 +
		handleOrHashtagMatches * 2
	);
}

function tokenizeObservationForRetrieval(
	observation: BeatMemoryObservation,
): string[] {
	return [
...
export function calculateObservationDiversityMultiplier(
	observation: Pick<
		BeatMemoryObservation,
		"sourceAuthors" | "observedAtStart" | "observedAtEnd"
	>,
	...
	const authorScore =
		minAuthorsForFullWeight <= 0
			? 1
			: Math.min(uniqueAuthors / minAuthorsForFullWeight, 1);
	const spanScore =
		minHoursForFullWeight <= 0
			? 1
			: Math.min(spanHours / minHoursForFullWeight, 1);
...
export function detectContestedObservations(
	...
	for (let i = 0; i < filtered.length; i++) {
		for (let j = i + 1; j < filtered.length; j++) {
			...
			if (obs1.sourceAuthors.length === 0 || obs2.sourceAuthors.length === 0)
				continue;
			...
			const hasSharedAuthors = obs2.sourceAuthors.some((a) =>
				obs1AuthorSet.has(a),
			);
...
function reinforceObservationsFromItems(
	...
	const itemKeywordSets = items.map(
		(item) => new Set(tokenize(`${item.authorHandle ?? ""} ${item.text}`)),
	);
...
function tokenize(text: string): string[] {
	...
	return uniqueKeywords(
		rawTokens.filter(
			(token) => token.length >= 2 && !tokenStopWords.has(token),
		),
	);
}
...
function uniqueTags(tags: readonly string[] | undefined): string[] | undefined {
	...
	const normalized = unique(
		tags.map((tag) => normalizeWhitespace(tag).toLowerCase()).filter(Boolean),
	);
...
function normalizeObservationPurposes(
	observationPurposes?: readonly BeatMemoryPurpose[],
	fallbackPurposes?: readonly BeatMemoryPurpose[],
): BeatMemoryPurpose[] {
	const purposes = observationPurposes?.length
		? observationPurposes
		: fallbackPurposes;
	return purposes?.length ? unique([...purposes]) : ["civility_context"];
}

function observationMatchesPurposes(
	observation: BeatMemoryObservation,
	requestedPurposes?: readonly BeatMemoryPurpose[],
): boolean {
	if (!requestedPurposes || requestedPurposes.length === 0) return true;
	return normalizeObservationPurposes(observation.purposes).some((purpose) =>
		requestedPurposes.includes(purpose),
	);
}

function mergeObservationPurposes(
	observations: BeatMemoryObservation[],
): BeatMemoryPurpose[] {
	return normalizeObservationPurposes(
		observations.flatMap((observation) => observation.purposes ?? []),
	);
}

function extractLinesMatching(
	lines: string[],
	pattern: RegExp,
	limit: number,
): string[] {
...
function buildRecurringGaps(
	observations: BeatMemoryObservation[],
	metrics?: BeatPurposeSummarySnapshot["recentMetrics"],
): string[] {
	const gaps: string[] = [];
	const lowConfidenceCount = observations.filter(
		(observation) => observation.confidence === "low",
	).length;
	const noAuthorCount = observations.filter(
		(observation) => observation.sourceAuthors.length === 0,
	).length;
	if (lowConfidenceCount > 0)
		gaps.push(
			`${lowConfidenceCount} recent observation(s) are low confidence.`,
		);
	if (noAuthorCount > 0)
		gaps.push(
			`${noAuthorCount} recent observation(s) lack source-author metadata.`,
		);
	if (
		metrics?.evaluation &&
		typeof metrics.evaluation === "object" &&
		"abstainCount" in metrics.evaluation
	) {
		gaps.push(
			"Recent evaluation metrics include abstentions; inspect coverage gaps before relying on this purpose summary.",
		);
	}
	return gaps;
}

function buildSourceCoverageNotes(
	authorCount: number,
	observationCount: number,
	metrics?: BeatPurposeSummarySnapshot["recentMetrics"],
): string[] {
	const notes = [
		`Snapshot draws on ${observationCount} observation(s) from ${authorCount} source author(s).`,
	];
	if (authorCount > 0 && authorCount < 3) {
		notes.push(
			"Source diversity is thin; treat purpose-level conclusions as tentative.",
		);
	}
	if (
		metrics?.ingestion &&
		typeof metrics.ingestion === "object" &&
		"skippedSourceCount" in metrics.ingestion
	) {
		notes.push(
			"Recent ingestion metrics are attached; check skipped sources and API limits when reviewing coverage.",
		);
	}
	return notes;
}
...
function maxIso(values: string[]): string {
	return values.length > 0
		? values.reduce((max, value) =>
				Date.parse(value) > Date.parse(max) ? value : max,
			)
		: "";
}
