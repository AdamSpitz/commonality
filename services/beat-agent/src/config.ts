...
function readConfidenceFrom(env: NodeJS.ProcessEnv): BeatAgentConfidence {
	...
	throw new Error(
		"Invalid BEAT_AGENT_MINIMUM_CONFIDENCE; expected high, medium, or low",
	);
}
...
export function loadConfigFromEnv(
	...
	return {
		...
		ethereumRpcUrl: readStringFrom(
			["BEAT_AGENT_ETHEREUM_RPC_URL", "ETHEREUM_RPC_URL"],
			env,
		),
		...
		ipfsApiUrl: readStringFrom(
			["BEAT_AGENT_IPFS_API", "IPFS_API"],
			env,
			"http://localhost:5001",
		),
		ipfsGatewayUrl: readStringFrom(
			["BEAT_AGENT_IPFS_GATEWAY", "IPFS_GATEWAY"],
			env,
			"http://localhost:8080",
		),
		paymentAddress: requireEnvFrom("BEAT_AGENT_PAYMENT_ADDRESS", env),
		serviceMarginPercent: readNumberFrom(
			["BEAT_AGENT_SERVICE_MARGIN_PERCENT", "SERVICE_MARGIN_PERCENT"],
			env,
			20,
		),
		ethUsdPrice: readNumberFrom(
			["BEAT_AGENT_ETH_USD_PRICE", "ETH_USD_PRICE"],
			env,
			3000,
		),
		gasPriceMultiplier: readNumberFrom(
			["BEAT_AGENT_GAS_PRICE_MULTIPLIER", "GAS_PRICE_MULTIPLIER"],
			env,
			1.2,
		),
		estimatedInputTokens: readNumberFrom(
			["BEAT_AGENT_ESTIMATED_INPUT_TOKENS", "ESTIMATED_INPUT_TOKENS"],
			env,
			3000,
		),
		estimatedOutputTokens: readNumberFrom(
			["BEAT_AGENT_ESTIMATED_OUTPUT_TOKENS", "ESTIMATED_OUTPUT_TOKENS"],
			env,
			500,
		),
		rateLimitWindowMs: readNumberFrom(
			["BEAT_AGENT_RATE_LIMIT_WINDOW_MS", "RATE_LIMIT_WINDOW_MS"],
			env,
			60000,
		),
		rateLimitMaxRequests: readNumberFrom(
			["BEAT_AGENT_RATE_LIMIT_MAX_REQUESTS", "RATE_LIMIT_MAX_REQUESTS"],
			env,
			10,
		),
		minimumConfidence: readConfidenceFrom(env),
		memoryFilePath: readOptionalStringFrom(["BEAT_AGENT_MEMORY_FILE"], env),
		evaluationLogFilePath: readOptionalStringFrom(
			["BEAT_AGENT_EVALUATION_LOG_FILE"],
			env,
		),
		metricsLogFilePath: readOptionalStringFrom(
			["BEAT_AGENT_METRICS_LOG_FILE"],
			env,
		),
		platformApiUrl: readOptionalStringFrom(
			["BEAT_AGENT_PLATFORM_API_URL", "PLATFORM_API_URL"],
			env,
		),
		trustedFinderKey: readOptionalStringFrom(
		...
		finderStateFilePath: readOptionalStringFrom(
			["BEAT_AGENT_FINDER_STATE_FILE"],
			env,
		),
		finderAttesterUrl: readOptionalStringFrom(
		...
		minAuthorsForFullWeight: readNumberFrom(
			["BEAT_AGENT_MIN_AUTHORS_FOR_FULL_WEIGHT"],
			env,
			3,
		),
		minHoursForFullWeight: readNumberFrom(
			["BEAT_AGENT_MIN_HOURS_FOR_FULL_WEIGHT"],
			env,
			6,
		),
		diversityNeutralFloor: readNumberFrom(
			["BEAT_AGENT_DIVERSITY_NEUTRAL_FLOOR"],
			env,
			0.25,
		),
		maxUntrustedChars: readNumberFrom(
			["BEAT_AGENT_MAX_UNTRUSTED_CHARS"],
			env,
			4000,
		),
	};
