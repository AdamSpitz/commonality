...
import { wrapUntrusted } from "@commonality/beat-memory";

export type BeatRequestJsonCompletionFn = <T>(
...
export interface EvaluateBeatContentWithLlmParams {
	...
	request: Pick<
		BeatAgentEvaluationRequest,
		"contentCanonicalId" | "declaredPerspective"
	>;
...
export async function evaluateBeatContentWithLLM(
	...
	const requestJsonCompletionFn =
		params.requestJsonCompletionFn ?? requestJsonCompletion;
...
export function buildBeatAgentPrompt(
	params: EvaluateBeatContentWithLlmParams,
): string {
	...
	const safeAmbientContext = params.context.ambientContextUsed.map(
		(citation) => ({
			...citation,
			observation: wrapUntrusted("observation", citation.observation, {
				maxChars,
			}),
		}),
	);
	...
	return params.promptTemplate
		...
		.replaceAll(
			"{content}",
			wrapUntrusted("post", params.content, { maxChars }),
		)
		.replaceAll("{declared_perspective_context}", declaredPerspectiveContext)
		.replaceAll(
			"{local_context_json}",
			JSON.stringify(safeLocalContext, null, 2),
		)
		.replaceAll(
			"{ambient_context_json}",
			JSON.stringify(safeAmbientContext, null, 2),
		);
}
...
export function normalizeBeatAgentEvaluationResult(
	...
	const abstainReason =
		decision === "abstain"
			? normalizeAbstainReason(raw.abstainReason ?? raw.abstentionReason)
			: undefined;
	const reasoning =
...
function normalizeDecision(
	decision: unknown,
): BeatAgentEvaluationResult["decision"] {
	if (
		decision === true ||
		String(decision).toLowerCase().trim() === "positive"
	) {
		return "positive";
	}
	...
	if (
		decision === false ||
		String(decision).toLowerCase().trim() === "negative"
	) {
		return "negative";
	}
...
function extractResultFromText(text: string): Record<string, unknown> {
	...
	if (
		...
		lowerText.includes('"confidence": "medium"')
	) {
		confidence = "medium";
	}
