...
The Worker fetches Pinata first using `PINATA_GATEWAY_ORIGIN` plus `PINATA_GATEWAY_KEY`. Pinata will not serve HTML from `gateway.pinata.cloud` (`ERR_ID:00023`) or from `*.mypinata.cloud` (`ERR_ID:000` — add a custom domain). A DNS-only (grey-cloud) CNAME such as `ipfs-origin.testnet.commonality.works` → `brown-racial-sailfish-957.mypinata.cloud` plus a Pinata custom-domain registration is required; an orange-cloud CNAME hits Cloudflare error 1014. Each upstream fetch is capped at a few seconds so a stalled `ipfs.io` lookup cannot 504 the Worker. Public CID gateways remain fallbacks and currently 429 (`gatewaychanges.ipfs.io`). Successful responses are cached at Cloudflare.
