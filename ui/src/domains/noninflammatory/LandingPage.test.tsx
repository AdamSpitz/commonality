...
describe('NoninflammatoryLandingPage', () => {
  it('renders the key landing-page ideas from the product spec', () => {
    ...
    expect(screen.getByText("No need to go wading through the other side's bullshit")).toBeInTheDocument()
    expect(screen.getByText("AI does the filtering so you don't have to")).toBeInTheDocument()
    expect(screen.getByRole('heading', { level: 6, name: 'Want to find out when your own side is lying to you?' })).toBeInTheDocument()
    expect(screen.getByText("Get recommendations vetted by *your* side's AI filter, for noninflammatory content from the *other* side")).toBeInTheDocument()
    ...
    expect(screen.getByRole('link', { name: 'View popular filters' })).toBeInTheDocument()
    expect(screen.getByRole('link', { name: 'View popular Civility-related statements on Tally' })).toBeInTheDocument()
    expect(screen.getByRole('link', { name: 'Explore fundable content' })).toBeInTheDocument()
    expect(screen.getByRole('link', { name: 'Nominate noninflammatory content' })).toBeInTheDocument()
  })
})
