...
describe('CreateStatementForm', () => {
  ...
  describe('when wallet is connected', () => {
    ...
    describe('IPFS and blockchain callbacks', () => {
      it('logs IPFS upload progress via onIPFSUpload callback', async () => {
        const user = userEvent.setup()
        const mockStatementData = { content: 'My statement', format: 'text/plain' }
      ...
      it('logs signing progress via onSigned callback', async () => {
        const user = userEvent.setup()
        const mockStatementData = { content: 'My statement', format: 'text/plain' }
        const consoleSpy = vi.spyOn(console, 'log')
        ...
        await waitFor(() => {
          expect(consoleSpy).toHaveBeenCalledWith('Statement signed, tx:', '0xtxhash')
        })

        consoleSpy.mockRestore()
      })

      it('logs list update progress via onListUpdated callback', async () => {
        const user = userEvent.setup()
        const mockStatementData = { content: 'My statement', format: 'text/plain' }
        const consoleSpy = vi.spyOn(console, 'log')
        ...
        await waitFor(() => {
          expect(consoleSpy).toHaveBeenCalledWith('Created statements list updated, tx:', '0xlisttxhash')
        })
