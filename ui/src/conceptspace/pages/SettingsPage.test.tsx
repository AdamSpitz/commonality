...
describe('SettingsPage', () => {
  ...
  describe('Initial rendering', () => {
    ...
    it('displays "Trusted statement-connection sources" section heading', () => {
      render(<SettingsPage />)

      expect(screen.getByText('Trusted statement-connection sources')).toBeInTheDocument()
    })
    ...
    it('displays description text about statement-connection sources', () => {
      render(<SettingsPage />)
  ...
  describe('Empty state', () => {
    it('displays empty state message when no sources configured', () => {
      render(<SettingsPage />)

      expect(screen.getByText(/no sources configured/i)).toBeInTheDocument()
    })

    it('shows "0 sources configured" count', () => {
      render(<SettingsPage />)

      expect(screen.getByText('0 sources configured')).toBeInTheDocument()
    })
  })

  describe('Loading from localStorage', () => {
    it('loads trusted sources from localStorage on mount', () => {
    ...
    it('loads multiple sources from localStorage', () => {
  ...
  describe('Adding a source', () => {
    ...
    it('persists the added source to localStorage', async () => {
      const user = userEvent.setup()
      render(<SettingsPage />)
    ...
    it('updates the source count after adding', async () => {
      const user = userEvent.setup()
      render(<SettingsPage />)
    ...
    it('removes empty state message after adding first source', async () => {
      const user = userEvent.setup()
      render(<SettingsPage />)
...
describe('Removing a source', () => {
    it('removes a source when delete button is clicked', async () => {
      localStorage.setItem(TRUSTED_ATTESTERS_KEY, JSON.stringify([VALID_ADDRESS_1, VALID_ADDRESS_2]))
      const user = userEvent.setup()
      render(<SettingsPage />)
      ...
      const stored = JSON.parse(localStorage.getItem(TRUSTED_ATTESTERS_KEY)!)
      expect(stored).not.toContain(VALID_ADDRESS_1)
    })
    ...
    it('shows empty state after removing the last source', async () => {
      localStorage.setItem(TRUSTED_ATTESTERS_KEY, JSON.stringify([VALID_ADDRESS_1]))
      const user = userEvent.setup()
      render(<SettingsPage />)
    ...
    it('updates the source count after removing', async () => {
      localStorage.setItem(TRUSTED_ATTESTERS_KEY, JSON.stringify([VALID_ADDRESS_1, VALID_ADDRESS_2]))
      const user = userEvent.setup()
      render(<SettingsPage />)
...
expect(screen.getByText(/removed/i)).toBeInTheDocument()
  ...
  describe('Source count pluralization', () => {
    ...
    it('shows plural "sources" for count of 0', () => {
      render(<SettingsPage />)

      expect(screen.getByText('0 sources configured')).toBeInTheDocument()
    })

    it('shows plural "sources" for count of 2', () => {
      localStorage.setItem(TRUSTED_ATTESTERS_KEY, JSON.stringify([VALID_ADDRESS_1, VALID_ADDRESS_2]))
