...
function getDocContent(docPath: string): LoadedDoc | null {
  ...
  if (raw === 'index' || raw === 'commonality') {
    return lookupMarkdown('commonality')
  }
  const prefixed = [
    raw,
...
function publicDocsRoute(internalPath: string): string {
  if (internalPath === 'commonality' || internalPath === 'commonality/index') return 'index'
...
function docHomeDomain(internalPath: string): string | null {
  ...
  if (top === 'shared' || top === 'commonality') return null
...
export function DocsPage() {
      ...
      <Box sx={{ maxWidth: 720 }} data-testid="docs-missing">
        <Paper variant="outlined" sx={{ p: 4, textAlign: 'center' }}>
          ...
          <Typography color="text.secondary" sx={{ mb: 3 }}>
            Try the Commonality docs home, or the jobs catalog.
          </Typography>
  ...
  const showExamples = docPath === 'index' || docPath === '' || docPath === 'commonality'
