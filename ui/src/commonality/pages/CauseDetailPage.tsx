...
export function CauseDetailPage({ editMode = false }: { editMode?: boolean }) {
  ...
  /** Commonality operator address that authors coherence badges (for viewer trust). */
  ...
  return (
    ...
    <CauseBoardLayout
      ...
      <Box>
        ...
        {hasCoherenceBadge && (
          <InfoChip
            title={`Commonality's coherence checker attested this version as coherent construction (title and description match the statements). Attested by operator ${onChainBadge!.attesters[0]}.`}
            size="small"
            color="success"
            variant="filled"
        ...
        {showCoherenceAbsence && (
          <InfoChip
            ...
            title={coherenceOperator
              ? 'Commonality\'s coherence checker has not published a badge for this version. That is not a finding that the cause is incoherent, but it does mean that we haven\'t confirmed that the title and description match the statements, so you may want to read them especially carefully.'
              : 'Could not reach Commonality\'s coherence checker, so this page cannot confirm whether a badge exists.'}
          />
        )}
        {isFreshDraft ? (
