...
function copyContractAddresses(projectRoot: string): void {
  try {
    ...
    const newLines = [
      ...
      `VITE_BENEFICIARY_REGISTRY_ADDRESS=${addresses.BENEFICIARY_REGISTRY_ADDRESS || ''}`,
      `VITE_BENEFICIARY_VERIFIER_ADDRESS=${addresses.BENEFICIARY_VERIFIER_ADDRESS || ''}`,
      `VITE_BENEFICIARY_ESCROW_ADDRESS=${addresses.BENEFICIARY_ESCROW_ADDRESS || ''}`,
