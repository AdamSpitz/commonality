...
- [ ] **CSM:** automate bridge-statement publication visibility on Tally, signing-to-movement-count propagation if implemented, and CSM links to Civility/Tally/Aligning/LazyGiving. (Cross-domain link coverage automated in `npm run test:vitest --workspace=ui -- CrossDomainSmoke`; CSM mediator configuration and Tally nudger opt-in deep-link construction are covered in `npm run test:vitest --workspace=ui -- csmMediatorNudger`; CSM product signpost links to Civility/Tally/Aligning/LazyGiving are covered in `npm run test:vitest --workspace=ui -- CsmPages`; publication/count propagation still pending.)
