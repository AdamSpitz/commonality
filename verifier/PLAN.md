...
- [ ] **Add more `known-bad.*` fixtures** for checks that are easy to accidentally make too forgiving. Done so far in this area: `known-bad.ui-test-plan` proves stale UI test-plan references are rejected, `known-bad.workflows` proves unbacked/missing-surface workflow inventory is rejected, `known-bad.docs-broken-refs` proves missing relative Markdown links are rejected, and `known-bad.env-testnet-smoke` proves unreachable configured testnet endpoints are rejected when the guarded smoke is explicitly enabled. Concrete remaining candidates:
  - [ ] `known-bad.env-testnet-smoke-malformed`: prove `env.testnet-smoke` rejects HTTP-200-but-bad responses, such as RPC responses without a usable `result`, GraphQL responses with `errors`, and app URLs serving an error/blank shell. The current unreachable-endpoint canary does not prove semantic validation of successful HTTP responses.
  - [ ] `known-bad.validation-roster`: prove `coverage.validation-roster` rejects a required manual/LLM role group that is missing, points at a nonexistent check id, or is deferred/stale without an explicit policy.
  - [ ] `known-bad.domains`: prove `coverage.domains` rejects a missing product domain, nonexistent bounded surface file, missing smoke/review/docs coverage story, or route absent from the live domain manifests.
  - [ ] `known-bad.meta-verifier-health-significance`: prove the gating threshold for meta LLM outputs is stable — high/medium verifier recommendations or significant automation candidates must make `meta.verifier-health` non-green, while low/nice-to-have-only output must remain visible but non-gating.
  - [ ] `known-bad.supervisor-freshness`: prove a supervisor with a freshness policy turns stale-but-passing child results into `uncertain`, not `pass`, while preserving concrete child `fail`/`error` status.
  ...
  - [ ] `known-bad.stack-guarded-command`: once the stack scripts emit structured health evidence, prove an exit-0 command with unhealthy service/data evidence is rejected instead of passing from exit code alone.
