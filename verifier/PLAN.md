...
- **Health evidence is partly theater.** `stack/fresh-seeded.sh` emits a *hardcoded* `pass` JSON for every probe; the structured-evidence mechanism in `guarded-command-check.mjs` is sound, but a script that always writes "pass" defeats it (the `--fail` curls + `set -e` are what actually gate today). Make the stack scripts compute each probe's evidence from its real result (capture curl status/body, emit per-probe pass/fail), so the evidence layer isn't trivially satisfiable — and so a `known-bad` fixture can prove a degraded-but-exit-0 stack is caught.
...
- **No flakiness / trend awareness.** Results are retained but nothing watches for a check that flips pass/fail/pass or for slow degradation. A leaf that's green half the time shouldn't be trusted as green. Add a `meta` check over the retained results that flags instability/flakiness and surfaces it under `meta.verifier-health`.
