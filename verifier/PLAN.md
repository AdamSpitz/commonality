...
**Items 1 and 2 are now complete** (the operations/degradation canary set; and `ai-fixtures.deterministic` extended to `explorer-curator`, with `content-finder` evaluated and deliberately excluded as a no-own-model finder). **Item 3 is now in progress:** two standing qualitative-judgment leaves are done — `review.docs-coherence` and `review.landing-compelling` (see below). The remaining leaf is `review.workflow-clarity`.
