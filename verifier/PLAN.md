...
- [ ] **Add more `known-bad.*` fixtures** for checks that are easy to accidentally make too forgiving. Done so far in this area: `known-bad.ui-test-plan` proves stale UI test-plan references are rejected, `known-bad.workflows` proves unbacked/missing-surface workflow inventory is rejected, `known-bad.docs-broken-refs` proves missing relative Markdown links are rejected, `known-bad.env-testnet-smoke` proves unreachable configured testnet endpoints are rejected when the guarded smoke is explicitly enabled, `known-bad.validation-roster` proves broken manual/LLM roster coverage is rejected, `known-bad.domains` proves broken domain coverage inventories are rejected, and `known-bad.supervisor-freshness` proves stale green child results cannot roll up as green. Concrete remaining candidates:
