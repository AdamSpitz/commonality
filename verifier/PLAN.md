...
- **Performance now has a cheap live latency probe, but not load/UX timing.** `operations.performance-budget` checks client asset size and `operations.seeded-stack-latency` probes representative HTTP endpoints on a running seeded stack with repeated samples and p95 latency budgets. The latency probe is now backed by `known-bad.seeded-stack-latency` (proves it fails on wrong-status and latency-budget breaches and does not false-fail on a healthy endpoint). Remaining: measure indexer lag under a burst of events and/or browser page TTI instead of treating HTTP endpoint latency as sufficient launch confidence.
- **Product LLM leaves judge source, not the rendered product.** The product/docs judgment leaves now run in **exploration mode** (see README operating-model): they read the docs hierarchy and source themselves via read-only tools rather than from a hand-picked bundle. But "is the landing page compelling" still really wants the *rendered* page/screenshots, and they read source TSX, not rendered HTML. Remaining: feed rendered HTML and/or Playwright screenshots into the product judgment leaves so they assess what a visitor actually sees.
