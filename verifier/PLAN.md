...
The nightly local deep cadence is available through
`scripts/verifier-nightly-deep-cadence.sh`, but its workstation cron entry is
disabled. The systemd verifier scheduler and heartbeat cron are disabled too;
routine verification is now change-driven through `verifier:work`, while deep
local/testnet evidence is an explicit `verifier:prepare` campaign. The first
successful manual deep run was on 2026-07-06: `stack.fresh-seeded`,
`operations.local-stack-health`, `stack.restart-consistency`,
`operations.indexer-lag`, `artifact.ipfs-domain-smoke`, `stack.user-journeys`,
and `stack.deployment-depth` all passed.
