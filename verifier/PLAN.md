...
- **Performance now has a cheap live latency probe, but not load/UX timing.** `operations.performance-budget` checks client asset size and `operations.seeded-stack-latency` probes representative HTTP endpoints on a running seeded stack. Remaining: measure API p95 under repeated requests, indexer lag under a burst of events, and/or browser page TTI instead of treating one-shot endpoint latency as sufficient launch confidence.
