...
- [ ] **Promote more `meta.llm-to-automated-candidates` suggestions** to deterministic checks or explicit known-gap records. Done so far: `review.docs-broken-refs` (wired into `meta.verifier-health`), `known-bad.report-attestation` coverage for incomplete/stale/blocker reports, and the `performance-acceptability` known-gap record. Keep mining future advisory suggestions for objective sub-criteria. (USER'S NOTE: do we have a verifier-check that mines all the other LLM-based tasks for these kinds of ideas? Like, watch them all, and see whether they're doing anything that could have been done by conventional code? Is that what meta.llm-to-automated-candidates is doing?)
