...
- **Performance is only bundle size.** `operations.performance-budget` checks client asset size; the test plan (`coverage/testing-plan-items.json`) names performance acceptability as a launch dimension but nothing measures real latency/throughput against the running stack (API p95, indexer lag under a burst of events, page TTI). Add a cheap but *real* latency probe against the seeded stack rather than leaving perf as a pure known-gap.
...
- **`review.docs-coherence` markdown-only false positives — largely resolved by exploration mode.** Background (2026-06-12 project-wide review): the 2026-06-11 run flagged `verifier:state` as a "stale script name" (high) and `.env.secrets.example` as non-existent (medium) — both existed, but the check couldn't see `package.json` or the file tree, so it guessed staleness from cross-doc inconsistency. Exploration mode now gives the leaf read-only repo access, so it can stat files and read `package.json` itself before judging, and the deterministic broken-link scan (`checkBrokenRefs`) still runs alongside. **Remaining (defense-in-depth):** consider pre-verifying `npm run <script>` references against root/workspace `package.json` scripts the way `checkBrokenRefs` verifies links, so the cheapest, most fakeable failures are caught deterministically rather than depending on the model choosing to look.
