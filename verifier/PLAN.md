...
- [ ] **Add more `known-bad.*` fixtures** for checks that are easy to accidentally make too forgiving. Done so far in this area: `known-bad.ui-test-plan` proves stale UI test-plan references are rejected, `known-bad.workflows` proves unbacked/missing-surface workflow inventory is rejected, and `known-bad.docs-broken-refs` proves missing relative Markdown links are rejected.
