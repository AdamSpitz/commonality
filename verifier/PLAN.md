...
- **Security facet is thin for an on-chain product.** `facet.security` rolls up only `review.security.contracts` (one attestation/LLM judgment). For the scariest surface, add deterministic backing: a static analyzer (e.g. slither) (USER'S NOTE: wait, don't we already have slither in the hardhat project?) as an automated check, and contract access-control / invariant property tests surfaced as their own leaf, so security isn't resting on a single subjective report.
