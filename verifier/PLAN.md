...
- **The deepest functionality signal is dormant.** `stack.fresh-seeded`, `stack.restart-consistency`, `artifact.ipfs-domain-smoke`, and `env.testnet-smoke` are the only checks that actually boot the stack and hit real endpoints — and in normal operation they never run (manual + opt-in env). On the dashboard "4 skipped by policy" is indistinguishable from "we have never once proven the whole system boots."
  - **Visibility + freshness gate — DONE.** `stack.deployment-depth` is a deterministic leaf under `facet.functionality` that reads the deep checks' retained history, finds the most recent run that actually *passed* (an opt-in refusal does not count), and reports "last end-to-end boot: N days ago" — `uncertain` when that proof is stale (>7-day cadence) or never happened, never `fail`, so it ages the facet toward yellow instead of letting it rest on fast unit tests. Right now it honestly reports "never booted." Backed by `known-bad.deployment-depth` (proves a refusal-only history still reads as never-booted). See README.
  - **Remaining: the cadence itself.** Still need a scheduled/CI path (e.g. nightly) that actually *runs* the deep checks with their opt-ins so `stack.deployment-depth` can flip green — that is infra outside this repo's check graph (a CI job / cron runner invoking the guarded checks with `COMMONALITY_VERIFIER_ALLOW_DESTRUCTIVE=1` etc.); wire it where the scheduler lives.
- **End-to-end data-correctness + real user-journey E2E — wired in (`stack.user-journeys`).** These were the two highest-value gaps: nothing the verifier *ran* drove the running product end-to-end or proved an on-chain write reads back identically through indexer → GraphQL → rendered UI. It turned out the proof already existed but was never executed by the verifier: the UI Playwright journey projects (`tally`, `lazyGiving`, `content-funding`) write on-chain via the SDK, wait for the Ponder indexer, then assert the rendered UI shows the value — the full round-trip — but only `artifact.ipfs-domain-artifacts` was wired in. `stack.user-journeys` now runs those journey projects as a guarded (`COMMONALITY_VERIFIER_ALLOW_E2E_STACK=1`), manual, release-candidate check under `facet.functionality`, registered in `coverage/guarded-check-policy.json`. Guarded so it adds no always-on cost; it participates in the facet's 7-day freshness aging. Remaining follow-ups: (a) strengthen at least one journey from *presence* assertions (UI shows the content text) to strict *value-equality* — read a rendered numeric/string value and assert it equals the same field fetched directly from the indexer GraphQL, so it catches a wrong-but-present value, not just a missing one; (b) add the two journeys the PLAN named but the suite lacks (newcomer donor funding/attestation, CSM movement-to-action); (c) the deployment-depth cadence item below should drive this nightly alongside the other deep stack checks.
...
- **Security facet was thin for an on-chain product — static analysis now wired in (`review.security.slither`).** `facet.security` previously rolled up only `review.security.contracts` (one attestation/LLM judgment). The user's note was right: slither already existed in the hardhat project, but only as a silent-skip step of `lint:slither` (`command -v slither … || echo skipping`) — buried in generic lint, no-op when the binary is absent, and never surfaced as security signal. `review.security.slither` now runs slither (honoring `hardhat/slither.config.json`) as a deterministic automated leaf under `facet.security`: it keys off slither's `success` flag (its process exit code reflects *detector count*, not failure), derives gating from detector impact (`High` → red, `Medium` → yellow, lower recorded but green), and goes `uncertain` — never a silent pass — when the tool is missing/times out/emits unparseable output. It honestly yellows the facet today on 2 real Medium findings (`incorrect-equality`). Backed by `known-bad.security-slither` (proves a High-impact detector forces `fail`). **Remaining:** triage/resolve (or config-exclude with rationale) the 2 Medium findings; and contract access-control / invariant property tests surfaced as their own leaf, so security isn't resting on a single subjective report plus one static analyzer.
