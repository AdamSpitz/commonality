...
- **Performance now has a cheap live latency probe, but not load/UX timing.** `operations.performance-budget` checks client asset size and `operations.seeded-stack-latency` probes representative HTTP endpoints on a running seeded stack with repeated samples and p95 latency budgets. Remaining: measure indexer lag under a burst of events and/or browser page TTI instead of treating HTTP endpoint latency as sufficient launch confidence.
