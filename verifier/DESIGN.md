...
**When touching an LLM check, first decide which kind it is** — these kinds get opposite treatment:
...
- **Founder-authored E2E checks** (`review.founder-e2e.*`): the founder writes, in their own words, what a site or use case is supposed to be; that text is stored verbatim (e.g. [`checks/review/founder-e2e/commonality.md`](./checks/review/founder-e2e/commonality.md)) and injected into the prompt **without rewriting**. The LLM looks at the built surface and reports whether it more-or-less matches, plus independent user-judgment notes. Do **not** have the check or another model "improve" the brief. Edit the `.md` when adding detail.
- **Two-person testnet lab** (`review.testnet-two-person-lab`): a chat-session / LLM walk of live Commonality as two independent clients. Deterministic mutation canaries already prove write→index; this leaf is the human-shaped overlay. Prefer `npm run verifier:llm -- review.testnet-two-person-lab`. Dual injected wallets are skipped unless the session actually has them.
