...
Voice calibration — judge only the in-scope sites (CauseStarter, Civility, Common Sense Majority). Other Vite domains may exist in the repo; do not fail the product for their copy. Apply each in-scope domain's own audience and purpose. Common Sense Majority's copy aims for a RECOGNITION register, not a PERSUASION one: it assumes the reader already shares the worldview and uses confident, knowing language. Apply that calibration to CSM-facing movement copy only. CauseStarter is the primary entry: organizer-first, cause-context signing and funding. A product or cause-specific vertical such as Civility may legitimately open with the concrete problem it solves, including frustration, when the problem is recognizable, concise, and quickly connected to the product mechanism and action. Flag problem-first copy only when it wallows in grievance, becomes preachy, or delays/obscures what was built. (If the docs describe a different intended voice, defer to the docs and note the discrepancy.)
