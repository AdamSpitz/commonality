...
Voice calibration — apply each domain's own audience and purpose; do not impose one domain's register on all eight sites. Commonality's CSM-facing copy aims for a RECOGNITION register, not a PERSUASION one: it assumes the reader already shares the worldview and uses confident, knowing language. Apply that calibration to CSM-facing movement copy only. A product or cause-specific vertical such as Civility may legitimately open with the concrete problem it solves, including frustration, when the problem is recognizable, concise, and quickly connected to the product mechanism and action. Flag problem-first copy only when it wallows in grievance, becomes preachy, or delays/obscures what was built. (If the docs describe a different intended voice, defer to the docs and note the discrepancy.)
