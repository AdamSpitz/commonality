...
First understand who the product is explicitly trying to reassure and what 'not scary' means for it (the founder/target-audience docs the README points to make this explicit). Then read the actual user-facing surface for the in-scope sites only: CauseStarter, Civility, and Common Sense Majority (landing/home pages under ui/src/domains/ and ui/src/causestarter/). Other Vite domains are out of scope. The product's explicit goal is that the screens look and feel like the giving/banking apps the user already trusts — NOT like a crypto trading terminal. If it's scary, it's not done.`
  })}
