...
emit(async () => {
  ...
  if (badConfigEndpoints.length > 0 || wrongChainConfig.length > 0 || failedScripts.length > 0 || forbiddenHits.length > 0 || missingRequired.length > 0) {
    return fail(`Testnet app config failed: ${badConfigEndpoints.length} config endpoint failure(s), ${wrongChainConfig.length} wrong chain config(s), ${failedScripts.length} script fetch failure(s), ${forbiddenHits.length} forbidden value(s), ${missingRequired.length} missing required value(s).`, { findings });
  }
