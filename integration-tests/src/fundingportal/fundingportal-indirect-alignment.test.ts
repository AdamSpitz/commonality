...
import { AlignmentAttestationsAbi, ProjectFactoryAbi, ImplicationsAbi } from '@commonality/sdk/abis';
...
import { PROJECT_ALIGNMENT_TOPIC, type AlignmentAttestationsContract, toSubjectId } from '@commonality/sdk/fundingportals';
...
import { getAlignedProjects, getIndirectlyAlignedProjects } from '@commonality/sdk/fundingportals';
...
describe('Funding Portal - Indirect Project Alignment', () => {
  ...
  it('should find projects indirectly aligned via single implication', async function() {
    ...
    assert.strictEqual(
      indirectAlignment.subjectId.toLowerCase(),
      toSubjectId(projectDetails.tokenAddress).toLowerCase(),
  ...
  it('should handle multiple projects with mixed direct and indirect alignments', async function() {
    ...
    assert.strictEqual(
      directAlignments[0].subjectId.toLowerCase(),
      toSubjectId(projectB.tokenAddress).toLowerCase(),
    ...
    assert.strictEqual(
      ...
      toSubjectId(projectA.tokenAddress).toLowerCase(),
  ...
  it('should handle multiple implication levels (S1 → S2, query by S2)', async function() {
    ...
    assert.strictEqual(
      s2IndirectAlignments[0].subjectId.toLowerCase(),
      toSubjectId(projectDetails.tokenAddress).toLowerCase()
    );
