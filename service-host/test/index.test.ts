		...
		},
		{
			serviceAppFactories: {
				"implication-graph-nudger": () =>
					createStubApp("implication-graph-nudger"),
...
describe("service host", () => {
	it("mounts routed worker apps under their configured prefixes", async () => {
		...
		try {
			const implicationResponse = await fetch(
				`${server.baseUrl}/implication-graph-nudger/quote`,
			);
			...
			const bridgeResponse = await fetch(
				`${server.baseUrl}/bridge-creator/quote`,
			);
			...
			const beatAgentResponse = await fetch(
				`${server.baseUrl}/us-political-twitter-beat-agent/quote`,
			);
	...
	it("builds the bundled service config from environment variables", () => {
		const config = loadServiceHostConfigFromEnv({
			...
			IMPLICATION_FINDER_ATTESTER_URL:
				"http://attester.example/implication-attester",
	...
	it("does not require non-enabled service env vars for an attester-only bundle", () => {
		...
		assert.deepStrictEqual(
			...
			["implication-attester", "content-attester"],
		);
		assert.deepStrictEqual(
			config.services.map((service) => service.routePrefix),
			["/implication-attester", "/content-attester"],
		);
	});
	...
	it("builds multiple instances of the same kind from SERVICE_HOST_INSTANCES", () => {
		const config = loadServiceHostConfigFromEnv({
			...
			SERVICE_HOST_INSTANCES:
				"content-attester-neutral,content-attester-left-eval-right",
			ETHEREUM_RPC_URL: "http://rpc.example",
			OPENROUTER_API_KEY: "openrouter-key",
			ALIGNMENT_ATTESTATIONS_CONTRACT_ADDRESS: "0xalignment",
			ALIGNMENT_TOPIC_STATEMENT_CID: "bafybeig",
			...
			CONTENT_ATTESTER_LEFT_EVAL_RIGHT_PROMPT_TEMPLATE:
				"Left-eval-right prompt: {{content}}",
		});
		...
		assert.strictEqual(
			config.services[0]?.routePrefix,
			"/content-attester-neutral",
		);
		assert.strictEqual(
			config.services[1]?.name,
			"content-attester-left-eval-right",
		);
		assert.strictEqual(config.services[1]?.kind, "content-attester");
		assert.strictEqual(
			config.services[1]?.routePrefix,
			"/content-attester-left-eval-right",
		);
	});
	...
	it("prefers instance-specific env vars over kind-level env vars", () => {
		const config = loadServiceHostConfigFromEnv({
			SERVICE_HOST_PORT: "3011",
			SERVICE_HOST_INSTANCES:
				"content-attester-primary,content-attester-secondary",
			ETHEREUM_RPC_URL: "http://rpc.example",
			OPENROUTER_API_KEY: "openrouter-key",
			ALIGNMENT_ATTESTATIONS_CONTRACT_ADDRESS: "0xalignment",
			ALIGNMENT_TOPIC_STATEMENT_CID: "bafybeig",
	...
	it("uses explicit route prefixes from instance-specific env vars in multi-instance mode", () => {
		const config = loadServiceHostConfigFromEnv({
			SERVICE_HOST_PORT: "3011",
			SERVICE_HOST_INSTANCES:
				"content-attester-neutral,content-attester-secondary",
			ETHEREUM_RPC_URL: "http://rpc.example",
			OPENROUTER_API_KEY: "openrouter-key",
			ALIGNMENT_ATTESTATIONS_CONTRACT_ADDRESS: "0xalignment",
			ALIGNMENT_TOPIC_STATEMENT_CID: "bafybeig",
		...
		assert.strictEqual(
			config.services[0]?.routePrefix,
			"/neutral-content-attester",
		);
		assert.strictEqual(
			config.services[1]?.routePrefix,
			"/secondary-content-attester",
		);
	});
	...
	it("builds a beat-memory service from environment variables when enabled", () => {
		const config = loadServiceHostConfigFromEnv({
			SERVICE_HOST_PORT: "3011",
			BEAT_MEMORY_ENABLED: "true",
			IMPLICATION_ATTESTER_ENABLED: "false",
			CONTENT_ATTESTER_ENABLED: "false",
			BEAT_AGENT_ENABLED: "false",
			IMPLICATION_FINDER_ENABLED: "false",
			CONTENT_FINDER_ENABLED: "false",
			IMPLICATION_GRAPH_NUDGER_ENABLED: "false",
			BRIDGE_CREATOR_ENABLED: "false",
			EXPLORER_CURATOR_ENABLED: "false",
			BEAT_MEMORY_BEAT_ID: "us-political-twitter",
			BEAT_MEMORY_PURPOSES: "civility_context,general_beat_context",
			BEAT_MEMORY_MEMORY_FILE: "./data/beat-memory.json",
			BEAT_MEMORY_ROUTE_PREFIX: "/beat-memory",
		});

		assert.strictEqual(config.services.length, 1);
		assert.strictEqual(config.services[0]?.name, "beat-memory");
		assert.strictEqual(config.services[0]?.kind, "beat-memory");
		assert.strictEqual(config.services[0]?.routePrefix, "/beat-memory");
		assert.strictEqual(
			(config.services[0]?.config as Record<string, unknown>).beatId,
			"us-political-twitter",
		);
		assert.deepStrictEqual(
			(config.services[0]?.config as Record<string, unknown>).purposes,
			["civility_context", "general_beat_context"],
		);
	});

	it("builds multiple beat-memory instances from SERVICE_HOST_INSTANCES", () => {
		const config = loadServiceHostConfigFromEnv({
			SERVICE_HOST_PORT: "3011",
			SERVICE_HOST_INSTANCES: "beat-memory-us-politics,beat-memory-local-news",
			BEAT_MEMORY_PURPOSES: "general_beat_context",
			BEAT_MEMORY_US_POLITICS_BEAT_ID: "us-politics",
			BEAT_MEMORY_LOCAL_NEWS_BEAT_ID: "local-news",
			BEAT_MEMORY_LOCAL_NEWS_PURPOSES: "bridge_opportunity_context",
		});

		assert.strictEqual(config.services.length, 2);
		assert.strictEqual(config.services[0]?.name, "beat-memory-us-politics");
		assert.strictEqual(config.services[0]?.kind, "beat-memory");
		assert.strictEqual(
			config.services[0]?.routePrefix,
			"/beat-memory-us-politics",
		);
		assert.deepStrictEqual(
			(config.services[0]?.config as Record<string, unknown>).purposes,
		...
		assert.deepStrictEqual(
			(config.services[1]?.config as Record<string, unknown>).purposes,
	...
	it("builds multiple beat-agent instances from SERVICE_HOST_INSTANCES", () => {
		...
		assert.strictEqual(
			(config.services[1]?.config as Record<string, unknown>)
				.ethereumPrivateKey,
			"0xlocalNews",
		);
	});
	...
	it("supports mixed kinds in SERVICE_HOST_INSTANCES", () => {
		const config = loadServiceHostConfigFromEnv({
			SERVICE_HOST_PORT: "3011",
			SERVICE_HOST_INSTANCES:
				"content-attester-neutral,implication-graph-nudger",
			ETHEREUM_RPC_URL: "http://rpc.example",
			OPENROUTER_API_KEY: "openrouter-key",
			ALIGNMENT_ATTESTATIONS_CONTRACT_ADDRESS: "0xalignment",
			ALIGNMENT_TOPIC_STATEMENT_CID: "bafybeig",
