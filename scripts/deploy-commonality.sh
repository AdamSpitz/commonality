#!/usr/bin/env bash
#
# Build the Commonality Docker image and deploy the container locally.
#
# Usage:
#   ./scripts/deploy-commonality.sh              # build + run on http://localhost:8090
#   ./scripts/deploy-commonality.sh --build-only
#   ./scripts/deploy-commonality.sh --stop
#
# Optional env:
#   COMMONALITY_UI_PORT                 host port (default 8090)
#   COMMONALITY_UI_IMAGE                image tag (default commonality-ui:dev)
...
COMMONALITY_UI_PORT="${COMMONALITY_UI_PORT:-8090}"
COMMONALITY_UI_IMAGE="${COMMONALITY_UI_IMAGE:-commonality-ui:dev}"
CONTAINER_NAME="${COMMONALITY_UI_CONTAINER_NAME:-commonality-ui}"
MODE="${1:-}"
...
map_contract_env() {
  ...
  export VITE_BENEFICIARY_IDENTITY_ADDRESS="${VITE_BENEFICIARY_IDENTITY_ADDRESS:-${BENEFICIARY_IDENTITY_ADDRESS:-}}"
  export VITE_BENEFICIARY_REGISTRY_ADDRESS="${VITE_BENEFICIARY_REGISTRY_ADDRESS:-${BENEFICIARY_REGISTRY_ADDRESS:-}}"
  export VITE_BENEFICIARY_ESCROW_ADDRESS="${VITE_BENEFICIARY_ESCROW_ADDRESS:-${BENEFICIARY_ESCROW_ADDRESS:-}}"
...
require_local_contract_env() {
  # Fail fast when local deploy is missing addresses Commonality needs at runtime.
  ...
  if [ "${#missing[@]}" -gt 0 ]; then
    echo "Error: Commonality local deploy is missing required contract addresses:" >&2
    printf '  %s\n' "${missing[@]}" >&2
    echo "" >&2
    ...
    echo "  ./scripts/deploy-commonality.sh" >&2
    exit 1
  fi
}

if [ "$MODE" = "--stop" ]; then
  echo "Stopping Commonality + its helper services..."
  docker rm -f "$CONTAINER_NAME" 2>/dev/null || true
  docker_compose stop commonality cause-assist alignment-trust-bootstrap 2>/dev/null || true
...
load_env_file "$ROOT/commonality-ui/.env"
...
if [ -z "${VITE_WALLETCONNECT_PROJECT_ID:-}" ]; then
  ...
  echo "  (https://cloud.reown.com) in the environment or commonality-ui/.env, then rebuild."
fi
map_contract_env
require_local_contract_env
...
ensure_local_indexer() {
  ...
  mkdir -p "$ROOT/data/hardhat" "$ROOT/data/ipfs" "$ROOT/data/ponder" ui commonality
  ...
  [ -f "$ROOT/commonality-ui/.env" ] || touch "$ROOT/commonality-ui/.env"
...
# Domain SPAs that Commonality tool cards deep-link to via *.localhost:8088.
# Same LOCAL_UI_DOMAINS switch as services.sh (default: commonality only).
...
ensure_local_tool_stack() {
  ...
  # Stable defaults used by Commonality runtime config.json tool cards.
...
echo "Building Commonality + helper service images..."
docker_compose build cause-assist alignment-trust-bootstrap commonality-ui

if [ "$MODE" = "--build-only" ]; then
  echo "Build complete: $COMMONALITY_UI_IMAGE (+ helper services)"
  exit 0
fi
...
echo "Deploying Commonality and helper services on http://localhost:${COMMONALITY_UI_PORT}/"
docker_compose up -d --force-recreate cause-assist alignment-trust-bootstrap commonality-ui christian-bridge-creator

echo "Waiting for health..."
max_attempts=40
attempt=1
while [ "$attempt" -le "$max_attempts" ]; do
  if curl --silent --show-error --fail "http://localhost:${COMMONALITY_UI_PORT}/" >/dev/null 2>&1; then
    break
  fi
  sleep 1
  attempt=$((attempt + 1))
...
if [ "$attempt" -gt "$max_attempts" ]; then
  echo "Error: Commonality did not become healthy on port ${COMMONALITY_UI_PORT}" >&2
  docker_compose logs --tail=80 commonality-ui || true
  exit 1
fi

echo ""
echo "Commonality is deployed."
echo "  URL:     http://localhost:${COMMONALITY_UI_PORT}/"
echo "  Image:   $COMMONALITY_UI_IMAGE"
echo "  Tools:   http://localhost:8088/  (admin index for domain SPAs)"
echo "  Logs:    docker compose logs -f commonality-ui"
echo "  Stop:    ./scripts/deploy-commonality.sh --stop"
echo ""
echo "Tool deep-links (open from Commonality or browser):"
...
if ! "$ROOT/scripts/check-local-config-sync.sh"; then
  echo "" >&2
  echo "Error: local config sync check failed after Commonality deploy." >&2
  exit 1
fi
