#!/usr/bin/env bash
#
# Combines deployments/<network>.env + deployments/operator-addresses.env + .env.secrets
...
SECRETS_FILE="$(commonality_service_secrets_file "$ROOT")"
OPERATOR_SECRETS_FILE="$(commonality_operator_secrets_file)"
DEPLOYMENT_FILE="$ROOT/deployments/${NETWORK}.env"
WALLETS_FILE="$ROOT/deployments/operator-addresses.env"
...
ROOT_VARS=(
	...
	DEPLOYER_ADDRESS CONTRACT_ADMIN_ADDRESS ENS_OWNER_ADDRESS IMPLICATION_ATTESTER_ADDRESS CONTENT_ATTESTER_ADDRESS BEAT_AGENT_ADDRESS
	CAUSE_ASSIST_COHERENCE_ATTESTER_ADDRESS
	BENEFICIARY_VERIFIER_TRUSTED_SIGNER_ADDRESS VERIFIER_ADDRESS
	...
	ALIGNMENT_TRUST_DENYLISTED_ADDRESS
	VITE_NONINFLAMMATORY_TOPIC_CID VITE_DEFAULT_NUDGERS VITE_CSM_MEDIATOR_NUDGER
	OPENROUTER_API_KEY OPENROUTER_MODEL XAI_API_KEY VITE_WALLETCONNECT_PROJECT_ID VITE_PRIVY_APP_ID VITE_PRIVY_CLIENT_ID
	PIMLICO_API_KEY BASE_SEPOLIA_BUNDLER_URL BASE_SEPOLIA_PAYMASTER_URL BASE_BUNDLER_URL BASE_PAYMASTER_URL
	...
	BEAT_MEMORY_ENABLED BEAT_MEMORY_BEAT_ID BEAT_MEMORY_PURPOSES BEAT_MEMORY_BEAT_DEFINITION_JSON BEAT_MEMORY_BEAT_DEFINITION_FILE
	BEAT_MEMORY_INGESTION_STATE_FILE BEAT_MEMORY_MEMORY_FILE BEAT_MEMORY_METRICS_LOG_FILE BEAT_MEMORY_WORKER_POLL_INTERVAL_MS BEAT_MEMORY_LLM_EXTRACTION_ENABLED
	BEAT_AGENT_ENABLED BEAT_AGENT_BEAT_ID BEAT_AGENT_NAME BEAT_AGENT_PROMPT_TEMPLATE BEAT_AGENT_PROMPT_TEMPLATE_FILE BEAT_AGENT_INGESTION_STATE_FILE BEAT_AGENT_MEMORY_FILE
	BEAT_AGENT_EVALUATION_LOG_FILE BEAT_AGENT_METRICS_LOG_FILE BEAT_AGENT_FINDER_ENABLED BEAT_AGENT_FINDER_STATE_FILE BEAT_AGENT_FINDER_ATTESTER_URL BEAT_AGENT_BEAT_KEYWORDS
...
# ============================================================
{
	...
	echo "VITE_PRIVY_APP_ID=${VARS[VITE_PRIVY_APP_ID]:-}"
	echo "VITE_PRIVY_CLIENT_ID=${VARS[VITE_PRIVY_CLIENT_ID]:-}"
	if [ "${VARS[COMMONALITY_ENVIRONMENT]:-local}" = "mainnet" ]; then
		echo "VITE_PRIVY_SMART_WALLET_BUNDLER_URL=${VARS[BASE_BUNDLER_URL]:-}"
		echo "VITE_PRIVY_SMART_WALLET_PAYMASTER_URL=${VARS[BASE_PAYMASTER_URL]:-}"
	else
	...
	echo "VITE_PUBLISHED_DATA_CONTRACT_ADDRESS=${VARS[PUBLISHED_DATA_CONTRACT_ADDRESS]:-}"
	...
	echo "VITE_BENEFICIARY_IDENTITY_ADDRESS=${VARS[BENEFICIARY_IDENTITY_ADDRESS]:-}"
	echo "VITE_BENEFICIARY_REGISTRY_ADDRESS=${VARS[BENEFICIARY_REGISTRY_ADDRESS]:-}"
	echo "VITE_BENEFICIARY_VERIFIER_ADDRESS=${VARS[BENEFICIARY_VERIFIER_ADDRESS]:-}"
	echo "VITE_BENEFICIARY_ESCROW_ADDRESS=${VARS[BENEFICIARY_ESCROW_ADDRESS]:-}"
	...
	echo "VITE_DEFAULT_ALIGNMENT_TRUST_ROOT=${VARS[VITE_DEFAULT_ALIGNMENT_TRUST_ROOT]:-}"
	...
	echo "VITE_TEST_DATA_REGISTRY_URL=${VARS[VITE_TEST_DATA_REGISTRY_URL]:-}"
} >"$ROOT/ui/.env"
