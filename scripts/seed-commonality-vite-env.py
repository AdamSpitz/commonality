#!/usr/bin/env python3
"""Seed ui/.env and commonality-ui/.env from Docker Commonality config.json.

Requires Commonality Docker on http://localhost:8090 (or COMMONALITY_CONFIG_URL).
...
OUTS = (ROOT / "ui" / ".env", ROOT / "commonality-ui" / ".env")
CONFIG_URL = os.environ.get("COMMONALITY_CONFIG_URL", "http://localhost:8090/config.json")
...
def main() -> int:
    try:
        ...
        print("Start Docker Commonality first: ./scripts/deploy-commonality.sh", file=sys.stderr)
        return 1
    ...
    lines = [
        "# Auto-seeded from Docker Commonality config.json for local Vite dev.",
        "# Docker stack stays up for hardhat/indexer/IPFS/gateway; use http://localhost:5174",
        f"# Source: {CONFIG_URL}",
        "# Re-run: python3 scripts/seed-commonality-vite-env.py",
        "",
