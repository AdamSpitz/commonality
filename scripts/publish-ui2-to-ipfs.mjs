...
const buildDomain = 'causestarter'
const distDir = path.join(rootDir, 'ui2', 'dist')
const artifactDir = process.env.UI_IPFS_ARTIFACT_DIR || path.join(rootDir, 'data', 'ui-ipfs', buildDomain)
...
async function loadUiBuildEnvFromFiles() {
  ...
  const ui2Env = await loadEnvFile(path.join(rootDir, 'ui2', '.env'))
  const env = { ...uiEnv, ...ui2Env }
...
async function main() {
  ...
  console.log('Building CauseStarter (ui2) for IPFS...')
  runOrThrow('npm', ['run', 'build', '--workspace=@commonality/sdk'], { env: buildEnv })
  runOrThrow('npm', ['run', 'build', '--workspace=ui2'], {
    env: {
      ...buildEnv,
  ...
  console.log('')
  console.log('CauseStarter published to local IPFS.')
